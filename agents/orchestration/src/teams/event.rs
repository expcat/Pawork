...
//!   event-sourcing：命令面校验后构造事件 → `apply` 折叠进 [`crate::teams::state::TeamAggregate`]
...
/// 共享任务板上的一个任务条目（事件快照；与 [`crate::AgentTask`] 同构，
...
///
/// 每个变体都是一次不可变的协作事实；[`crate::teams::state::apply`] 是其纯函数折叠，
