...
//!
//! mailbox 与 run loop 解耦：消息持久化（[`crate::teams::event::TeamEvent::MailboxPosted`]
//! 即事实），worker 按需 [`pull`] 拉取，拉取产生
//! [`crate::teams::event::TeamEvent::MailboxDelivered`]，标记已读产生
//! [`crate::teams::event::TeamEvent::MailboxRead`]。广播消息对除发送者外的全员可见。
...
use crate::teams::event::Recipients;
use crate::teams::ids::MailboxMessageId;
use crate::teams::state::{MailboxEntry, TeamAggregate};
...
mod tests {
    use super::*;
    use crate::teams::event::TeamEvent;
    use crate::teams::ids::{MemberRole, TeamId};
    use crate::teams::state::apply;
    use pawork_domain::TenantId;
