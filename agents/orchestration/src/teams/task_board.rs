//! 共享任务板命令校验（pure）。
//!
//! 复用 P12 [`crate::TaskId`] / [`crate::TaskState`] 图结构与
//! 依赖语义，但 owner 由「团队认领」决定（[`crate::teams::event::BoardTask::owner`]）。
//! 本模块只做命令面校验并构造事件，事件落地与状态折叠由 [`crate::teams::service`] /
//! [`crate::teams::state::apply`] 负责——不写 run loop、不执行任务。

use pawork_domain::AgentId;
use crate::{TaskId, TaskState};

use crate::teams::error::TeamError;
use crate::teams::event::BoardTask;
use crate::teams::state::{is_legal_task_transition, TeamAggregate};
