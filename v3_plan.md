...
| **下一波次** | R6 波 B(engine compact 分支水位 ∥ host resume/fork 收编 + desktop 投影分支语义;storage API 接缝已冻结) |
| 阻塞 | 无 |
