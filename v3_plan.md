...
| 字段 | 值 |
| --- | --- |
| 当前阶段 | R4([plan/R4-host-decomposition.md](plan/R4-host-decomposition.md))🔵 进行中 |
| 阶段状态 | R0 🟢(波 0/A/B/C 全部收口,2026-08-18;改判 3+1 项见 ADR-038 落实改判记录);R1 🟢(波 A–E 全部收口,2026-08-19);R2 🟢(波 A/B/C/D 全部收口,2026-08-20);R3 🟢(波 A/B/C/D 全部收口 + 整阶段审计修复,2026-08-20~21);R4 🔵(波 A 进行中,2026-08-21);R5–R9 ⚪ |
...
| R4 波 A 进度 | 阶段1 部分落地(2026-08-21):抽出 UsageService/TaskService/ImportService/ExtensionService 四服务,AppCore 门面委托、对外 pub API 形状不变;测试随迁 12 条到服务模块 + 共享 testsupport;lib.rs 4131→3477,gui_host 仍 2402(巨 match 未改);Session/Run/Approval 未拆,行数目标未达成。`cargo test -p pawork-app` 121 绿(1 ignored)。审查修 import.rs 未用 SessionCompatReport 别名。glm_worker 429 中断,未跨波 |
| **下一波次** | R4 波 A 剩余(Session/Run/Approval 抽取 + gui_host 巨 match 改 registry 分发;crates/app;串行单一 owner;无 ADR 闸门) |
| 阻塞 | 无 |
