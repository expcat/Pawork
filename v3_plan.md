...
| 字段 | 值 |
| --- | --- |
| 当前阶段 | R3([plan/R3-protocol-unification.md](plan/R3-protocol-unification.md))⚪ 未开始 |
| 阶段状态 | R0 🟢(波 0/A/B/C 全部收口,2026-08-18;改判 3+1 项见 ADR-038 落实改判记录);R1 🟢(波 A–E 全部收口,2026-08-19);R2 🟢(波 A/B/C/D 全部收口,2026-08-20);R3–R9 ⚪ |
| 已完成波次 | R0 波 0(ADR-038)、R0 波 A(大块归档)、R0 波 B(小块删除与降级)、R0 波 C(D16 git 服务裁剪 + 收口,2026-08-18;补判 commit.rs 归档)、R1 波 A(ADR-039 Accepted + api→domain golden 先行平移 + diagnostics 迁宿主撤包,2026-08-19;members 37→35)、R1 波 B(storage/providers/workspace 三大合并 + host/app 装配缝,2026-08-19;members 35→28)、R1 波 C(mcp→tools ∥ quota+provider-control→control-plane + host/app 装配缝,2026-08-19;members 28→25)、R1 波 D(gui-server→app `gui_server/` ∥ channels→cli `channels/` ∥ sdk→client `headless/` + probe→client tests/example,2026-08-19;members 25→21)、R1 波 E(members 定稿 21 + 19 库 `git mv` 扁平 `crates/` + design.md §2 重写 + 红线断言随迁 + 21 包定向测试 + 真实冒烟,2026-08-19;整阶段审查已修复 probe 暴露的动态模型切换与 client 错误帧路由缺陷,并收紧三条红线回归;修复后 desktop probe 全绿)、R2 波 A(L1 rand→getrandom 6 点 + L2 parking_lot→std::sync 52 处含 orchestration 死声明 + L3 base64→auth 本地 base64url 模块,对拍 golden 先行后固化 13 组固定向量,2026-08-19;rand/parking_lot/base64 退出直接依赖,根 workspace 声明已清)、R2 波 B(升级 U1–U8+U10 九项全落地,2026-08-20:notify 8.2(debouncer 死声明删,file_index 零适配)+ portable-pty 0.9(官方 signal() 替 Display 解析 hack,甩 nix 0.25 老栈)+ windows 0.61.3(0.58 退出,2 处适配,msvc 交叉 check 绿)+ ts-rs 12.0.1(7 个 .d.ts 索引签名去 ? 属形状变化,用户拍板 A 接受并登记)+ reqwest 0.13.4(上游强制 rustls-tls→rustls+form,TLS 信任栈 webpki-roots→rustls-platform-verifier 与 cmake 构建依赖已登记,redirect/proxy 语义不变)+ toml 1.1.4(47 测试双绿)+ rusqlite 0.40.2(SQLite 3.53.2,backup/迁移回归绿)+ sha2 0.11(RFC 7636 golden 字节不变)+ directories 6.0.0(macOS 路径快照 golden×2 先行,逐字节一致);lock 836→830,CLI 闭包多版本清零,desktop 树例外 sha2 0.10.9/toml 0.8.23/windows 0.57 登记;审查 F2/F3 文档修正与加固候选已落任务书),R2 波 C(rmcp =2.2.0→=3.1.3 升级决议落地,2026-08-20:64 条 MCP 契约测试 + 隔离断言 + 真实 stdio server 冒烟与基线逐字节一致;codec.rs 17 行 fail-closed 适配(InputRequiredResult 专名措辞、EchoServer 返回 CallToolResponse),dev 死声明 macros 移除;MSRV 1.85→1.88(rmcp 3.x 为 edition 2024),lock 830→826)、R2 波 D(收口断言,2026-08-20:`cargo tree -d` 断言 notify 8.2.0/reqwest 0.13.4 单版本、windows 0.58 退出;desktop 树例外四项核实(sha2 0.10.9←gpui_http_client、toml 0.8.23←cbindgen→gpui、windows 0.57←sysinfo←zed-scap、thiserror 1.x 传递);CLI 闭包传递残留登记 base64 0.22/0.23(reqwest/rmcp)、syn 2/3(async-trait)、thiserror 1.x(portable-pty→filedescriptor),直控面清零;lock 836→826 净 -10;增量编译同协议对比 2.1s→2.3s 无回退,干净全量 check 单元 242→221;`cargo check -p pawork -p pawork-desktop` 双绿;xAI OAuth 自动刷新 + MCP stdio 冒烟四项通过;raw 输出归档 plan/R2-cargo-tree-duplicates-2026-08-20.txt) |
| **下一波次** | R3 波 A(按 [plan/R3-protocol-unification.md](plan/R3-protocol-unification.md) §3 波 A:Registry 设计 + protocol 内落地(表驱动或 const fn;含 17+9 条帧 golden 复核);GUI 通道切换到 registry 派生(gui_host dispatch 宣告/授权走 registry,巨 match 拆解留 R4);串行契约面;无 ADR 闸门,硬前置 R1 已 🟢) |
| 阻塞 | 无 |
