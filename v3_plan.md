...
| 字段 | 值 |
| --- | --- |
| 当前阶段 | R2([plan/R2-dependency-governance.md](plan/R2-dependency-governance.md))🔵 进行中(波 A/B/C 已收口) |
| 阶段状态 | R0 🟢(波 0/A/B/C 全部收口,2026-08-18;改判 3+1 项见 ADR-038 落实改判记录);R1 🟢(波 A–E 全部收口,2026-08-19);R2 🔵(波 A/B/C 收口);R3–R9 ⚪ |
...
| **下一波次** | R2 波 D(收口,按 [plan/R2-dependency-governance.md](plan/R2-dependency-governance.md) §4 波 D:`cargo tree -d` 重复断言(notify/reqwest 多版本与 windows 0.58 位消失),desktop 树例外登记(sha2 0.10.9←gpui_http_client、toml 0.8.23←cbindgen→gpui、windows 0.57←sysinfo、thiserror 1.x 传递),lock 包数前后与增量编译耗时对比(波 C 收口实测 826);无 ADR 闸门) |
| 阻塞 | 无 |
