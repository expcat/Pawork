...
| R8 波 C 虚拟化人工验收 | 波 C(2026-08-24)Timeline 改 gpui list() 变高虚拟化:长会话滚动/回底重挂/Entry 菜单锚点/truncate 四例渲染行为无自动门禁,并入波 E K-03;P3-4 已知差异——Entry「···」菜单开着时条目滚出可视区被卸载,浮层消失但 open_menu 状态残留(滚回自现、Escape/外点仍有效),波 E 定夺修复或接受 | 波 E K-03 人工验收 |
...
| R8 波 D mod.rs 行数回弹 | ui/mod.rs 波 C 824 行 → 波 D 收口 1031 行(三页签接线 + 审查修复),超 900 阶段目标;changes.rs(705)/resources.rs(210)已拆出,余量有限——波 E 重瘦或在 R8 任务书修订退出标准 | 波 E 定夺 |
