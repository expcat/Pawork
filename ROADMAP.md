...
| --- | --- |
| 活动线 | **Settings — 模型与供应商** |
| 状态 | 🟢 SET-1/SET-2 已审查提交（协议词汇 + Host settings 门面）；SET-3 Settings 壳已审查提交（只读供应商页接通）；写操作尚未开放。 |
...
| 下一动作 | SET-4：四家认证闭环（Z.AI/DeepSeek API key；Kimi API key + OAuth；xAI OAuth + API key）。 |
