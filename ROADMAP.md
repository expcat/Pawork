...
| [R2](plan/R2-dependency-governance.md) | 依赖治理 | rand/parking_lot/base64 本地化;notify 8、windows 0.61、portable-pty 0.9、ts-rs 12、reqwest 0.13、toml 1.1、rusqlite 0.40、sha2 0.11 升级;lock 多版本去重断言;rmcp 3.x 专项 | 各 crate Cargo.toml + 少量调用点 | R1 | 🟢(波 A ✅ 2026-08-19,L1–L3 本地化收口:rand/parking_lot/base64 退出直接依赖,getrandom 0.4 统一 OS 熵,auth 本地 base64url 含对拍固定向量;波 B ✅ 2026-08-20,U1–U8+U10 九项落地:notify 8.2/windows 0.61.3/portable-pty 0.9/ts-rs 12(7 文件形状变化用户拍板接受)/reqwest 0.13.4(rustls 栈切换已登记)/toml 1.1.4/rusqlite 0.40.2/sha2 0.11/directories 6(macOS 快照 golden×2),lock 836→830;波 C ✅ 2026-08-20 rmcp 专项决议升级落地:=2.2.0→=3.1.3(64 条 MCP 契约测试 + 隔离断言 + 真实 stdio server 冒烟与基线逐字节一致;codec.rs 17 行 fail-closed 适配,dev 死声明 macros 移除;MSRV 1.85→1.88,lock 830→826);波 D ✅ 2026-08-20 收口:`cargo tree -d` 断言 notify/reqwest 单版本、windows 0.58 退出,desktop 树例外四项核实,CLI 闭包传递残留 base64 0.22/0.23、syn 2/3、thiserror 1.x 登记(直控面清零);lock 836→826 净 -10;增量编译同协议 2.1s→2.3s 无回退,全量 check 单元 242→221;check pawork+desktop 双绿;xAI OAuth 刷新 + MCP stdio 冒烟四项通过) |
...
| rmcp 3.x | ✅ 已决议升级(R2 波 C 2026-08-20:锁 `=3.1.3`;64 条 MCP 契约测试 + 隔离断言全绿,真实 stdio server 冒烟与 2.2.0 基线逐字节一致;codec.rs 17 行 fail-closed 适配,dev 死声明 macros 移除,MSRV 1.85→1.88,lock 830→826;复评条件:rmcp 下个 major 或 wire 协议变化时重跑同套门) | R2 波 C |
| 上游传递多版本残留(base64/syn/thiserror) | R2 波 D 实测 CLI 闭包:base64 0.22.1(reqwest 0.13.4→hyper-util)/0.23.1(rmcp 3.1.3)、syn 2.0.119/3.0.3(async-trait 0.1.92)、thiserror 1.0.69(portable-pty 0.9→filedescriptor 0.8.3;desktop 树另有 async_zip/postage/tokio-socks 传递);均上游传递,pawork 直控面已单版本;随上游对齐版本自然消除,无独立动作 | R9 复跑 `cargo tree -d` 核对 |
| directories 5→6 | 目录语义兼容(`dev.pawork.pawork` 布局)评估后升级或显式锁定 | ✅ 已决议升级(R2 波 B:6.0.0,macOS 路径逐字节不变,快照 golden×2 先行,2026-08-20) |
