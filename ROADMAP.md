...
| [R3](plan/R3-protocol-unification.md) | 协议与投影同源化(T3+T5) | 单一 command/capability registry,GUI 帧/headless/ACP 三通道 mapping 同源派生(宣告=授权=实现);Timeline 投影 reducer 下沉 protocol 共享模块,host/desktop 同源 + 投影 golden;OnFailure 档位裁决 | protocol、app、cli(headless/acp)、client、desktop projection | R1(R2 可并行) | 🔵(波 A ✅ registry 落地 + GUI 通道切派生;波 B ✅ headless/ACP 切 registry;波 C ✅ 2026-08-20:投影 reducer 下沉 protocol::projection + 三消费点同源 + 投影 golden 三种子与两端对拍,CR08-08 根治;probe-smoke 真实冒烟通过) |
