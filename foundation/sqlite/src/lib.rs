//! Pawork 串行 SQLite Actor、备份与通用 migration 框架。
//!
//! 每个 [`DatabaseActor`] 在专用线程持有唯一的 `rusqlite::Connection`。异步调用方
//! 只能通过有界命令通道提交闭包，不能取得或共享连接，因此数据库写入天然串行化。
//!
//! 本 crate 不持任何业务 schema，也不定义 session 表。业务迁移由各 owner 包
//! 通过 [`migrate`] / [`schema_version`] 在独立账本表上执行。
