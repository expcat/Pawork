...
- 开发期**尽量少测试**：每阶段只保留**关键模块级测试**（能证明该阶段核心行为的最小集合）；不设阶段门禁，与 [../v1-migration-reference.md](../v1-migration-reference.md) §6「开发期无门禁」一致并强化，全量门禁集中在 S12。
...
| release 目标 | ≥ 99% | S12 Release Gate：三场景均值 ≥99%，单场景仍不得低于低目标 |

- **执行方式**：真实 provider 打点（凭证遵守 §1.1，缺失即终止）；默认不进 CI，本地/手动触发；结果随 `pawork usage` 与事件流可查。
- **落点**：S6 起首个对话场景命中测试；S11 补 agent/长任务/多 Agent 场景；S12 纳入 Release Gate。
...
| D8 | 缓存命中率目标 95/97/99 与真实命中测试 | ✅ 已确认（2026-08-14） | 见 §1.3；S6 对话场景起步，S11 补全场景，S12 纳入 Release Gate |
...
| `docs/design.md` + `ROADMAP.md` | design.md §5 G 表状态列更新；ROADMAP.md §3.2 任务状态更新（按 ROADMAP §6 状态回写约定） |
