...
1. 把 V1 的 88 crate / 约 23.6 万行重组为 **40 包 + 3 应用**（`pawork`、`protocol-probe`、S7 起的 `apps/desktop`；独立 Cargo workspace `Pawork_v2/`），可独立发布，约 15 个高外部价值包按 W1–W4 波次发布 crates.io（§7）。插件四包目录预留、本轮不激活。
...
| 功能 | 参照 |
| --- | --- |
| 先锁定最小 Agent 信息架构，再实现本机单窗口 | [gui-design.md](gui-design.md)；Codex Desktop 主对话壳；OpenCode Desktop/Web 的流式+工具行；根仓 [desktop-gui.md](../../docs/features/desktop-gui.md) 四层，**不**搬 P19 全量 Surface |
