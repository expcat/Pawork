...
>
> 关联文档：[../ROADMAP.md](../ROADMAP.md)（任务总索引）· [../plan/](../plan/)（逐阶段任务书）· [references.md](references.md)（参照项目手册）· [task-guide.md](task-guide.md)（任务实现规范）· [v1-migration-reference.md](v1-migration-reference.md)（V1 Review 与迁移词典全文）。
...
1. 把 V1 的 88 crate / 约 23.6 万行重组为 **40 包 + 2 应用**（独立 Cargo workspace `Pawork_v2/`），可独立发布，约 15 个高外部价值包按 W1–W4 波次发布 crates.io（§7）。
2. **纵向优先**：先交付内置工具真实接线、能在真实仓库完成编码任务的 CLI Coding Agent，再横向合入扩展生态与控制面——直接矫正 V1「组件齐全、主干未通电」病灶（[v1-migration-reference.md](v1-migration-reference.md) §1.2）。
...
| `pawork-session` | S1（event store 核心） | S5（`compaction`）、S8（导入器 `import::formats`）、S9（lifecycle + 分支/Fork UX） | V1 migration 序列全量复用；分支数据层 S1 即就绪 |
| `pawork-testkit` | S2（MockProvider 最小） | 随各域契约补 mock/断言 | |
| `pawork-workspace` | S2（roots/相对路径） | S8（file-index + `@file`） | |
...
| `pawork-blob-store` | S7（artifact/protected/checkpoint） | — | `PWB1` golden 先行 |
| `pawork-mcp` | S8（含 rmcp 收口） | — | |
| `pawork-resources` | S8 | — | AGENTS.md/Skills/profiles |
| `pawork-compat` | S8 | — | 五来源只读导入 |
| `pawork-protocol` | S9（六合一 + typegen） | — | `--json` unstable → 正式协议 |
| `pawork-transport` | S9 | — | remote feature 锁 rustls |
| `pawork-gui-server` | S9 | — | Resume/Replay/慢客户端隔离 |
| `pawork-sdk` | S9 | — | headless --json-stdio |
| `pawork-client` | S9 | — | GPUI Desktop 接入口预留 |
| `pawork-channels` | S9（acp 优先） | codex/claude/remote-control 按需 feature | |
| `protocol-probe`（bin） | S9 | — | 协议自检 |
| `pawork-wasm-host` | S10 | — | |
| `pawork-plugin` | S10（+`market`） | — | 验签收敛单一 signing 模块 |
| `pawork-hooks` | S10 | — | 与 wasm lifecycle 信任域分离 |
| `pawork-lsp` | S10 | — | resource/sandbox 注入 |
...
| 配置 schema | `config-service`：TOML、`ConfigTier`（Builtin<Global<Profile<Workspace<Session<Run）、`PaworkConfig`/`ProviderConfig{id, base_url}`（**无 api_key 字段**） | S0 最小 / S8 完整 | S0 只实现 Builtin+Global+Workspace 三层读取与合并，但 schema 字段与文件位置照抄 V1；Profile/Session/Run 层 S8 补齐 |
...
| blob 格式 | `PWB1` + protected AEAD 边界（ADR-032） | S7 | golden 先行 |
| GUI 协议 | `gui-protocol` 帧（ADR-036）、headless-json、core-api | S9 | S1 起的 `--json` 输出标注 **unstable**，S9 对齐正式 headless 协议 |
...
| `pawork run`（非交互单次）+ `--json` JSONL 事件流（unstable） | Codex exec / headless 输出形态；S9 对齐正式 headless 协议 |
...
| 输出截断 + 完整输出落工件 | 上下文预算纪律（S5 铺垫、S7 artifact 接管）；对照 research §5.3 前缀稳定技巧 |
...
### S8 MCP、资源与兼容导入（[任务书](../plan/S8-mcp-resources.md)）
...
### S9 服务化与客户端（[任务书](../plan/S9-serve-clients.md)）
...
| `pawork gui serve` 多客户端并发 + 断线 Replay + 慢客户端隔离 | V1 gui-server 资产（外部无直接对标） |
...
| PTY 交互式命令 | V1 pty-service（PTY 重连语义） |

### S10 扩展生态（[任务书](../plan/S10-extensions.md)）

| 功能 | 参照 |
| --- | --- |
| WASM 插件安装→注册→撤销（Ed25519+blake3 验签、market 源） | OpenCode JS 插件生态为**红线排除的反例**（§6.1，WASM 是纯 Rust 替代路线）；Pi Packages 轻量能力包（候选 §6.4 C1）；Codex 连接器目录（候选 §6.4 C3） |
| 用户 Hooks（pre/post-tool、turn 边界、可短路） | OpenCode `tool.execute.before/after`（JS 形态被排除，语义对标）；V1 user-hooks 注入式执行器 |
| LSP 语义查询工具（定义/引用/诊断） | V1 lsp-runtime（async LSP client，生态稀缺）；OpenCode LSP 自动安装矩阵为候选增强（§6.4 C4） |
...
| G4 | 子 Agent 声明式 provider/model/账户绑定 | opencode agent.model + 权限派生；CCR 子代理标签（反例，不采纳）；opencodex 模型即子代理 | Agent Profile/spawn 参数声明绑定 → RouteContext → provider-control 选账户；默认继承父绑定、显式覆盖；预算经 budget-gate 分配 | P1 | S8 profile 铺垫 / S11（方案 F4-A+B） |
...
| G6 | 账户/端点配置导入 | cc-switch SQLite SSOT、CLIProxyAPI auth-dir、opencodex config、Claude/Codex 官方布局 | pawork-compat 增加账户与端点只读导入源，secret 直接转存 Keychain 不落中间文件 | P2 | S8（方案 F1 附属） |
...
以下功能因违反 Pawork 架构红线（[ADR-001](../../docs/adr/ADR-001-pure-rust-core.md) 纯 Rust、[ADR-019](../../docs/adr/ADR-019-no-tui.md) 无 TUI）**不纳入路线图**，仅记录排除理由以备回溯。

| 功能 | 来源 | 排除理由 |
| --- | --- | --- |
| 交互式全屏 TUI（themes/keybinds/sounds/Ctrl+G 编辑器） | OpenCode / Pi | ADR-019 明确不实现 TUI；Pawork 以 CLI 交互模式 + GPUI Desktop 为用户界面 |
| JS/TS 插件运行时（Bun/Node 扩展、hot-reload、`tool.execute.before/after` JS hooks） | OpenCode / Pi | 纯 Rust 红线（ADR-001）；S10 以 WASM 插件 + in-process hooks 替代 |
| npm 生态传输（`@opencode-ai/sdk`、npm 插件安装、Bun runtime） | OpenCode / Pi | 同上；插件分发走 git + WASM，不走 npm |

### 6.2 CLI 交互与命令体验

| ID | 功能 | 来源 | 说明 | 优先级 |
| --- | --- | --- | --- | --- |
| A1 | 自定义 slash 命令 / Prompt Templates | OpenCode `/commands`、Pi Prompt Templates | 用户定义 Markdown prompt 片段 + `$ARGUMENTS`/`{{var}}` 变量替换，作为 `/name` 命令调用；可绑定 agent/model/subtask。S0 CLI 只有内置命令，S8 resources 有 profiles 但无此轻量 prompt-snippet 机制 | P1 |
| A2 | `pawork init` AGENTS.md 生成器 | OpenCode `/init` | 扫描仓库结构 → 交互式问答 → 生成/更新 AGENTS.md。S8 只 *加载* AGENTS.md，不生成 | P1 |
| A3 | Turn 级 undo/redo | OpenCode `/undo` `/redo` | 回退上一轮用户消息 *及其关联文件改动*（经 Git）。区别于 S7 的 Run 级 checkpoint/rollback——粒度是「对话轮次」而非「整个 Run」或「单个 Tool Call」 | P2 |
...
| B4 | References（工作区外引用） | OpenCode | 将额外本地目录或克隆的 Git 仓库注册为 `@alias` 上下文源，模型可引用其文件。S8 `@file` 只在当前工作区 roots 内 | P2 |
...
| B7 | `pawork mcp-server`（作为 MCP Server） | Codex `codex mcp-server` | Pawork 自身作为 MCP Server 暴露 `pawork` / `pawork-reply` 工具，让其他 MCP Client（IDE、其他 Agent）驱动 Pawork 会话。S8 只做 MCP Client | P2 |

### 6.4 扩展生态

| ID | 功能 | 来源 | 说明 | 优先级 |
| --- | --- | --- | --- | --- |
| C1 | 能力包打包与 git 分发 | Pi Packages | 将 skills + prompt templates + hooks + themes 打包为单一 manifest，支持 git 仓库安装/分享。S10 plugin market 只管 WASM 插件；这是更轻量的「资源包」分发（不含 JS/TS 代码） | P2 |
...
| C3 | 连接器目录（Connector directory） | Codex plugins | 预置 MCP connector 目录（Gmail / Drive / Slack / Calendar / GitHub 等），一键安装 + OAuth 配置 UX。S8 可外接任意 MCP server，但没有预置目录和一键安装体验 | P2 |
| C4 | LSP 自动安装矩阵 + diagnostics 反馈 | OpenCode lsp | 内置常用语言服务器（rust-analyzer / gopls / ts-server / pyright…）的自动发现与安装；并将 LSP diagnostics 作为上下文反馈给模型。S10 LSP 是「LSP 查询作为工具」（定义/引用/诊断），不含自动安装矩阵 | P3 |

### 6.5 集成与分发

| ID | 功能 | 来源 | 说明 | 优先级 |
| --- | --- | --- | --- | --- |
| D1 | 第一方 IDE 扩展 | OpenCode、Codex | VS Code / Cursor / Windsurf / JetBrains 扩展，经 ACP 或 GUI Connection Protocol 连接 `pawork`（不嵌入 Core）；支持 open-file/selection context、in-place review。S9 有 ACP 协议但无第一方扩展产品 | P1 |
...
| D4 | Web UI 浏览器客户端 | OpenCode `opencode web` | 作为 GUI Connection Protocol 的 Web client（本地 web app、LAN bind、basic-auth），与 Desktop client 并列。S9 有 gui-server + GPUI client，无 Web client | P2 |
...
- **P1（7 项）**：自定义命令、AGENTS.md 生成器、webfetch/websearch、图片输入、IDE 扩展——这些是竞品的基础体验差异点，建议在 S2–S8 主干阶段择机并行追加（工具类在 S2/S4 工具集扩展时一并加入，CLI 体验类在 S0/S9 CLI 正式化时补齐）。
- **P2（14 项）**：核心功能补全，建议在对应阶段（S8 resources/MCP、S9 serve/clients、S10 extensions、S11 workflow）作为增强项纳入，或作为 S12 后的独立迭代。
...
**注意事项**：
 ...
 2. CLI 体验类（A1–A4）在 S0 CLI 最小实现或 S9 CLI 正式化阶段追加，写入集限定在 `pawork-cli`。
 3. D1（IDE 扩展）虽列为 P1，但实现路径是独立的 IDE 扩展项目经 ACP 连接，不影响 Core 包；可与 S9 并行。
