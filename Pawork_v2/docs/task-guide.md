...
```text
按 Pawork_v2/docs/task-guide.md 执行 Pawork_v2/plan/S<N>-*.md 的〈波次/任务名，如「波 B：pawork-net」〉。
...
- **阶段外任务**：`按 Pawork_v2/docs/task-guide.md 执行〈任务书路径 + 章节〉`（例：`docs/research/multi-account-quota-plan-merge.md §4`）。
...
- **与根 [../../AGENTS.md](../../AGENTS.md) 的关系**：根文件 §5 的 L0–L3 验证分级、Workspace Full Gate 升级条件与分级报告模板面向 V1 仓库；在 `Pawork_v2/` 内工作时按本节（及 §3.3）的开发期放宽执行，S12 Release Hardening 时恢复全量门禁。其余章节（架构红线、命名、提交与分支、安全与权限、子代理使用）继续全量适用。
...
- **跨阶段并行**：满足 [../ROADMAP.md](../ROADMAP.md) §2 依赖关系的前提下，S5/S6/S7 可并行、S9/S10 部分可并行；主干阶段（S0–S4）建议串行推进以保住「每步真实可测」的节奏。
