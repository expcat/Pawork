...
mod tests {
    ...
    #[test]
    fn non_unique_reports_conflict() {
        let (service, id, root, _ws_dir) = make_service();
        fs::write(root.join("a.txt"), "dup\ndup\n").unwrap();
        let err = edit(&service, &id, &single_edit("a.txt", "dup", "x")).unwrap_err();
    ...
    fn multi_segment_atomic() {
        ...
        edit(&service, &id, &input).expect("edit");
        assert_eq!(
            fs::read_to_string(root.join("a.txt")).unwrap(),
            "FOO\nBAR\nbaz\n"
        );
    ...
    #[test]
    fn multi_segment_partial_failure_rolls_back() {
        let (service, id, root, _ws_dir) = make_service();
        fs::write(root.join("a.txt"), "foo\nbar\n").unwrap();
        let input = json!({
            "path": "a.txt",
        ...
        let err = edit(&service, &id, &input).unwrap_err();
    ...
    #[test]
    fn fuzzy_edit_preserves_terminal_newline() {
        let (service, id, root, _ws_dir) = make_service();
