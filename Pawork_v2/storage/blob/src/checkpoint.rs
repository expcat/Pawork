...
//!
//! 基于 [`crate::ArtifactStore`]（内容寻址 Blob Store）实现写操作的
...
//! ## 关于实现细节
//!
//! `FileSnapshot.pre_blob` 经 [`BlobId`] 的 hex serde 派生，磁盘 JSON 仍是
//! 64 字符 hex 字符串（与 V1 手写绕道形状兼容）。`state` 的锁用
//! `std::sync::Mutex` 而非 `tokio::sync::Mutex`：`list_changes` 是同步访问器，
//! 本 crate 从不跨 `.await` 持锁（FS/Blob 操作均在锁外完成）。
...
use crate::{ArtifactStore, ArtifactStoreError, BlobId};
...
/// 单个文件在某次写操作前的快照。
///
/// `pre_blob` / `pre_hash` 均为 `None` 表示写前该文件不存在（新增文件）。
/// `pre_blob` 序列化为 64 字符 hex 字符串。
