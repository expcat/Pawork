...
| --- | --- | --- |
| `pawork-plugin`（extensions/plugin） | 激活：plugin-package + marketplace（feature `market`）；**三处 Ed25519+blake3 验签收敛为本包单一 `signing` 模块**；market HTTP 走 `pawork-net`；安装/撤销路径操作经 `pawork-policy`，删除走精确字面路径 | [archive/M6](archive/M6-extensions.md) pawork-plugin 节 |
| `pawork-wasm-host`（extensions/wasm-host） | 激活：wasm-plugin-host + hook-runtime（降级为包内 `lifecycle` 模块）；wasmtime 锁本包、默认构建路径不含；验签经 `pawork-plugin::signing` trait 注入 | [archive/M6](archive/M6-extensions.md) pawork-wasm-host 节 |
| `pawork-hooks`（extensions/hooks） | 激活：user-hooks 注入式执行器；与 wasm lifecycle 信任域分离（宿主进程内 vs 沙箱内），不同注册位点；hook 决策事件化入 envelope | [archive/M6](archive/M6-extensions.md) pawork-hooks 节 |
| `pawork-lsp`（extensions/lsp） | 激活：lsp-runtime（async LSP client）；resource 依赖注入指向 `pawork-resources` loader 抽象、sandbox 经 `pawork-exec` 注入；LSP 能力注册为工具 | [archive/M6](archive/M6-extensions.md) pawork-lsp 节 |
...
- [archive/M6-extensions.md](archive/M6-extensions.md)（本阶段主文档：四包迁移细则与接线位点）
