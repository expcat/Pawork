...
| V2 包 | 动作 | V1 来源 |
| --- | --- | --- |
| `pawork-protocol`（foundation/protocol） | **增强/收口**：六合一（core-api 拆六模块 + 在 S7 最小帧上补齐 golden + client-adapter-api + client-auth + headless-json + schema-typegen）；「协议版本 × 包版本」映射表 | [archive/M0](archive/M0-skeleton-foundation.md) pawork-protocol 节全文 |
...
| V2 包 | 动作 | V1 来源 |
| --- | --- | --- |
| `pawork-gui-server`（host/gui-server） | **增强**：在 S7 单客户端上合并 connection-manager + snapshot-service；多客户端并发、断线 Replay、慢客户端隔离 | [archive/M5](archive/M5-connectivity-clients.md) pawork-gui-server 节 |
| `pawork-client`（clients/gui-client） | **增强**：补齐多客户端/Resume API；Desktop 已在 S7 消费本包 | [archive/M5](archive/M5-connectivity-clients.md) pawork-client 节 |
| `pawork-channels`（host/channels） | 激活：`acp` feature 优先接线；`codex`/`claude`/`remote-control` 三 feature 随后按需（可移至本阶段后并行批），共享审计/gate 模式 | [archive/M5](archive/M5-connectivity-clients.md) pawork-channels 节 |
