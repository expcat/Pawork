...
- [ ] 设计文档 §8 已勾选，实现未超出 §3 Surface。
- [ ] 本机打开 Desktop：新建会话 → 真实模型流式多轮 → 侧栏可 resume。
- [ ] 只读或写入工具调用在 Timeline 可见；写入审批可点同意/拒绝，语义与 CLI 一致。
- [ ] Composer 取消当轮；关窗后 CLI 侧 Run 若已开始则继续（或按 ADR-026 不因 GUI 退出而杀）。
- [ ] 切换已配置 provider/model 后下一轮走新通道。
- [ ] 杀 Desktop 再开：同一 session 时间线连续，不丢已落盘事件。
- [ ] 中文 IME 与多行粘贴可用（开发机平台）。
