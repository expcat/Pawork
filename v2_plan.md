...
| 字段 | 值 |
| --- | --- |
| 当前阶段 | S8（[plan/S8-git-checkpoint.md](Pawork_v2/plan/S8-git-checkpoint.md)） |
| 阶段状态 | 🔵 进行中 |
...
| S8 波次进度 | 波 A（2026-08-17）：并行激活 `pawork-git`（`vcs/git`：git-service + diff-service；domain→exec 取消桥；补 untracked/submodule/CRLF/中文文件名/`--force` stage、parser golden + proptest）与 `pawork-blob-store`（`storage/blob`：artifact 默认面 + `protected` PWB1 hex golden + `checkpoint`；`FileSnapshot` 改 derive serde）。定向测试全绿：git 99；blob default 14 / `--all-features` 42。`cargo tree -p pawork-git` 无 `pawork-workspace`。archive M1/M3 正文仍缺失，细则回退 §4.1。未接线 engine/app/cli |
| **下一波次** | **S8 波 B（串行）**：engine/app/cli 接线 + 审批 diff 预览 + 冒烟 |
| 阻塞 | 当前波无阻塞。S6 的 ChatGPT/xAI 自然临期真实 refresh 人工验收继续挂账，S6 保持 🔵；它不阻塞 S8 |
