...
| 字段 | 值 |
| --- | --- |
| 当前阶段 | S11（[plan/S11-workflow-control.md](plan/S11-workflow-control.md)）波 A 已完成；下一波为 S11 波 B（不自动开启） |
...
| **下一波次** | **S11 波 B**（工作流三包：`pawork-workflow` / `pawork-memory` / `pawork-review`）。不自动开启。 |
