...
| 字段 | 值 |
| --- | --- |
| 当前阶段 | S10（[plan/S10-serve-clients.md](plan/S10-serve-clients.md)）10a 波 A–B、10b 与收口实现已完成；下一波为 S10 冒烟补齐 |
...
| **下一波次** | **S10 冒烟补齐**：真实 ACP 编辑器（Zed `agent_servers` → `pawork acp serve`）；两 gui-client 连真实 `gui serve` + Replay；`sessions fork` 两分支 resume；`--json` 新格式人工对照；darwin `service --apply`（launchd）。不自动开 S11 |
