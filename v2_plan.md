...
| 字段 | 值 |
| --- | --- |
| 当前阶段 | S9（[plan/S9-mcp-resources.md](plan/S9-mcp-resources.md)）波 B 已完成；下一波为 S9 波 C |
| 阶段状态 | S9 🔵；S8 🟢；S6 仍 🔵（OAuth 临期 refresh 挂账，不阻塞 S9） |
...
| S9 波次进度 | 波 A（2026-08-17）：并行激活 `pawork-mcp`（rmcp `=2.2.0` 锁进内部 `codec`，公开 `McpPeer` 无 SDK 类型；stdio fail-closed；MCP 工具与内置工具共用 `ToolRegistry`）、`pawork-resources`（AGENTS.md / Skills / profiles，hooks/lsp/templates/watch 不迁）、`pawork-config`（`Loader::with_session` / `with_run`，六层矩阵 + Profile 文件切换；未重写 merge）。定向测试 `pawork-mcp` 59 / `pawork-resources` 61 / `pawork-config` 44 全绿。未接 host/engine/cli（波 C）。波 B（2026-08-17）：并行激活 `pawork-compat`（五来源只读导入；MCP/Hook 本地薄类型，`cargo tree` 无 rmcp）、`pawork-session` `import::formats`（compat 四来源 + export v3 + pi；外部源 Secret 前缀拒绝；三路 Immediate 单事务）、`pawork-workspace` file-index（构建/查询/去抖 watch；`@file` 消费者留波 C）。定向测试 `pawork-compat` 16 / `pawork-session` 53 / `pawork-workspace` 12 全绿。未接 host/engine/cli |
| **下一波次** | **S9 波 C（串行）**：engine 注入点 + cli（`mcp` / `import` / `@` 引用）+ 冒烟 |
| 阻塞 | 无阻塞本波。S6 的 ChatGPT/xAI 自然临期真实 refresh 人工验收继续挂账，S6 保持 🔵；S9 不依赖该项挂账 |
