...
> 2026-09-20 门禁重构：L1 的追加包去重后与 Provider 合成一次 Cargo 调用，采用 `--tests` 覆盖 lib/bin 单测和集成目标；Desktop 仍用其 Spec 的图形构建命令。L2 的录制 `/usage` 字节与形状断言合入 `server_smoke.py`，移除第二次相同服务器回放。下方历次耗时与数量是历史证据；当前结果见 [测试重构计划](testing-refactor-plan.md)。
