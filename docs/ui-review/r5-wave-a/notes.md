...
> 状态：🟢 结构门禁通过。SSIM 记录值远低于 0.99，按 R3/R4 先例移交 R8 终局门禁（待用户在 R5 收口时拍板追认同一安排）。
...
- 主因：fixture 演示内容形状差（模型名、Context unavailable、placeholder 文案 vs 定稿「Ask a follow-up…」、定稿纸夹/reasoning/进度条为 capability 诚实缺省）。fixture 重塑已在 R3 拍板 c 移交 R8。
...
- 真 IME composition 取证走 R7/R8 U3（U1 已覆盖 is_composing 闸门逻辑）。
- placeholder 文案（「Message Pawork…」）与定稿演示文案（「Ask a follow-up…」）差异为已知文本差，R8 统一裁决。
