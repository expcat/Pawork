...
- **R3 退出标准未决项**：State A/B/C 分区 SSIM ≥0.99 依赖 fixture 演示数据重塑或遮罩策略调整（Wave A 已登记 ROADMAP §5，待用户拍板）；本波不触及。

## 5. 状态

Wave B 已实现、自动门禁与真窗口验证通过、证据归档（[u2-nav/](u2-nav/) Slice 4 的 22 相位 + [visual/](visual/) Computer Use 截图）；**等待人工视觉验收**。
