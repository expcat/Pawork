...
本轮产品扩展（2026-09-24，契约 [API 1.24](Plan/product-contracts-2026-09-24.md)）：视频仅远程 HTTP(S) 引用；Plan 复用现行评审状态机；持续目标由 Host 逐轮执行、显式预算与人工标准确认，重启不自动续跑；技能录制生成可编辑现行技能产物；绘图为手绘 PNG；插件列表不代表已实现插件市场。文件夹与 Chrome/Edge 页面作为用户显式选定的不可信参考附件。验收分层状态见 [产品验收](Plan/product-and-acceptance.md)。
