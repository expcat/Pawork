...
| 会话存储 | `session-store`：`session_events` DDL（`UNIQUE(session_id, sequence)`、`CHECK(sequence > 0)`）、append-only 双触发器、`AppendReceipt`、migration 序列（DB `CURRENT_SCHEMA_VERSION = 12`，与信封版本 1 相互独立；v11 为 R4 波 B 附加的 `command_ledger` 宿主幂等表，纯新增不动 v1–v10 DDL，不进 export；v12 为 R6 波 A 分支 lineage 原生化——`messages` 整表重建去 `DEFAULT 'main'`，回填即校验、无事件背书的孤儿行 fail-closed，检入真实库升级 golden；R6 波 B 的 branch snapshot 以 append-only event ledger 重建消息并按 lineage 可见 compaction 水位折叠，schema 不变） | S1 | 直接复用 V1 migration 序列全量建库，保证 V1 库文件可打开升级 |
...
| 会话分支 / `pawork session fork`（仅闭合 turn 后稳定事件：run 终态或 compaction 完成） | Pi session tree/clone（JSONL 树形，research §2.2）；OpenCode 子 session（research §2.1 task 工具）；DeepSeek Harness `ctx.sessions.fork`（从同一 `SessionEvent` 流切边界） |
