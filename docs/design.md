...
>
> **状态（2026-08-25）**：既有功能与结构阶段已归档；当前主线为新 R1–R8 Desktop UI 99% 还原与全功能模拟操作验收，剩余非 UI 工作顺延 R9–R10，R11 为设计稿与实际 UI 终局比对（见 [../ROADMAP.md](../ROADMAP.md)）。§2 的功能映射按 V2 阶段（S0–S13）组织，是「功能 ↔ 参照项目」的持续事实源，不代表现行包布局（现行布局见 [architecture.md](architecture.md) §2）。
