...
| 会话存储 | `session-store`：`session_events` DDL（`UNIQUE(session_id, sequence)`、`CHECK(sequence > 0)`）、append-only 双触发器、`AppendReceipt`、migration 序列（DB `CURRENT_SCHEMA_VERSION = 11`，`messages.branch_id` 为投影附加列，与信封版本 1 相互独立；v11 为 R4 波 B 附加的 `command_ledger` 宿主幂等表，纯新增不动 v1–v10 DDL，不进 export） | S1 | 直接复用 V1 migration 序列全量建库，保证 V1 库文件可打开升级 |
