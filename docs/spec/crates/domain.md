...
API 1.24：`VideoContent {url, media_type}` 只表示远程 HTTP(S) 引用，纯语法校验拒绝内嵌凭据、空主机、控制字符、file/data 与非视频 MIME；实际 URL/endpoint 校验由 Provider 补齐。`ModelCapabilities.video_input` 与 `CapabilityRequirements.video_input` 缺省 false，不能由图片能力推导。`GoalEvent::Created` 附加可选 `budget_tokens/max_runs`，`Resumed` 附加可选 `max_runs`，旧事件缺省保持可反序列化。新增 golden 为 `video_content.json` 与 `goal_budget.json`，用户确认的契约范围见 [协议 Spec](protocol.md) API 1.24 节。
