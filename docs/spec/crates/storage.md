...
| `catalog.rs` / `client_adapter.rs` tests | 目录排序/归档过滤；Registry insert/CAS/remove 乐观并发（借 `pawork-protocol` adapter 消费） |
