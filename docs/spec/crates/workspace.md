...
- `PaworkConfig` 字段语义：`default_provider` / `default_model`（缺省选择）、`web_search`（SEARCH-1：Provider 服务端搜索开关，缺省 false；消费方为 `pawork-app` RunService，未声明能力的模型 fail-closed）、`profile`（激活 profile 名）、`trust_workspaces`（安全开关，仅 Builtin/Global 可设）、`proxy_url`（全局出站代理，仅 Builtin/Global 可设；GUI Network 与手工编辑共用 workspace 外的 Global `config.toml`）、`providers: Vec<ProviderConfig>`（id / base_url / default / `use_proxy`：`None`/`true` 跟随全局代理，显式 `false` 绕过 / `disabled_models`：ADR-055 模型禁用 denylist）、`profiles: Vec<ProfileConfig>`（name + `ProfileOverrides`）、`extra`（未知键透传，供上层扩展段消费）。
