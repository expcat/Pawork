...
- `gui_host/tests.rs`（~45 条，本包最大测试集）：
  ...
  - SET-2 settings：`auth_set_api_key` 主路径（wiremock 验证 Bearer 命中→原子替换→事件/响应/status 全脱敏）与失败路径（401→`auth_verify` Error、旧凭证保留、Failed 事件无明文）；SET-4 A3 xAI 双认证主路径与替换移除旧 OAuth 各一条；
