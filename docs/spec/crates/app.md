...
| `src/loop_ctx.rs` | ~430 | `SessionLoopCtx` 实现 `pawork_engine::LoopContext`：审批请求转宿主、工具执行经 `ToolScheduler`（串行工具无论位于首位或中间都独占批次）、写前 checkpoint、压缩（fork recovery branch + snapshot）、message/request id 发号、事件 emit |
...
| `src/gui_server/session.rs` | ~1000 | 单连接握手与帧循环：协议版本检查、command 盖 client 戳、capability 门（未授予在宿主前拒绝）、Resume 三态调度（replay / SnapshotRequired / up-to-date）、Heartbeat→Pong、订阅确认、lagged→ReplayUnavailable 帧；ADR-045 `deliverable_to_negotiated` 按协商 minor 门控推送——`TerminalExited`（since 1.3）不推给协商 <1.3 的连接（老客户端 serde 遇未知变体会 decode 失败断流），该连接仍可从快照 `terminal_sessions` 的 `state` 获知终态；`host_error_to_protocol` 把宿主 `not_found` 映射为既有 `RequestNotFound` 码（其余维持 Internal），ADR-045 的幂等边界在 wire 上可观察；RV-10 起会因上游 I/O 阻塞的查询（远程账户额度 HTTP、ModelList 目录探测）经 `query_waits_on_upstream` 判入连接级 `JoinSet` 独立任务集合，不在串行主循环饿死其后的轻量配置读（响应按 request_id 关联、乱序安全，无 request_id 的 Snapshot 仍走串行主路径），命令与其它查询保持串行 |
...
| `src/gui_host/handlers/run_start.rs` | ~400 | RunStart：凭证或供应商代理变化使当前 adapter 失效（provider_stale），即使 provider/model 相同，下一轮也先重装配；目录兜底 provider_pending 同样允许连接后重装配。provider/model 切换校验（未知 fail-closed；ADR-055 D4 起切换后生效对被禁用时 `model_disabled` fail-closed，在 expand_at_refs 与 ActiveGuiRun 登记之前，不留幽灵 run）→ 按 session 归属 workspace `expand_at_refs` 展开 → 消费本机 `attachment_ids`（未完成 / 未知 id fail-closed，分类失败不消费暂存；仅附件时丢掉空草稿 Text part）→ 登记 `ActiveGuiRun`（快照 workspace id 与 roots）→ 解析 `RunStart.effort`（非法名 `invalid_effort` fail-closed，不启动 Run）→ spawn `chat_turn` 并传入本轮 `web_search`；模型切换发诊断事件；engine 未报终态即死时才补发合成 `RunChanged{Failed}` + `run.failed` |
