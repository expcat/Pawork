...
| `src/gui_host/mod.rs` | ~920 | `GuiHostAdapter`：实现 `GuiHost`；`QUERY_HANDLERS`/`COMMAND_HANDLERS` 静态分发表（与 protocol registry `gui.available` 双射，SET-2 起六个 Settings 入口，SET-6a 再增 `general_settings` / `set_proxy_url`，SET-6b 再增 `permissions_settings` / `set_approval_mode` / `workspace_trust`、SET-6c 再增 `mcp_test` / `mcp_server_remove`）、幂等 wrap（scope 隔离 + begin/record）、snapshot 组装（含重启后 pending approvals 重建，Workspaces 段输出 v14 注册表全集合）、timeline 分页（limit 默认 200、clamp 1..=500，游标跨未投影事件推进）；SET-2 `auth_flights` 按 provider_id 单飞守卫（auth_start / auth_set_api_key / auth_cancel 共用，Arc 身份防误删他人 flight） |
