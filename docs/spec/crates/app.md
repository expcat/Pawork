...
| `src/gui_host/handlers/settings.rs` | ~830 | SET-2/SET-6a/SET-6b/SET-6 终端页 Host Settings 门面（ADR-046/047/048/050）十三个 GUI 入口：`provider_auth_status`（通道 descriptor 从 `CHANNEL_REGISTRY` 派生 display_name / endpoint_label / auth_methods（SET-4 起八通道）；auth 四态 none/connecting/connected{method,masked}/error，connecting 由 auth_flights 推导、脱敏复用 auth_status，env 命中报 connected；SET-4 A3 起 auth_state 按 auth_methods 数据判定——声明 api_key 的通道先查 api key 凭证再查 OAuth meta，双认证通道显示 method 与实际存储凭证一致；catalog 三态 remote/fixed_fallback/unavailable 经 `models_overview` + join_all 并行探测，4s 超时，无持久缓存与后台轮询；SET-5 起响应顶层增 `default` 字段——取 `core.config()` 生效配置的 default_provider/default_model，两者皆 Some 输出 `{provider_id, model_id}` 对象，否则 null）、`auth_set_api_key`（trim 空校验→`verify_api_key` 内存验证 10s 超时→`store_default_api_key` 原子替换→SET-4 A3 替换语义：声明 oauth 的通道移除旧 OAuth 条目（删除失败 fail-closed）→`AuthChanged::Succeeded`；验证失败发 Failed 且旧凭证天然保留；xAI 双认证通道经同一 verify-then-replace 门）、`auth_start`（仅 oauth 流；映射 `oauth_begin`，Device Flow 回 verification_url/user_code/expires_at；后台任务 `oauth_finish` 与 CancellationToken select，终态经 AuthChanged 下发，取消路径由 cancel 侧发）、`auth_cancel`（按 flight 种类标记放行：仅 OAuth 等待 flight 可移除并取消、发 Cancelled；api_key 验证 flight 拒绝取消且登记保留——双认证通道亦不例外，D3；无进行中操作幂等 Accepted）、`auth_remove`（按 auth_methods 数据判定：OAuth 删 token / ApiKey 删文件均发 Removed，双认证通道依次清理两类条目；env 凭证命中时仍清理已存条目，仅无可删项时提示 unset 变量；完全无凭证 not_found）、`set_default_model`（provider 已知且 model 属当前可运行目录→`write_default_model_pair` 写 Global 层；SET-5 起写盘成功后经短写锁同步 core 内存 config 默认对（`AppCore::set_default_model_pair`，pub(crate)，无 reload/watch），同会话重查 `provider_auth_status` 即新值）、`general_settings`（SET-6a / ADR-047：`Data({"proxy_url": string|null})`，取 `core.config().proxy_url`）、`set_proxy_url`（校验=用 `http_from_config` 同路径预构目标 `reqwest::Client`（含 None）；`global_config_path` 缺失报 `config_unavailable`；`write_proxy_url` 原子写失败报 `config_write`；写锁内 `AppCore::set_proxy_url` 直接赋值含 None 并换入预构 client，禁止 `merge_with`；回执 `Data({"proxy_url": 写后值})`；非法 URL 统一 `invalid_proxy_url`，文案只带解析原因类别，不含原文 URL）、`terminal_settings`（SET-6 终端页 / ADR-050 D2：`Data({"shell": string|null, "columns": u16, "rows": u16})`——shell 取 Global 持久值（null=平台默认），columns/rows 为生效值、未设回落 `PtyWindowSize::default()` = 80×24 单一事实源）、`set_terminal_settings`（ADR-050 D3 全态写：fail-closed 校验 shell trim 非空且（含路径分隔符→路径存在，否则 PATH 可解析为文件）、columns/rows ∈ 2..=1000，非法统一 `invalid_terminal_settings` Error 保旧；`write_terminal_settings` Global 原子写→写锁内 `AppCore::set_terminal_settings` 直接赋值内存 `[terminal]`；回执 Data 为写入后的完整 TerminalSettings 形状；`shell: null` 显式清除回平台默认）、`permissions_settings`（SET-6b / ADR-048 D1：`Data({approval_mode: snake_case 串, workspace_trusted: bool, trust_workspaces_global: bool|null, workspace_id: string})`——前两个取会话内存态，global 取 `core.config().trust_workspaces` 只读展示，workspace_id 为 Host 权威 attached id（实现期修订增补））、`set_approval_mode`（SET-6b / ADR-048 D2：仅收五个 snake_case 规范值的 `approval_mode_from_wire`——不复用 CLI `parse_approval_mode` 的 kebab/on_failure 别名，未知值 `invalid_approval_mode` Error 保旧；写锁内 `AppCore::set_approval_mode` 只改内存态，不持久化、只影响之后启动的 run；回执 `Data({"approval_mode": 写后值})`）、`workspace_trust`（SET-6b / ADR-048 D3：校验 workspace_id 匹配当前 attached workspace（`core.workspace_id()`），不匹配 `unknown_workspace` Error 保旧；同一把写锁内校验+写入防竞态；`AppCore::set_workspace_trusted` 不写盘；回执 `Data({"workspace_trusted": 写后值})`）；明文 key 只在 handler 内存短暂停留，事件/响应仅携带 masked_credential |
| `src/gui_host/tests.rs` | ~3450 | `cfg(test)` 内嵌测试集：双射 pin、timeline 分页、`@` 展开三态、幂等（重启存活/失败计数/InFlight 收敛）、审批三态与重启后广播收口、合成终态闸门（fail 不重复/cancel 不谎报/早死兜底）、fork、provider 切换、bus lagged、ADR-045 `terminal_close`（kill→广播 Killed→注销快照节，重复 close 报 not_found）与自然退出广播 Exited+exit_code、SET-2 settings（auth_set_api_key 成功→原子替换→事件/响应/status 脱敏；401 验证失败→旧凭证保留且无明文外泄）、SET-4 A3 xAI 双认证（主路径经 verify-then-replace 连接 api_key；切换认证移除旧 OAuth 条目）、SET-6a `set_proxy_url` 设置→重查一致 / 清除→重查 null / 非法 URL fail-closed 保旧值（HOME Drop 守卫）、SET-6b 权限与审批（set_approval_mode→重查一致且未知值保旧；workspace_trust 匹配 id→内存切换生效；id 不匹配 fail-closed 保旧）、SET-6 终端页 ADR-050（set_terminal_settings 设置→清除 shell→同会话重查一致；非法 shell 路径 / 越界尺寸 fail-closed 保旧值与盘；terminal_create 应用配置 shell 与 size（/usr/bin/true 立即退出证 shell 生效））等 |
