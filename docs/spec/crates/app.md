...
| `src/gui_host/handlers/terminal.rs` | ~540 | TerminalCreate/Write/Resize/Close：经 `PtyService`；`terminal_create` 过 PolicyEngine（capability=Process；NeverAsk/ReadOnly 直拒，AskUser fail-closed 落 Deny）；`resolve_terminal_cwd` 按注册表严格解析目标 workspace（未登记 fail-closed）并保留 workspace 相对 cwd 记账（注册表值编码 owner+cwd，字段声明在 gui_host/mod.rs 不动；根目录 canonical 空串经 `terminal_cwd_label` 归一为 `"."`，防面板 cwd 空白）；`terminal_snapshots` 回报该相对 `cwd` 键（SnapshotSection.data 为不透明 JSON，非 golden 帧；记账缺失省略键，Desktop 显示 unknown）；输出经事件广播，需 terminal-streaming capability。ADR-045：`terminal_close` 经 `PtyService::kill`（幂等）终止后 `forget_terminal` 从注册表注销（快照节不再出现），未知/重复 id 报 `not_found`；forwarder 是终态事件唯一广播点——`PtyEvent::Exit` 按 PtyService 权威状态区分 `Killed`（kill 先置态）/`Exited` 并携带真实 `exit_code`/`signal`，转发链路 `Err(_)` 广播 `Failed`（不臆造退出码），close 路径自身不广播（与自发 Exit 天然去重） |
