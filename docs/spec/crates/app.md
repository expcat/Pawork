...
| `src/gui_host/handlers/settings/general.rs` | ~65 | `general_settings` / `set_proxy_url`：`GeneralSettingsData`。校验用 `http_from_config` 预构 client；非法 URL `invalid_proxy_url`（文案只带解析类别，不含原文 URL）；写锁内 `set_proxy_url` 禁止 `merge_with` |
