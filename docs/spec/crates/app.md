...
| `src/subagents.rs` | ~500 | 子代理运行时：内建工具 `spawn_agent`/`wait_agent`/`close_agent`（全局启用 + 当前模型 `allow_spawn` 且非 child 会话才暴露；子代理不可再派生）；父子工具权限取规则交集；`SubagentRun` 生命周期（父终态等待收子、失败 / 取消级联）；`subagent.spawned` Diagnostic + child projection 还原列表真实状态；取消按 run 归属校验；ADR-063 `resolve_effort`：规则 default_effort（须在 allowed_efforts 内）> Global [reasoning] 模型级默认 > None，随子请求注入 reasoning 并回报 SubagentInfo.effort |
...
- `expand_at_refs(session_id, text) -> Vec<ContentPart>`（async，ADR-044 起按 session 归属 workspace 的 file-index 路由）：`@token` 命中时正文作为**独立** part 追加（不拼进 user text），按索引保留的 `root_index` 选择正确根目录，读取前走 `policy::resolve_workspace_path`，拒绝索引后被换成的越界 symlink / 特殊文件；文本最多读 64 KiB + 1 字节并截断标记；VISION-2 起图片扩展名（png/jpg/jpeg/gif/webp，大小写不敏感）展开为头标记 Text part + `ImageSource::Base64` Image part（alt_text 记相对路径，原始字节 ≤8 MiB，元数据超限不读取正文，否则最多读上限 + 1 字节；超限行省略标记不静默丢弃）；无 `@` 时返回单 Text part。`complete_at(query, limit)` 补全候选（未索引 workspace 自动先扫描，二进制文件同样入索引可命中）。`workspace_root()`。
...
- `services/*`、`loop_ctx.rs`、`checkpoint.rs`、`control.rs`、`protected.rs`、`approval.rs`、`extensions.rs`：各自带定向单测（resume seal 语义、压缩 lineage、usage 哨兵、审批模式解析、`at_tokens` 词法、VISION-2 `@` 图片附件展开与超限省略、base64 与图片 MIME 表等）。
  ...
  - 2026-09-15 审查：`extension.rs` 覆盖多根目录图片、超大稀疏图片、索引后路径被替换为越界 symlink / 非普通文件；`gui_host/events.rs` 覆盖 hosted 搜索 live / 历史两臂一致、完成 / 失败后追加来源与重复回放去重。Host 用 `protocol::projection::project_server_tool_event` 广播服务端工具展示事件。
