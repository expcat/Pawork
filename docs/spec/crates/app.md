...
| `src/provider_assembly.rs` | ~1250 | provider 装配单点：`assemble_provider`/`assemble_registry`、通道→协议解析（KimiOAuth→ChatCompletions 装配 `KimiCodeProvider`；xAI SET-4 双认证按存储形态解析凭证——api key 优先、无则 OAuth 含刷新）、OAuth 刷新装配、`switch_model`/`switch_provider`（含 ModelSwitched 诊断事件；ADR-055 D4 起目标模型在 `disabled_models` denylist 时 `AppError::ModelDisabled` fail-closed）、`model_catalog`/`models_overview`/`provider_models` 目录聚合（builtin + config 覆盖 + 运行期探测，kimi-code 静态目录合并）、`is_credential_pending`、ADR-054 `generate_session_title`（命名 provider 与当前已装配相同且凭证就绪时复用 adapter，否则全量装配；无工具一次性补全，64 output tokens、20s 超时，输出取首个非空行限长 72）、ADR-052 `provider_proxy` 按 provider 解析生效代理（Global `proxy_url` 统一生效，仅当该 provider 显式 `use_proxy = false` 时绕过；六个装配点统一接入，不按 provider 名称特判） |
...
| `src/gui_host/auto_title.rs` | ~150 | ADR-054 D4 自动标题编排：GUI RunStart 成功终态后独立 spawn（不阻塞终态事件）；标题仍为占位名 `New session` 且已配置 `naming_provider`/`naming_model` 才经 `AppCore::generate_session_title` 做无工具一次性补全；ADR-055 D4 起命名模型被禁用时跳过命名、保留占位名；写回前二次复核占位名（防命名期间用户改名）才 `rename_session` 并广播 `SessionMetaChanged`；未配置/失败/超时静默保留占位名 |
...
| `src/gui_host/handlers/run_start.rs` | ~290 | RunStart：provider/model 切换校验（未知 fail-closed；ADR-055 D4 起切换后生效对被禁用时 `model_disabled` fail-closed，在 expand_at_refs 与 ActiveGuiRun 登记之前，不留幽灵 run）→ 按 session 归属 workspace `expand_at_refs` 展开 → 登记 `ActiveGuiRun`（快照 workspace id 与 roots）→ spawn `chat_turn`；模型切换发诊断事件；engine 未报终态即死时才补发合成 `RunChanged{Failed}` + `run.failed` |
