...
| 路径 | 行数量级 | 承载内容 |
| --- | --- | --- |
| `src/lib.rs` | ~1 250 | `GuiClient` 全部实现：`ClientConfig`、`SessionInfo`、`ResumeOutcome`、`ClientError` / `ClientErrorKind`、私有 `FrameWant` 帧路由、握手 / 往返 / 订阅屏障 / Snapshot / Resume / Ack / Heartbeat；对 Desktop 的 re-export（protocol 类型、`projection`、`TOKEN_SCHEME`、transport 四类型）；7 个内联测试 |
...
| `src/headless/version.rs` | ~40 | `SDK_VERSION`（crate 版本）与 `SDK_API_VERSION`（SDK 期望的协议版本 = 1.2）；2 个内联测试 |
...
| `tests/probe.rs` + `tests/probe/harness.rs` + `tests/probe/scenarios.rs` | ~1 110 | `--self-test` 13 场景（MemoryTransport 进程内装配）；harness 提供 AppCore / GuiServer / 握手 / CLI 侧命令辅助 |
| `tests/client_tests.rs` | ~590 | headless SDK 契约测试（MockTransport + `tests/fixtures/` 5 个 JSON/JSONL fixture），24 测试 |
| `tests/spawn_e2e.rs` | ~350 | 真实进程 e2e：spawn 工作区 `pawork` 二进制（无 `headless` 子命令时 SKIP，不作门禁），3 测试 |
...
- `command(AppCommand, CommandSource, ActorIdentity)` / `query(AppQuery, ...)`：自动装配信封（自增 `request_id`）；`command_envelope` / `query_envelope` 接受调用方完整信封（幂等重放场景）。返回 `AppResponseEnvelope`。
...
- protocol：`ActorIdentity`、`ApiVersion`、`AppCommand/Query/Event/Response` 及信封、`ClientAuthentication`、`TOKEN_SCHEME`、`CommandSource`、`EventStream`、`GlobalSequence`、`GuiCapability`、`RunState`、`Snapshot`、`TimelineItem`、`ResumeDisposition`、`projection` 模块。
...
- **版本协商**：`ClientConfig::supported_api_versions` 默认跟随 `pawork-protocol::SUPPORTED_API_VERSIONS`（1.0 / 1.1 / 1.2），服务端取 major 相同的最高共同 minor；不兼容必须显式拒绝（`IncompatibleVersion`），后续 ServerFrame 信封版本漂移由 `ClientError::Version` 捕获（ADR-036）。headless 侧 `SDK_API_VERSION` = 1.2 同理。
...
**`tests/probe.rs`（`--self-test` 13 场景，MemoryTransport 进程内装配）**
...
**`src/lib.rs` 内联（7）**：FrameWant 匹配矩阵（request-scoped vs 连接级 Error）、事件等待者与响应等待者互不饿死、`next_event` 显式暴露 `ReplayUnavailable` 等。
