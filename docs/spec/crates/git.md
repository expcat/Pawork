...
2026-09-20 测试重构：基础 hunk、无末尾换行及 context 两侧标记只保留 tests/parser_contract.rs 的 golden 契约，删除完全重复的内联版本。十万行 diff 仍检查解析完整性，移除依赖机器负载的 500ms 断言。 执行状态见 Git 历史（37fae8f3:docs/testing-refactor-plan.md）。
