...
| `tests/xai.rs` | `xai-oauth` | 模型能力选 Responses/Chat；Grok 订阅 `/models` 目录、实际模型 ID 与协议路由、认证头与模型路由头、VISION-2 默认图像回填与 Responses 模型 WebSearch 声明；Responses 带 OAuth Bearer 的全链路往返 |
