...
- **渲染面自动门禁尚未完整**：R1 已建立 U1 与真窗口 AX/screenshot 来源；R6 Wave B 已把 DiffView 长行真实横滚纳入 U2（offset `-720.0 / 2477.0`），不再只靠历史人工取证。R7 Wave A 的 State A hover/active/focus 九图已由用户签字，并依用户决定以原生 AX tree/action + 纯键盘 + U2 替代本波 VoiceOver；R7 Wave B 的 focus/menu/approval 等价路径已由两套主 U2 与一套审查边角 U2 收口。VoiceOver 未执行，屏幕朗读措辞 / 顺序未验证；1080×720、字号放大、千级列表、平台偏好与性能留 Wave C，三张定稿图分区 SSIM 与终局视觉签字留 R8。
