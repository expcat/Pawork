...
| `src/ui/accessibility/app.rs` | ~8807 | 工作台三栏语义树与 Press 白名单（含 Settings identifier 派发、终端标签 / 新建标签）；P0-2 的 `task-rail-grouping` 发布目标动作 name + 当前视图 value，Press 直接切换，不再发布 expanded/menu child；P0-3 空态发布 title / description / 单一 New task action，disabled 时不发布 Press；P1 的 tool group / Review 与 Activity 几何、P2 的 credential summary 隐藏和 Settings 写 gate 与 render 同源；UX-05 model 菜单使用实际布局 / 内部列表滚动框裁剪 AX，打开定位当前项，搜索与键盘高亮共用筛选结果；Settings 页树拆到 `settings_*.rs`；OPT-3 增 Manage models 弹层（触发器 / 单模型 Switch / 全开全关 / 头行常驻 Refresh / 空目录）与四默认角色触发器·菜单的 identifier 派发；ADR-056 增展开 chevron（Press、value Collapsed/Expanded）与展开区 Credentials 凭证行 / Usage 空态节点；UI-2 会话行动作与项目头计数 AX 几何跟 render 同源；GUI2-01 增首页 / 宽窄字号实测布局回归；34 个测试 |
...
| `src/ui/changes.rs` | ~1274 | Changes 面：Files / Summary、只读 DiffView、latest-session mismatch fail-closed 与诚实 scope；empty / unavailable / stale 使用分层占位；GUI3-08 加载态保留标题并加静态骨架。GUI2-05 / R-04：`parse_hunk_header` / `diff_hunk_line_numbers` 从 hunk 头本地递推双列行号（不改 wire，畸形头空行号）；整行 `diff.*` 浅底 token；32px 紧凑文件行 `changes_status_chip`（M / A / D 色标 + 路径 + `+A/−D`）。`has_reviewable_files_for` 要求 active session、Ready、非 stale 且文件非空，供 Run summary render/AX 同源门控。折叠态 Header ActivityPopover 为 320px 内容宽，GUI 1.20 增「子智能体」展示卡（标题 + 运行/完成计数；ADR-063 起行内单行 = 名称 + 强度 + 状态，运行中 / 等待中稳定置顶，列表 clamp 至 5 行并限高滚动；仅绑定当前活动会话，无会话 / 加载 / 空态如实展示，取消入口留在协议层）；8 个测试 |
...
- **Inspector（cmd-i 开合；GUI2-05 起按 `shell_layout::InspectorPlacement` 放置）**：偏好关为 Hidden；宽窗（默认字号 > 1279 且 ≥ 1288，150% ≥ 1320）为 Side 440px 右栏；窄窗显式打开为 Center——同一面板占中央 Workspace（TaskRail 保留），顶部 36px「Back to conversation」`inspector-back`，不渲染 Timeline / Composer 且 AX 不发布，但保留实体、草稿、跟随 / 滚动偏移与连接状态；`pending_approval` 时面板顶部「Return to the conversation to handle the approval」`inspector-approval-hint`（点击即返回，不默认批准）；跨窄窗切换 `InspectorMotion::snap` 不动画。面板头 48px = 统一工具 / 文件 / PTY 标签栏与打开工具菜单 Dropdown（`inspector-panel`，value 当前面板，菜单 `inspector-panel-menu` 列实际接通的 Changes / Terminal / Resources，`MenuKind::InspectorPanel`）+ 36×36 `inspector-collapse`，取代 UI-1 三页签；默认 Home 工具入口，Activity / Review 显式进入 Changes；Changes 内二级条 40px / 每项 96px / 12px 字；各页滚动状态独立保留。
...
| `projection/tests.rs` | 79 | 既有 snapshot/replay、Run/Timeline/TaskRail 与 Terminal 投影；未开始任务钉顶 / 复用、live 活动清钉顶、多标签 Close 回落与 create 失败保兄弟标签；Settings Data 走 protocol 类型 fail-closed（缺 nullable 键 / 类型错误）；SET-3～6d 解析与 stale/禁写回归仍在。 |
