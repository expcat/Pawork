...
| `src/controller/mod.rs` | ~2330 | `DesktopController`（connect / 事件泵 / 独立 15s 心跳任务）；`ControllerEvent`；Command·Query 构造与信封解包后 `serde_json::from_value` 到 protocol Data。`SetApprovalMode.mode` 仍为 String（`ApprovalModeWire::as_str()`）。18 个测试 |
| `src/controller/session.rs` | ~684 | workspace_add / session create（ADR-054：`Option<workspace_id>`，None 直建无归属会话）·rename·archive·fork / run start·cancel（ADR-063 起 RunStart 携带 effort）/ model_list / snapshot·timeline 分页；rename/archive 回执后重取 snapshot；UX-08 `archive_session(session_id, archived)` 复用反归档写口，`SessionArchiveFinished` 携带写后状态确认；open_session 失败发 SessionOpenFailed{session_id}；子代理对话栏分页 load_subagent_timeline（同一 SessionGet 链，回执走 SubagentTimelineLoaded/Failed 按子会话 id 归属，不污染主 Timeline） |
...
| `src/platform.rs` | ~240 | `Platform`（tokio multi_thread Runtime，`handle()` / `block_on()`）；`default_socket_path` / `socket_path_for_instance` / `token_path_for_instance` / `token_path_for_socket` 路径发现；deny-list 断言；4 个测试 |
...
| `src/ui/subagent_panel.rs` | ~495 | Inspector「子代理」对话栏：顶栏代理 chip（display_agents 顺序，点击切换并重查）+ Refresh；正文 = 元信息（标题 · 生效模型 · 状态）+ 主代理→子代理任务盒（子会话首条 user 消息）+ 过程（assistant 复用主 Timeline Markdown 渲染、思考限高滚动、工具行、RunState / Error）+ 子代理→主代理结果盒（subagent_list result）；未选 / 加载 / 失败分层如实，跟随滚动与回底控件同终端语义；AX 为 Group 状态摘要（行级交互走键盘焦点链，不发布无真实布局的行级节点） |
...
| `src/ui/changes.rs` | ~1274 | Changes 面：Files / Summary、只读 DiffView、latest-session mismatch fail-closed 与诚实 scope；empty / unavailable / stale 使用分层占位；GUI3-08 加载态保留标题并加静态骨架。GUI2-05 / R-04：`parse_hunk_header` / `diff_hunk_line_numbers` 从 hunk 头本地递推双列行号（不改 wire，畸形头空行号）；整行 `diff.*` 浅底 token；32px 紧凑文件行 `changes_status_chip`（M / A / D 色标 + 路径 + `+A/−D`）。`has_reviewable_files_for` 要求 active session、Ready、非 stale 且文件非空，供 Run summary render/AX 同源门控。折叠态 Header ActivityPopover 为 320px 内容宽，GUI 1.20 增「子智能体」展示卡（标题 + 运行/完成计数；ADR-063 起行内单行 = 名称 + 生效模型 + 强度 + 状态，运行中 / 等待中稳定置顶，列表 clamp 至 5 行并限高滚动；行可点击（进入 Inspector「子代理」对话栏）；仅绑定当前活动会话，无会话 / 加载 / 空态如实展示，取消入口留在协议层）；8 个测试 |
...
| `src/ui/u1_probe.rs` | ~409 | R1 Wave C U1 spike：真实 TextInput/Button/overflow 探针；R5 Wave B 增 SelectAll/Copy/Cut/Undo/Redo、IME commit 单次入栈（真实 EntityInputHandler 路径）与空输入不可发送、Wave B 键位（含 Shift-Enter）经 keystroke→keymap→action 真实链路覆盖；14 个测试 |
...
| `projection/tests.rs` | 81 | 既有 snapshot/replay、Run/Timeline/TaskRail 与 Terminal 投影；未开始任务钉顶 / 复用、live 活动清钉顶、多标签 Close 回落与 create 失败保兄弟标签；Settings Data 走 protocol 类型 fail-closed（缺 nullable 键 / 类型错误）；SET-3～6d 解析与 stale/禁写回归仍在；子代理对话栏状态（选代理复位 / 分页去重 / 异会话与异代理 fail-closed / live 过滤）主路径一条。 |
| `platform.rs` | 4 | socket / token 默认路径与 instance 命名；socket→token 推导；deny-list 恰为 `{pawork-client, pawork-terminal, pawork-browser}`；扫描器覆盖别名 / target 表（负例含 dev-dependencies 排除） |
| `ui/settings/mod.rs` | 5 | SET-6d：空 shell Save 映射为 null、尺寸合法才可保存；OPT-3 角色区与 Manage models 弹层渲染 gate；角色控件 identifier 构造/解析互逆。 |
| `ui/mod.rs` | 19 | 既有键位、tab_stop、TaskRail 导航、Composer 与 per-session 草稿接线；进入/Refresh/重连 `refresh_all_settings`；R6 Wave A Header Activity；R6 Wave B Inspector 键盘目标、terminal gate 与 workspace 草稿/回执归属；R7 Wave C 钉字号放大/缩小/重置键位登记；终态关闭 gate；GUI4 终端按键直通与自动 fit resize |
...
| `ui/input_area.rs` | 6 | Composer placeholder / 面板高度、伪二级分组、未连接过滤、模型单行标题与全禁用空态 |
| `ui/theme.rs` | 9 | UI-1 AA 文字 / 交互面对比度、TaskRail、Header/Timeline 与 Composer token；GUI3-08 近黑层次与有限动效常量；100%/125%/150% TextScale、P0 foundation spacing/radius/focus/menu token 与 Activity 320×144 基准 token 断言（缩放几何由 AX 测试覆盖） |
...
| `ui/changes.rs` | 8 | ActivityPopover 摘要、非空可审阅文件 gate、二级页签、epoch/path/session 三重拒旧、断线 stale、latest-session mismatch 与真实横滚内容模型 |
| `ui/inspector.rs` | 2 | 默认 Changes；终端输出控制序列解析；实际面板与 AX 在 accessibility 测试中核对移除旧控件、输出可聚焦与视口几何 |
...
| `ui/timeline_entry.rs` | 9 | R4 Wave A 纯逻辑：tool 已知状态本地化与未知状态原样保留/ 状态分类 / ToolRowView 构造；`display_time` epoch 串 → 相对词（now/Nm/Nh/Nd 边界）与非法串原样兜底 |
