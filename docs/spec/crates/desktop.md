...
| `src/controller.rs` | ~1723 | `DesktopController`（connect / 事件泵 / 空闲心跳 / 全部 Command·Query 构造与响应解析）；`ControllerEvent` 枚举（`MessageSent` 随回执携带发送文本，R4 Wave B）；`DiffFileSummary` / `DiffFileDetail` / `DiffHunkDetail` / `DiffLineDetail` / `GitDiffInfo` / `McpServerEntry` 视图模型；12 个测试 |
...
| `src/projection.rs` | ~3536 | `DesktopProjection` 渲染适配投影；`ConnectionState` / `ResumeState` / `ResumeApply` / `TerminalState` / `PendingApproval` / `ModelEntry` / `ActiveRun` / `SessionSummary` / `WorkspaceSummary` / `SessionLiveStatus`（R3 Wave A：Running / NeedsInput；R3 Wave B 增 Blocked——最近一条 `RunChanged` 为 failed / interrupted 终态 live 派生，优先级 NeedsInput > Running > Blocked；`session_live_status()` 跨会话读 active_runs 成员关系，apply_event 在 active-session 闸门前维护 live `RunChanged` 成员登记与后台 `ToolApprovalRequired`/`ToolCompleted`；`note_session_run` 在 `MessageSent` 乐观登记 Running；R4 Wave B 增 `note_user_echo` 本地乐观回显——active session 直接 push UserMessage 行、event_id `local-echo-{run_id}`、借用最大 wire sequence 不进 seen 不占号段，重放后由持久化行替换；`session_unread()` 独立 unread 通道——非 active session 的 Session-stream 活动事件标记、`select_session` 清除）/ TaskRail 分组类型；snapshot 段解析器；F-13 定稿 `run_status_label`（Task tokens | quota | tok/s | Run 竖线分隔）与 `show_reconnect` / `workspace_empty_hint_visible` 可见性谓词（render 与 AX 共源）；R4 Wave A 增 `TimelineRow` 渲染行组装（`timeline_rows()`：连续同 run ToolCall 合组、紧邻终态吸收为 RunSummary、非终态 RunState 归 RunPhase，终态判定唯一定义源 = `fork_boundary.is_some()` 无字符串匹配）与 `workspace_header_title()` / `workspace_header_status()` Header 谓词；37 个测试 |
| `src/ui/mod.rs` | ~3013 | `AppView` 宿主：R4 Wave A 增 Workspace Header（F-05：`workspace_header_element` 骨架常存 104 高含 36 安全条，任务标题 24px semibold truncate、branch 仅 `header_branch()` 诚实源——GitDiffInfo.branch 且无 session_mismatch 时显示、终态只画 live 可派生 Running / Needs input / Blocked 不画 Completed 绿点）；R6 Wave A 起最右 40×37 动作槽随 Inspector 状态互斥：展开态 `header-new-task`，折叠态 `inspector-toggle` Activity 触发器，Popover 右缘对齐触发器并向下展开，StatusBar 只保留 F-13 居中信息串；`on_review_changes`（Run 摘要卡主按钮：关菜单 → 展开 Inspector → 切 Changes tab → refresh，不可用给 status_hint）；连接生命周期、`ControllerEvent` 消费（含分页进行中 / 事件静默跟踪；R4 Wave B：`MessageSent` 回执在 active session 调 `note_user_echo` 乐观上屏并 `timeline_changed`，非 active 不 echo）、`MenuKind` 单开互斥与外点衔接标记、gpui 动作与键位表 `APP_VIEW_KEYBINDINGS`（R3 Wave B 增 cmd-alt-up / down / n 任务循环与 next-needs-attention）、tab_stop 表 `MAIN_PATH_TAB_STOP_IDS`（R5 Wave A：Send/Cancel 收成单槽 `composer-action`，idle 挂 send / running 挂 cancel）与 rail 前缀 `RAIL_TAB_STOP_IDS`（scope → grouping → 全局新建，reconnect -17 / 行级 -16 档、composer 1 档链尾；`RailStop` 焦点链按 design §3.6 组装项目头 / 定向新建 / task 行，折叠项目只保留头部）、Tab / Shift-Tab 映射 `focus_next` / `focus_prev` 走真实 tab_index 链、菜单键盘高亮、任务循环、`rail_scroll_to_active`、`pending_scope_focus`、空态引导文案 `WORKSPACE_EMPTY_HINT`、三栏整体装配与 AX 同步；1s tick（run 时钟重绘 + barrier settle 发射）；15 个测试 |
...
| `src/ui/theme.rs` | ~549 | 深色单主题 token：六组 29 色 + 字阶 `font`（XS=11 / SM=12 / BASE=13 / MONO="Menlo"，R3 Wave A 增 TITLE=22 / BODY=18 / BODY_SM=17）+ `metrics` 尺寸常量（含 TaskRail、Header/Timeline、Composer；R6 Wave A 增 Inspector 顶层 100×58、Changes 二级 96×56、2px 选中下划线、ActivityPopover 320×320 合同）；静态 `dark()` 访问器；`impl Global` 仅为未来主题挂载点；8 个定向测试 |
...
| `src/ui/inspector.rs` | ~281 | Inspector 面板：顶层 Changes / Terminal / Resources 三页签（`InspectorTab`，R6 Wave A 起默认 Changes）；顶层固定 100×58、18px 字与 2px 选中下划线；Terminal 页（cwd / size 行、FollowScroll 输出区 + 回底、终端输入 + Start/Size）；无输出占位文案 `TERMINAL_EMPTY_OUTPUT`（视觉与 AX 共源，R2 Wave B）；`ensure_terminal` 懒创建；1 个测试 |
| `src/ui/changes.rs` | ~727 | Changes 面：Files / Summary 二级页签固定 96×56、17px 字与 2px 选中下划线；`ChangesPanelState`（双 epoch 防过期）、文件清单、DiffView（hunk 着色 + 横滚）、session_mismatch banner；折叠态 Header ActivityPopover 标题/Changes 权威摘要，不伪造 Agent；6 个测试 |
...
AX 焦点口径（R7 Wave A）：浮层菜单打开时，触发器 AX 节点让出 focused（发布谓词为 `open_menu.is_none() && trigger_focus.is_focused(window)`），高亮菜单项成为树内唯一 focused 节点（macOS 菜单惯例；VoiceOver 焦点随高亮移动）；WorkspaceConfirm 浮层行同样接入 `focused(ix == highlight)` 高亮体系。Timeline 行级动作（Run 摘要卡 Review changes、条目「···」菜单）行级 `on_activate` 与 click 同 handler 同 enable gate，键盘路径按 event_id 复核防虚拟化越权。
...
132 个测试全部内嵌于 bin target（`#[cfg(test)]` 模块；无 crate `tests/` 目录），按文件分布：
...
| `ui/mod.rs` | 12 | 既有键位、tab_stop、TaskRail 导航、Composer 与 per-session 草稿接线；R6 Wave A 增 Inspector 展开/折叠时 Header Activity 触发器与 Popover 可见性矩阵 |
...
- `--features gpui/runtime_shaders`：gpui 默认构建在编译期调用 Metal 着色器编译器；开发机仅有 Xcode CLT 时缺 Metal Toolchain 会构建失败，runtime_shaders 把着色器编译推迟到运行时使本机可闭环（R8 起的标准口径，历波收口 28/28 → 43/43 → 53/53 → 62/62 → 74/74 → 78/78 → 84/84 → 88/88 → 93/93 → 94/94 → 105/105 → 107/107 → 119/119 → 129/129 → 131/131 → 132/132 绿）。
...
- **ActivityPopover 迁移已实现、真窗口 connected 证据待收口**：R6 Wave A 已把触发器从 StatusBar 迁至折叠态 Workspace Header 右上并向下展开约 320px，render/AX 共用几何；定向测试覆盖可见性矩阵与 AX 锚点公式。手工前台截图只证明 1440 内容区 rail 288 / workspace 712 / Inspector 440 与两级 strip，窗口当时未进入 Connected 投影。当前自动 fixture 启动路径在本机 AX 侧递归返回 `AXApplication`，两次超时；因此 F-12 仍保持“已实现、等待 State B connected 真窗口取证”，不得追记为视觉通过。证据见 [r6-wave-a](../../ui-review/r6-wave-a/notes.md)。
