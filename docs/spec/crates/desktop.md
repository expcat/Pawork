...
| 路径 | 行数 | 承载内容 |
| --- | --- | --- |
| `src/controller/attachments.rs` | ~150 | 显式选择本机文件后的后台有界读取；附件字节 Debug 脱敏，64 KiB 分块上传，API 1.22 gate；仅 Host 接收后清草稿 |
| `src/ui/attachments.rs` | ~230 | 系统选择器、逐任务附件 / 搜索三态、能力提示、移除与实测 AX，首页也可仅附件发送 |
...
| `src/ui/subagent_panel.rs` | ~700 | Inspector「子代理」对话栏：顶栏代理 chip（display_agents 顺序，名称前 Ø6 状态点——running 蓝 / waiting 琥珀 / completed 绿 / failed 红；点击切换并重查，切换后选中 chip scroll_to_item 滚入横滑视口）+ Refresh；正文 = 元信息（标题 · 生效模型 · 状态）+ 主代理→子代理任务盒（仅子会话首条 user 消息；后续 user 回合用普通用户样式，不冒充任务盒）+ 过程（assistant 复用主 Timeline Markdown 渲染、思考全文随外层 transcript 滚动——不做内嵌滚动避免滚轮被内层吃掉、工具行、RunState / Error）+ 子代理→主代理结果盒（subagent_list result，同样不做内嵌滚动）；未选 / 加载（含已选代理但列表未返回时显示 loading）/ 失败分层如实，跟随滚动与回底控件同终端语义（回底仅真溢出时浮出）；换代理 / 切会话 / 断线对话栏贴底复位，content_arriving 仅对被选子会话事件触发；AX 为 Group 状态摘要并按实测布局发布 chip / Refresh / 回底 Button（chip 坐标按横滑视口裁剪，Press 走与 render 同源的 select / refresh / jump handler） |
...
| `src/ui/accessibility/settings_subagents.rs` | ~380 | 子代理页 AX：开关 / 输入 / Save / 模型规则控件与 render 同 gate 同 identifier；ADR-063 目录同源过滤启用模型，强度默认 / 范围 chip 节点与 render identifier 同源 |
...
| `src/ui/input_area.rs` | ~1232 | UI-4 Composer：880px 居中列，两侧至少 28px、顶部 16px、底部 24px；输入卡片 16px 内边距、16px 圆角、轻阴影、聚焦 accent 描边、12px 输入 / 动作间距、110–220px 高度预算。模型 Raised chip 与 ADR-063 推理强度 chip（相邻排布，标签 = 显式强度或「自动」，菜单含自动 + 当前模型可选范围；选模型自动带入默认）与 Send/Cancel 同为 36px 命中区，模型触发器随选中名称收缩（上限 220px，长名截断），菜单向上展开：已连接且有启用模型的供应商组头与模型同行展开（未连接 / 0 启用整组隐藏）。卡片外显示只读项目 chip 与 ContextMeter，UX-03 将无项目文件工具提示与项目新建入口并入元信息行，窄窗随宽度换行，status_hint 与字号反馈改落 StatusBar 右栏；render / AX 共用实测布局与说明。per-session 草稿、IME、发送 / 取消与空模型 gate 沿用；2026-09-21 增本机附件和搜索开关，附件行最多 96px 并可滚动，卡片按附件增加高度；没有队列能力。5 个定向测试；空 display_name 回退 id 并入真实分组菜单用例。实际 GPUI 布局与 AX 操作区对照位于 accessibility/app.rs |
...
| `src/ui/settings/subagents.rs` | ~760 | GUI 1.20「子代理」页：全局开关 + 并发上限（1..=16）+ 按模型规则（可发起 / 可充当 + 7 项功能权限 chip + 重置默认）；ADR-063 目录只列启用模型，能力徽标（图像 / 搜索）+ 默认强度 cycle + 可选范围 chips；全部即时全态写、回执收敛不乐观更新，写在途禁用防重复提交 |
...
浏览器复用 Inspector 键盘 / 裁剪 / 关闭与依赖边界测试；新增 `browser_panel_offline_navigation_boundary_and_lifecycle` 覆盖离线 AX 入口、宽窄窗地址栏、非法 URL 在创建原生视图前拒绝、Enter 不发送草稿、任务隔离、隐藏保留与关闭清空。导航真实行为以当前构建真窗口 + 本地 HTTP 请求日志验证，状态见 [路线图](../../ROADMAP.md#右侧浏览器首版2026-09-16)。
...
GUI2-01（2026-09-10）：壳层、侧栏与首页已实现；定向测试 227/227 与候选构建通过，本项真窗口路径完成、用户验收未完成；真实 Provider 请求 HTTP 401，验证结果见 [活动路线图](../../ROADMAP.md#gui2-01-工作台壳首页与侧栏)。新增 `welcome_and_rail_follow_actual_layout` 定向回归覆盖首页状态、实测 AX 和两种尺寸 × 三档字号；不改变协议、创建写口或持久化。
