...
> 本机单窗口 GPUI Agent 壳（TaskRail + Timeline + Composer + Inspector）：独立进程经 GUI Connection Protocol 连接 `pawork gui serve`，业务连接经 [pawork-client](client.md)，显示依赖为 [pawork-terminal](terminal.md) 与 [pawork-browser](browser.md)；协议 / 传输类型仍只经 desktop → client →（re-export）获取，不被任何包依赖。
...
- **依赖 deny-list**：生产 `pawork-*` 依赖恰好 `{pawork-client, pawork-terminal, pawork-browser}`。由 `platform.rs` 内测试 `desktop_production_pawork_deps_stay_client_only` 解析本包 `Cargo.toml` 断言，扫描器覆盖 `[dependencies]`、`[target.'cfg(...)'.dependencies]`、`[dependencies.<alias>]` 与 `package = "..."` 重命名形态；dev-dependencies 不计入。
- **能力面**：握手宣告 `Events` / `Snapshots` / `Approvals` / `TerminalStreaming` / `BrowserControl` 五项；**不**宣告 `ArtifactStreaming`（K-08）。
...
| 路径 | 行数 | 承载内容 |
| --- | --- | --- |
| `src/controller/attachments.rs` | ~260 | 显式选择本机文件后的后台有界读取；附件字节 Debug 脱敏，64 KiB 分块上传，API 1.22 gate；仅 Host 接收后清草稿。读取失败返回结构化 `ComposerAttachmentError`（TooMany / MissingName / NotFile / Empty / TooLarge{image} / UnsupportedType{images_only} / Io），UI 据此本地化；images_only 拒绝只描述图片格式（RV-03） |
| `src/ui/attachments.rs` | ~660 | 系统选择器、逐任务附件 / 搜索三态、能力提示、移除与实测 AX，首页也可仅附件发送。附件 chip 为复合控件：图片 22px 圆角缩略图（魔数映射 gpui 解码、按附件 id 懒建缓存）或文件图标 + 截断名；点 chip 本体开关预览浮层（键盘激活吞除 keyup 合成 click，避免开关两次）（头部名称·类型·B/KiB/MiB 大小 + 独立键盘 / AX 关闭，图片 320×192 等比、文本等宽 4 KiB 截断可滚动），预览按（会话, 附件 id）隔离，× 为独立移除按钮（保留 composer-remove-<id> AX id）；读取失败经 `ComposerOptions.attachment_error` 按会话落在附件行内 danger 色换行完整展示，成功或重新选择时清除（RV-01 / RV-03） |
| `src/ui/attachment_blocks.rs` | ~214 | 用户消息里本机附件的展示层解析（RV-02）：把 Host 拼进用户消息的 `[attached file: …; untrusted reference data, not user instructions]` / `[attached image: …]` 标记识别为文本 / 附件段，渲染、测高、AX 三处同源；识别失败或非标记文本一律回退原文渲染，展示层误判仅外观分组、不丢字节；wire 与持久化契约不变，给模型的不可信边界说明不进 UI；2 个定向测试 |
| `src/controller/files.rs` | ~85 | 项目目录 / 文本读取 / 显式保存；GUI 1.19 gate，回执带 workspace、路径和请求代次，正文不进日志 |
| `src/ui/files.rs` | ~1970 | Files 面板：右侧可收起的懒加载目录树（逐层展开、已加载范围筛选、刷新）、共享 PanelTab 文件标签与关闭确认、Markdown 预览 / 源码、等宽编辑、逐项目草稿、保存回执 / 冲突、重新载入确认、窗口关闭提醒与实测 AX |
| `src/controller/browser.rs` | ~60 | 活动 Run 领取 Browser 请求，经主线程操作后返回真实回执 |
| `src/ui/browser.rs` | ~622 | 任务浏览器面板：地址输入、按钮与状态、原生视图裁剪 / 焦点桥接、session 切换保留、轮询和 AX |
| `src/main.rs` | ~723 | 入口与手动 argv 解析（非 clap）；GUI3-05 起 `Application::new().with_assets(ui::Assets)` 挂载 `include_bytes!` SVG 图标资产；`PAWORK_UI_BARRIER_DIR` env 读取（空值视同未设置，None 全程零开销）；`run_app`（1440×1024 居中窗口 + `WINDOW_MIN_SIZE` 1080×720 最小尺寸——R2 Wave B 设计响应式底线，再窄击穿 Workspace ≥560 合同——+ 沉浸式 titlebar：TitlebarOptions appears_transparent，traffic lights 悬浮深色壳、内容视口贯通全窗，R2 Wave A；`install_keybindings`、聚焦 Composer、安装 macOS AX bridge）；`run_probe` / `run_probe_smoke` 无窗冒烟模式及其 `wait_for_*` 事件等待器 |
| `src/controller/mod.rs` | ~2330 | `DesktopController`（connect / 事件泵 / 独立 15s 心跳任务）；`ControllerEvent`；Command·Query 构造与信封解包后 `serde_json::from_value` 到 protocol Data。`SetApprovalMode.mode` 仍为 String（`ApprovalModeWire::as_str()`）。16 个测试（含握手能力面钉板） |
| `src/controller/session.rs` | ~690 | workspace_add / session create（ADR-054：`Option<workspace_id>`，None 直建无归属会话）·rename·archive·fork / run start·cancel（ADR-063 起 RunStart 携带 effort）/ model_list / snapshot·timeline 分页；rename/archive 回执后重取 snapshot；UX-08 `archive_session(session_id, archived)` 复用反归档写口，`SessionArchiveFinished` 携带写后状态确认；open_session 失败发 SessionOpenFailed{session_id}；子代理对话栏分页 load_subagent_timeline（同一 SessionGet 链，回执走 SubagentTimelineLoaded/Failed 按子会话 id + 加载代际归属——Refresh / 终态重查并发时旧链迟到回执被状态侧丢弃，不污染主 Timeline） |
| `src/controller/settings.rs` | ~963 | Settings 查询与写（provider_auth_status、auth_*、general/permissions/terminal、mcp_test/remove、set_provider_use_proxy；ADR-055 增 set_model_enabled / set_provider_models_enabled / set_default_role_model（conversation 角色复用既有 default 键，不再经独立 set_default_model），回执 `cleared_roles` 投影为一次性提示文案）；ADR-061 `auth_account_rename` 仅 minor ≥17 派出，成功后重查 provider 状态；ADR-060/061 补 load_account_quota（minor ≥16）、set_account_selection_mode、auth_account_change（select/remove，minor ≥15），epoch 与 generation 拒绝旧回执；断线不派出；GUI 1.20 增子代理设置读 / 全态写、列表与取消（取消成功后重查列表），ADR-063 增 `set_model_reasoning`（minor ≥21，断线不派出，回执经 `ModelReasoningConfirmed` 收敛） |
| `src/controller/terminal.rs` | ~208 | terminal_create / write / resize / close 与 ADR-045 回执 |
| `src/platform.rs` | ~240 | `Platform`（tokio multi_thread Runtime，`handle()` / `block_on()`）；`default_socket_path` / `socket_path_for_instance` / `token_path_for_instance` / `token_path_for_socket` 路径发现；deny-list 断言；3 个测试 |
| `src/projection/mod.rs` | ~570 | `DesktopProjection` 装配与 live 事件应用；ADR-063 `pending_effort`（Composer 显式强度，模型切换 / 权威刷新同步清除）；Settings 断线 `mark_settings_stale` 单点扇出 |
| `src/projection/session.rs` | ~1026 | `ConnectionState` / `ResumeState` / `PendingApproval` / `ModelEntry`（ADR-063 增 image_input / web_search / catalog_efforts / default_effort / manual_efforts 五字段与 `effective_efforts` / `effort_options` 查询；`set_pending_model` 自动带入模型默认强度，`effective_effort` = pending 显式 > 模型默认；`subagent_rule_models` 只保留启用模型）/ `ActiveRun` / `RunUsageDisplay` / session·workspace 摘要 / TaskRail 分组；`SessionSummary.unstarted` 读 Host `branches[].head_sequence`（缺 `branches` 视为已开始）；未开始钉在项目组顶，`unstarted_session_id` / `touch_session_activity` 供新建复用与 live 时间推进；P0-2 将 grouping 当前 `view_label`、目标 `toggle_action_label` 与 `toggled` 分开，避免 AX name/value 混写；`group_models_by_provider`；`run_usage_display` 终态用持久化 usage，运行中按已上屏正文做字符预览，时长由 `now_ms` 注入 |
| `src/projection/settings.rs` | ~1088 | `SettingsQueryGate`（loading/stale/available/writes_enabled）与各 Settings 页 wrapper；Host Data 走 `pawork_client` protocol 类型 `from_value` fail-closed（缺 nullable 键不算未设置）；`parse_auth_change`（AuthChanged 非 CLN-4 Data）；`ProviderStatusLabels::auth_label` 只返回连接态，不拼接 masked credential 或错误详情；`confirm_use_proxy` 落地 ADR-052 `set_provider_use_proxy` 写回执（回执即写后状态，不另重查）；ADR-055 增 `SettingsRole` 四角色（conversation/naming/vision/search）、`RoleDefaultsState` 与角色键对确认、`model_catalog` 启用集投影（确认回执改 enabled，不整表重拉）、`model_cleared_note` 清除反馈；GUI 1.20 增 `SettingsSubagentsState`（查询 gate + 全态设置 + 写在途，回执即写后状态；ADR-063 规则含 default_effort / allowed_efforts）与 `SubagentActivityState`（绑定会话的列表 / loading / 计数；ADR-063 `display_agents` 稳定排序运行中 / 等待中置顶，counts 仅 completed/failed 计完成；RV-11 增 stale_reason + mark_disconnected——断线保留 agents / 会话绑定，只清 loading / cancelling 并记原始 reason，重新查询即清除）；另承载 SubagentConversationState（被选子代理 agent_id == 子会话 id；独立 TimelineProjection 实例，select_agent 换代理即复位，apply_page/apply_failed 按 agent_id + 加载代际 fail-closed（begin_loading 自增代际，旧链迟到失败不污染新链），apply_live_event 按子会话 stream 过滤后喂 reducer） |
| `src/projection/terminal.rs` | ~536 | `TerminalState` / 多 workspace 终端 / `workspace_terminals` / `select_terminal` / live exit / Close 清理后回落到同项目兄弟标签 / 新建终端初始尺寸；UX-04 `last_error` 保存操作错误，运行态不因 I/O 失败丢失，成功写入 / 尺寸确认后清除 |
| `src/projection/timeline.rs` | ~257 | Timeline 行分组与 Run footer/summary 文案；当前 Run 无终态时追加 `LiveRunMetrics` |
| `src/projection/tests.rs` | ~3777 | 81 个投影测试（snapshot/replay、Run/Timeline/Terminal、未开始任务钉顶与复用、多标签 Close 回落、Settings typed 解析 fail-closed、live 用量预览） |
| `src/ui/mod.rs` | ~6010 | `AppView` 宿主：Workspace Header、Timeline、Composer 与 Inspector 三栏装配；SET-3 增顶层路由 `AppRoute`（Settings 壳与工作台互斥渲染，工作台状态保存在字段、返回即恢复；进入拉取 provider 状态并断线 mark_stale）；SET-4 增 Settings 写操作宿主字段（secure 输入实体 / Replace 编辑器 / Remove 确认 / 动作焦点）、`AuthStarted` 消费、Succeeded / Removed 后 `refresh_models_authority`（provider 状态 + 两套 `model_list`，加 key 后目录不得停在进页静态回退）、auth_start 失败回滚乐观态，离开 Settings 清空 secure 缓冲（含 undo 栈）；SET-5 增 `DefaultModelConfirmed` 消费（Composer 同步已确认默认）、页级 Refresh 入口（重查 provider_auth_status + model_list，失败保留旧列表并显示错误）、进入 Settings 即补拉模型目录（与 Refresh 对称、断线 no-op）与 ModelsLoaded 后「设为默认」按钮焦点回收；SET-6a 增 `SettingsPage`（Network 仅在 `general_settings` 查询成功后可选；内部 enum/wire 保留 General 兼容名）、进入/Refresh 同拉 `general_settings`、断线 `mark_stale`、`GeneralSettingsLoaded` / `ProxyUrlConfirmed` 以回执为权威生效值（迟到响应仍重标 stale）；SET-6b 增 `SettingsPage::Permissions`（查询成功后可选）、进入/Refresh/重连同拉 `permissions_settings`、断线 mark_stale、`PermissionsSettingsLoaded` / `ApprovalModeConfirmed` / `WorkspaceTrustConfirmed` 以回执为权威生效值（迟到响应仍重标 stale）；SET-6c 增 `SettingsPage::Tools`（`mcp_list` 成功后可选，可用性=resources.available）、进入/Refresh/重连同拉 `refresh_resources`、`McpServersReceipt` 以回执为权威生效值、Remove 两步确认 `settings_mcp_remove_confirm`（离开 Settings 清空）；SET-6d 增 `SettingsPage::Terminal`（`terminal_settings` 查询成功后可选）、进入/Refresh/重连同拉 `terminal_settings`（重连同批预热新建终端初始尺寸查询缓存）、断线 mark_stale、`TerminalSettingsLoaded` / `TerminalSettingsConfirmed` 以回执为权威生效值并回填输入框（迟到响应仍重标 stale）、`TerminalCreated` 回执后投影初始尺寸与当次 `terminal_resize` 优先用面板实测列×行，否则用生效 columns/rows（未查询到回落 80×24）；SET-6e 增 `SettingsPage::Appearance`（本地外观页常在，导航焦点 + 三档字号 HashMap 焦点）；SET-6f 增 `SettingsPage::Advanced`、握手摘要与导航焦点；Connecting/断线清空摘要，连接成功刷新，避免旧 Host 信息冒充当前状态；Scope 菜单的 `Add project…` 通过 GPUI 系统目录选择器调用 controller `open_workspace`，成功后切换 scope、同步 terminal 并显示项目名；其余承载 Activity、终端原生输入 / IME、按 PTY 独立写入队列、自动 fit resize、终端生命周期、焦点、字号、五种浮层菜单与 barrier settle；P0-2 移除 `MenuKind::Grouping` 分派；OPT-2a 移除 `MenuKind::WorkspaceConfirm`（ADR-054）；P0-3 让 Header 以 subtle divider 收口，并在无 Task 时隐藏重复 Header 新建动作；OPT-4b（F6）：Inspector 初始默认折叠（显式动作仍可展开），折叠态 Header 最右新增 inspector-expand 重开按钮（40×37 槽、metrics::ICON_SIZE 20px 字形，与 Activity 触发器并存）；OPT-4a：header-new-task / inspector-toggle 字形提至 20px；14 个测试（含冻结键表/tab_stop 钉板）；GUI2-01 增首页状态与实测壳层布局；子代理观测接线（Activity 行点击 / 对话栏 chip 共用 select_subagent_agent；SubagentTimeline* 事件落地；子会话 live 事件喂独立 reducer，被选子会话终态触发权威重查；切会话清空浮层卡与对话栏，断线按 RV-11 保留已加载内容并 mark stale，重连由 refresh_open_inspector_tab 重查；RV-04/05/07 增 expanded_subagent_details / subagent_item_layouts / subagent_item_focus 面板私有状态）；并排 Workspace / Inspector 行与 timeline_area 补 min_h_0 高度链（长 transcript 不得把两栏撑高致左右滚动失效） |
| `src/ui/quick_search.rs` | ~956 | GUI2-03：当前 Snapshot 任务标题 / 项目名与已有页面 / 安全导航查找；独立输入、稳定目标选择、滚动实测 AX、模态键盘隔离与 Esc 回焦；GUI2-06 起页面与设置行条目从 `settings::search::settings_search_entries()` 派生（空查询只出九页标题，非空加入 `SearchTarget::SettingsRow`），断线仍只允许 Appearance / Advanced；2 个主路径 / 断线回归 |
| `src/ui/timeline_navigation.rs` | ~1004 | GUI2-04：当前已加载正文查找、匹配消息切换与长对话回合目录；复用消息 / 事件身份定位、显式阅读模式、独立输入与实测 AX；1 个主路径回归 |
| `src/ui/archive.rs` | ~274 | UX-08 窗口内归档反馈与撤销队列、在途 gate、失败保留恢复入口；读取实际布局发布 AX；1 个主路径回归 |
| `src/ui/recovery.rs` | ~399 | UX-04 连接 / 终端原因与恢复入口、创建 gate、技术详情、按实际布局裁剪的 AX；恢复动作不修改 Host 权限 |
| `src/ui/shell_layout.rs` | ~486 | R2 Wave A 壳层几何合同：`resolve`（唯一计算入口，render 与 AX 树共享）——宽窗 rail=288 / Inspector 打开时 440（OPT-4b 起 AppView 初始偏好为折叠，resolve 合同不变），窗口宽 ≤1279 时 rail=240；GUI2-05 起输出 `InspectorPlacement { Hidden, Side, Center }`（`can_fit_side_inspector` 不满足且偏好打开 → Center 中央呈现，`inspector_open` 仅表示 Side；`InspectorMotion::snap` 跨窄窗不动画）（Workspace ≥560）；R7 Wave C 在 150% 字号下改用 320px rail，宽度不足 1320 时保持 Inspector 折叠，1080 窗口保留 760px Workspace；固定侧栏 `flex_none`，防长文本 min-content 挤窄 Inspector；rail 顶部 36px traffic-light 安全区；4 个 GPUI 布局测试 |
| `src/ui/accessibility.rs` | ~417 | 平台无关 `AxTree` / `AxNode` / role / action / request / rect 模型；声明 Settings AX 子模块；3 个测试 |
| `src/ui/subagent_panel.rs` | ~940 | Inspector「子代理」对话栏：顶栏代理 chip（display_agents 顺序，名称前 Ø6 状态点——running 蓝 / waiting 琥珀 / completed 绿 / failed 红；点击切换并重查，切换后选中 chip scroll_to_item 滚入横滑视口；RV-06 去定高，垂直 padding 内容驱动，100%/125%/150% 字号下文字 / 状态点 / 焦点框完整）+ Refresh；正文 = 元信息（标题 · 生效模型 · 状态）+ 主代理→子代理任务盒（仅子会话首条 user 消息；后续 user 回合用普通用户样式，不冒充任务盒）+ 过程（assistant 复用主 Timeline Markdown 渲染、思考全文随外层 transcript 滚动——不做内嵌滚动避免滚轮被内层吃掉、RV-04 工具行：PanelItem::Tool 携带 event_id/detail/arguments，默认折叠一行（图标 + tool_headline 目标 + 状态 + chevron），点击 / Enter 展开检查参数 JSON（pretty / 原文回退）与结果 / 错误原文（detail 原文展示，权限拒绝不自行分类编造；超 10 行按主 Timeline RESULT_PREVIEW_LINES 预览口径 + 展开全文）、RunState / Error）+ 子代理→主代理结果盒（subagent_list result；RV-05 与正文同源 message_body_element Markdown 渲染（含复制入口），>600 字符或 >10 行默认折叠为摘要行 + 展开 / 收起来回切，展开后全文随外层 transcript 滚动，不做内嵌滚动）；未选 / 加载（含已选代理但列表未返回时显示 loading）/ 失败分层如实，空态仅真 loading 时显示加载中（断线 / 未取到数据显示过期提示）；RV-11 断线保留代理列表 / 选中项 / 正文与阅读位置（不再贴底复位），面板显示本地化「已断线，内容可能过期」横幅（本地化前缀 + 原始 reason，数据不翻译），Refresh / Cancel 由连接门控禁用，重连由 refresh_open_inspector_tab 重查恢复原选择与阅读位置；换代理 / 切会话仍贴底复位，content_arriving 仅对被选子会话事件触发；AX（RV-07）为 Group 状态摘要并按实测布局发布 chip / Refresh / 回底 Button 与对话正文节点（任务盒 / user 回合 / assistant / thinking / 结果盒发 StaticText、工具行发 Button+Press 且值含展开态结果摘要；tracked 条目布局按 transcript 滚动视口裁剪、零面积不发布，Press 走与 render 同源 handler，dispatch 新增 subagent-tool-toggle- / subagent-result-toggle- 前缀 fail-closed）；1 个测试（断线保留内容主路径） |
| `src/ui/accessibility/app.rs` | ~8983 | 工作台三栏语义树与 Press 白名单（含 Settings identifier 派发、终端标签 / 新建标签）；P0-2 的 `task-rail-grouping` 发布目标动作 name + 当前视图 value，Press 直接切换，不再发布 expanded/menu child；P0-3 空态发布 title / description / 单一 New task action，disabled 时不发布 Press；P1 的 tool group / Review 与 Activity 几何、P2 的 credential summary 隐藏和 Settings 写 gate 与 render 同源；UX-05 model 菜单使用实际布局 / 内部列表滚动框裁剪 AX，打开定位当前项，搜索与键盘高亮共用筛选结果；Settings 页树拆到 `settings_*.rs`；OPT-3 增 Manage models 弹层（触发器 / 单模型 Switch / 全开全关 / 头行常驻 Refresh / 空目录）与四默认角色触发器·菜单的 identifier 派发；ADR-056 增展开 chevron（Press、value Collapsed/Expanded）与展开区 Credentials 凭证行 / Usage 空态节点；UI-2 会话行动作与项目头计数 AX 几何跟 render 同源；GUI2-01 增首页 / 宽窄字号实测布局回归；35 个测试 |
| `src/ui/accessibility/settings.rs` | ~690 | Settings rail / 页分发；内容列宽经 `SETTINGS_CONTENT_PAD`（32×2）与 render 全宽列同源（OPT-4c 取消 820 钳制）；OPT-4d 增导航选中/未选中两态 AX frame 逐像素一致断言（F4 零位移） |
| `src/ui/accessibility/settings_providers.rs` | ~830 | Provider 64px 概览 AX；连接、目录、详情分层，普通 group value 不含 credential、endpoint、错误或 raw model id；列几何经 `settings_content_ax_width` 与 render 全宽内容列（`SETTINGS_CONTENT_PAD` 32×2，OPT-4c）同源（auth-methods 并入 name value 后 connection/catalog 平移至 +300/+440）；enabled 动作才发布 Press；OPT-3 增「Default models」四角色区（触发器 value 为当前对或「未设置」，候选 = 已连接 provider 的已启用模型，vision/search 标注只保存不接路由）与 Manage models 弹层 AX（Switch 状态词 On/Off、Enable all/Disable all、Refresh 常驻头行、空目录 fail-closed 文案；ADR-063 强度默认 / 范围 / 重置节点与 render identifier 同源，几何取滚动列表实测布局） |
| `src/ui/accessibility/settings_general.rs` | ~157 | Network 页 AX（wire/identifier 保持 General 兼容名） |
| `src/ui/accessibility/settings_permissions.rs` | ~193 | Approvals 整行 radio AX（identifier `settings-approval-mode-{wire}` 不漂；selected/enabled/Press 与 render 同源） |
| `src/ui/accessibility/settings_tools.rs` | ~135 | MCP 页 AX |
| `src/ui/accessibility/settings_terminal.rs` | ~218 | 终端页 AX |
| `src/ui/accessibility/settings_subagents.rs` | ~380 | 子代理页 AX：开关 / 输入 / Save / 模型规则控件与 render 同 gate 同 identifier；ADR-063 目录同源过滤启用模型，强度默认 / 范围 chip 节点与 render identifier 同源；RV-08 并发范围提示常发、错误原因仅出现时发布（StaticText） |
| `src/ui/accessibility/settings_appearance.rs` | ~179 | 外观页 AX（含字号与语言切换节点，name / value / selected 与 render 同源） |
| `src/ui/accessibility/settings_advanced.rs` | ~72 | 高级页 AX |
| `src/ui/accessibility/settings_about.rs` | ~39 | 关于页 AX |
| `src/ui/accessibility/macos.rs` | ~1054 | ADR-042 AppKit bridge：`GPUIView` AX root、`NSAccessibilityElement` 虚拟元素、frame / parent / hit-test / focus / notification / retain-release、settable/action 双门与 action 回调；结构不变（identifier/role/press 能力/子树形状）时原位刷新既有 element 而非整树重建，内部树同步 super 直调不触发 action 回调；6 个 macOS 测试 |
| `src/ui/barriers.rs` | ~174 | UI fixture barrier 发射器（R1 Wave B）：`BarrierSink` 读 `PAWORK_UI_BARRIER_DIR`（None 零开销直通）；`timeline_stable`（settle_seq 单调自增 / session_id / entry_count）重写与 `approval_visible` 写/删；tmp+rename 原子替换、IO 失败静默；1 个测试 |
| `src/ui/theme.rs` | ~848 | 深色单主题 token + 以 16px 根字号表达的 `Rems` 字阶 + 100%/125%/150% `TextScale` + `metrics` 尺寸常量；GUI3-08 近黑层次 canvas `#141416` / panel `#0e0e10` / raised `#222226`；P0-1 字阶与 4/8/12/16/24/32 spacing、6/8/12 radius、Composer 16px 圆角、2px focus、36px icon button 命中区（OPT-4a；GUI3-05 起可见图标为 `ICON_SIZE`=20 / `ICON_SM`=16 SVG）；`motion` 含 Inspector 180ms 与 Switch 120ms ease-out，无循环闪动；单一 dark palette，不读取系统显示偏好；3 个可读性/字号边界测试 |
| `src/ui/i18n.rs` | ~1257 | 界面文案目录（English / 中文）与全局语言态：`Language`（默认 English）、`AtomicU8` 当前语言、`t(key)` / `t2(key, a, b)`（未知 key 原样返回）、`set_language`、`catalog_overview_label` 与纯函数 `localize*` 测试面；3 个定向测试 |
| `src/ui/timeline.rs` | ~1070 | Timeline 容器：gpui `list()` 变高虚拟化与显式跟随；`timeline_rows()` 同源组装行（消息 / 思考 / 错误 / 工具组 / 中间相位 / 终态摘要 / live 用量），按既有 run/order 把连续 tool 聚合并由 terminal summary 吸收重复相位；P1 以首个 tool event id 作为稳定折叠 key，live / replay 共用结构，折叠态进入共享行高 / 可见窗口 / AX 公式；`timeline_area` 同帧复用单份 `timeline_rows()` 快照（list 渲染闭包与溢出判定共享，不重复组装），条目与审批卡共用 `timeline_item_wrapper`；880px 居中阅读列、短会话 Top 对齐、首页无任务可直接 Composer 开 Unassigned 对话（Ghost New task 仍可达；GUI3-08 空态加装饰 Task 图标），`TIMELINE_OVERDRAW`=200px 合同不变；RV-02 起含本机附件的用户消息按 `user_segment_layouts` 分段测高（附件块 = border 2px + 折叠头 + 展开时 thinking 同款内容体，渲染 / 测高 / AX 同源单次累加） |
| `src/ui/timeline_entry.rs` | ~1899 | 消息、tool group、Run summary/footer 与 error 的工作单元呈现；GUI3-01 起 tool group header 由 `tool_headline`（内建 8 工具按 `path` / `command` / `pattern` 抽目标，80 字符截断，MCP / 畸形回退名称）驱动：单工具直接 headline，多工具 `N tools · 前 3 条 headline · <state counts>`，默认折叠，mouse / Enter / Space / AX 共用折叠状态；展开态 headline 作主行、参数 JSON 仅在无目标时次级显示、结果首 10 行预览 + `{event_id}:result` 展开键；运行中行「In progress」+ accent 静态点；`assistant_is_streaming` 驱动作者行「Generating」（空闲不常驻 Pawork）；用量入常驻运行栏、live / 终态页脚与「···」菜单 Info 行 / tooltip（上箭头入 / 下箭头出 / 时钟）；状态与 detail 不伪造 wire 缺失耗时。Run summary 按 Completed / Failed / Cancelled 区分，只有当前 Session 存在至少一个真实、可审阅的 Changes 文件时才显示唯一主 CTA `Review changes` 并聚焦 Changes；GUI3-04：Failed 为 Codex 式紧凑 pill（raised 底 + subtle 描边，16px `warning` 图标与单行截断原因同取 `danger_text` 红色警示，完整原因由 AX description 暴露），`failure_next_step` 命中认证关键词（401 / 403 / authentication / unauthorized / invalid api key）时追加一行 24px Raised「Open provider settings」（`run-open-providers-<event_id>`，复用 `on_manage_composer_models`），不加假 retry；Completed / Cancelled 仍 Ø20 状态圆标题行，取消无卡只留页脚；高度经 `timeline::run_summary_card_layout` 与 AX 同源；Open in editor 无 capability 不画；RV-02：用户消息含附件标记时 `user_message_body_element` 分段渲染——文本段仍 Markdown，附件段为默认折叠块（复用 thinking 折叠头与 `expanded_timeline_details`，展开显示完整文本内容），给模型看的英文控制说明不上屏，含附件气泡强制整宽；7 个测试 |
| `src/ui/markdown.rs` | ~1084 | UI-3 / UX-02 Markdown 子集：管线表格、保留原换行的代码复制、HTTP(S) 链接打开 / 复制；`message_actions` 供消息菜单共享；R-03 起 parse 结果按正文文本跨帧缓存（render / 测高 `MessageMeasure` / 消息菜单 / AX 代码复制共用；parse 只读取正文，宽度 / 字号不影响块结构、不参与缓存键，容量 4096 超限整表清空）。GUI3-03：`Block` 带 `language`；代码块 36px 代码头（左语言标签 / 「Code」回退，右 36×36 悬停显现 `⧉` 复制图标，AX `{event_id}-code-{index}` 常可 Press）；行内 HTTP(S) 链接经 `InteractiveText` 直接可点，链接打开 / 复制动作只在「···」菜单（行文案 `Open link N · {截断 URL}`）；表头 `surface.hover` 底、列宽按内容 `shape_line` 实测 + 12px；GUI3-08 流式助手最后一行末尾静态 2×14 caret（不改测高）；2 个定向测试 |
| `src/ui/approval_card.rs` | ~166 | 审批卡：警示卡 + Allow once / Allow for run / Deny 三按钮（P4 片 3 按钮 32px 槽位，卡高 `approval_card_height` 公式与 AX 同源：p_2 + 标题/reason（+可选 detail）行数 + 按钮行）；app 级 focus handle（虚拟化卸载不丢失）；禁用原因 tooltip；R7 Wave B 的 mouse / keyboard / AX 三路径汇入同一 gate，决策后关闭旧菜单并把焦点交回 Composer |
| `src/ui/input_area.rs` | ~1240 | UI-4 Composer：880px 居中列，两侧至少 28px、顶部 16px、底部 24px；输入卡片 16px 内边距、16px 圆角、轻阴影、聚焦 accent 描边、12px 输入 / 动作间距、110–220px 高度预算。模型 Raised chip 与 ADR-063 推理强度 chip（相邻排布，标签 = 显式强度或「自动」，菜单含自动 + 当前模型可选范围；选模型自动带入默认）与 Send/Cancel 同为 36px 命中区，模型触发器随选中名称收缩（上限 220px，长名截断），菜单向上展开：已连接且有启用模型的供应商组头与模型同行展开（未连接 / 0 启用整组隐藏）。卡片外显示只读项目 chip 与 ContextMeter，UX-03 将无项目文件工具提示与项目新建入口并入元信息行，窄窗随宽度换行，status_hint 与字号反馈改落 StatusBar 右栏；render / AX 共用实测布局与说明。per-session 草稿、IME、发送 / 取消与空模型 gate 沿用；2026-09-21 增本机附件和搜索开关，附件行最多 96px 并可滚动，卡片按附件增加高度；菜单 hint 区分 Files（图文混述含数量与大小限制）与 Image（仅图片限制）入口（RV-03）；没有队列能力。5 个定向测试；空 display_name 回退 id 并入真实分组菜单用例。实际 GPUI 布局与 AX 操作区对照位于 accessibility/app.rs |
| `src/ui/inspector.rs` | ~966 | InspectorTab 为 Changes / Terminal / Resources / Browser / Files 与默认 Home；PanelTab 统一文件 / 工具 / PTY 顶层标签，横向滚动、就地关闭、Home 工具入口与 Side / Center 放置同源。终端无旧尺寸 stepper，以输出视口自动 fit，经 terminal_view::terminal_screen / render_terminal_lines 渲染；输出面支持聚焦、键盘与 IME，生命周期经 Host。 |
| `src/ui/terminal_view.rs` | ~355 | 终端显示的 gpui 胶水层：[pawork-terminal](terminal.md) 属性分段 → 主题色 StyledText runs、Keystroke → KeyEvent 转换、resize 防抖常量；行缓冲解析 / 按键映射 / 面板像素→列×行估算在 `crates/terminal`；2 个测试 |
| `src/ui/changes.rs` | ~1274 | Changes 面：Files / Summary、只读 DiffView、latest-session mismatch fail-closed 与诚实 scope；empty / unavailable / stale 使用分层占位；GUI3-08 加载态保留标题并加静态骨架。GUI2-05 / R-04：`parse_hunk_header` / `diff_hunk_line_numbers` 从 hunk 头本地递推双列行号（不改 wire，畸形头空行号）；整行 `diff.*` 浅底 token；32px 紧凑文件行 `changes_status_chip`（M / A / D 色标 + 路径 + `+A/−D`）。`has_reviewable_files_for` 要求 active session、Ready、非 stale 且文件非空，供 Run summary render/AX 同源门控。折叠态 Header ActivityPopover 为 320px 内容宽，GUI 1.20 增「子智能体」展示卡（标题 + 运行/完成计数；ADR-063 起行内单行 = 名称 + 生效模型 + 强度 + 状态，运行中 / 等待中稳定置顶，列表 clamp 至 5 行并限高滚动；行可点击（进入 Inspector「子代理」对话栏）；仅绑定当前活动会话，无会话 / 加载 / 空态如实展示，取消入口留在协议层）；7 个测试 |
| `src/ui/resources.rs` | ~371 | Resources 页：MCP server 只读表 + `ResourcesPanelState`（epoch 防过期）+ 手动刷新；empty / unavailable / error / stale 分层；GUI3-08 加载态加静态骨架；SET-6c 权威回执 bump epoch；3 个测试 |
| `src/ui/settings/mod.rs` | ~1684 | English / 中文 Settings 壳与稳定九页导航（GUI 1.20 增「子代理」页与共享控件 identifier / 解析、权限标签、并发解析）（GUI2-06：导航按「模型 / 工作台 / 系统」三组加 12px 组头 `settings-nav-group-*`，组内全隐则组头隐；查询非空时导航被查找结果替换；`settings_element` 支持定位行标识）；OPT-4c 起内容列用满 Rail 外可用宽度（`SETTINGS_CONTENT_PAD` 32×2，不再保留 820px 上限），共享脚手架单列 + 滚动层，受限高度内纵向滚动、切页归零；OPT-4d 导航选中/未选中共用同一外壳几何（UI-5：中性背景 + 字重；焦点描边经 focus_ring 覆盖层，零布局参与），文字坐标零位移（F4）；共享状态 / gate、descriptor 驱动认证动作与 Connect API key 编辑入口；5 个测试（空 shell Save 映射 null；OPT-3 角色区/弹层渲染 gate；角色控件 identifier 构造/解析互逆） |
| `src/ui/settings/search.rs` | ~868 | GUI2-06：静态目录 `settings_search_entries()`（page / row_id / 中英标题 / 说明 / 别名）与 `filter_settings_search_entries`（忽略英文大小写、按页面 gate 过滤）；Settings 左栏返回按钮下独立 `TextInput` 查找框（`settings-search-input`，不进草稿、不读 secure 缓冲，离开 Settings 清空）、结果列表（`settings-search-result-*`，↑/↓ / Enter / Esc）；`locate_settings_entry` → 切页 → `finish_settings_locate` 用实测框滚入目标行并约 2s 标识（`surface.hover` + 2px accent 左缘）、焦点落控件；只导航不写；供 `quick_search.rs` 共享页面 / 设置行标题；2 个测试 |
| `src/ui/settings/provider_quota.rs` | ~515 | Go 账号三窗进度条 + 已用百分比、重置倒计时与过期/加载标注；AX 仍用完整已用/剩余/来源文本；刷新、选择模式控件与共享 AX 状态（ADR-060 / ADR-061 / UX-06） |
| `src/ui/settings/providers.rs` | ~2872 | provider 概览卡头（GUI2-06：固定 52px 单行五列——中性状态色点 · 名称 · 认证方式 · 「已连接 · N 个模型」chip · chevron，`settings-provider-header-{id}` 布局 id 钉 48–56px；原 64px 两列文字卡头作废）+ 独立认证操作详情行；普通层不显示 masked credential、endpoint、catalog error 或 raw model id；供应商列表后的「Default models」四角色区取代独立默认模型 section；Remove 保持二次确认；ADR-056 起卡片改为「64px 五列卡头 + chevron 展开区」（展开态按 provider_id 存 projection，有效展开 = 显式态 ∨ 流程态）：UX-06 展开区优先展示 Accounts（旧 Host 为 Credentials）区，其后是 Proxy / Manage models；账号区（ADR-061 紧凑卡：状态点 · 名称或行内重命名 · kind 徽标 · 当前选择 · Use/Rename/Remove；副标题 masked · 状态；添加不再要求手填名称，minor ≥17 发空串由 Host 生成默认名，旧 Host 本地回退脱敏 key / `OAuth`；空列表诚实空态，env fallback 不入列）与 Usage 行（Go 由 ADR-060/061 按账号展示三窗进度条与已用百分比；其它来源不可用时保留 unavailable）；展开区文本列一律 flex_row + flex_1 骨架（region 根 min_w_0 会以 Definite(0) 测量毒化 auto-basis 裸 flex_col 文本，真窗口实证）；ADR-052 provider 级代理 Switch：仅 Global `proxy_url` 已配置时可见，按生效值并排 On/Off 状态词，writes 总闸关闭（断线 stale）时 disabled；mouse / Enter / Space / AX Press 共用 `on_settings_toggle_provider_use_proxy`（OPT-3c 起控件形态为 `Switch`）；ADR-055 增供应商列表后的「Default models」四角色区（角色菜单候选 = 已连接且已启用，含 Clear；候选为空仍保留 Clear（鼠标/键盘/AX 同源）；vision/search 标注 save-only）与每 provider「Manage models」弹层（`MenuPanel` 内单模型 Switch 行 + Enable all/Disable all + 头行常驻 Refresh，禁用命中角色默认对时显示 cleared_roles 提示；目录为空显示诚实空态，Refresh 常驻头行，不渲染全开假按钮；ADR-063 每模型附推理强度行——来源徽标（目录 / 手动 / 未知）+ 默认强度 cycle + 范围 chips + 手动范围时重置，`set_model_reasoning` 全态写回执收敛不乐观更新，写在途禁用） |
| `src/ui/settings/general.rs` | ~178 | Network / HTTP proxy（wire 保持 General 兼容名） |
| `src/ui/settings/permissions.rs` | ~310 | 五档整行 radio（GUI2-06：Ø16 圆环 1.5px `text.secondary`、选中 Ø8 accent 实心，取代 `●` / `○` 字形）+ 会话信任；row click / Enter / Space / AX Press 共用 handler；`ApprovalModeWire` English 标签在 `approval_labels.rs` |
| `src/ui/settings/tools.rs` | ~234 | MCP list/test/remove |
| `src/ui/settings/terminal.rs` | ~245 | 终端默认值 |
| `src/ui/settings/subagents.rs` | ~760 | GUI 1.20「子代理」页：全局开关 + 并发上限（1..=16）+ 按模型规则（可发起 / 可充当 + 7 项功能权限 chip + 重置默认）；ADR-063 目录只列启用模型，能力徽标（图像 / 搜索）+ 默认强度 cycle + 可选范围 chips；全部即时全态写、回执收敛不乐观更新，写在途禁用防重复提交；RV-08 输入旁常显 1–16 范围提示、非法非空输入显示 danger 原因（render / AX 同源逐帧推导，空输入按待输入）；RV-09 全局开关文案改为对所有项目 / 新发起运行生效 |
| `src/ui/settings/appearance.rs` | ~234 | 本地三档字号 + 随当前字号即时变化的正文 / control 样例 surface + English / 中文语言切换（同源按钮，切换走 `AppView::set_language`） |
| `src/ui/settings/advanced.rs` | ~158 | 连接诊断 definition list，长路径正常换行与纵向滚动 |
| `src/ui/settings/about.rs` | ~72 | Host data directory 只读 definition list |
| `src/ui/settings/approval_labels.rs` | ~32 | 五档 English label/description |
| `src/ui/task_rail.rs` | ~1275 | Sessions 侧栏：GUI2-01 顶部两行——任务 / 新建、筛选 / grouping；12px 连接状态移到底部 Local / Settings；grouping 图标/tooltip 表达目标动作，mouse / Enter / Space 共用 `toggle_grouping`，切换关闭其它浮层并保留 active session、scope、draft 与 collapsed projects；日期/项目/44px task 行、状态点、unread/blocked、Reconnect 与 Local/Settings 语义不变；列表仍用行级 bounds，在 grouping/scope 变化后滚动 active task 到可见；rail 键盘焦点链与 scope 菜单从触发器下方 8px 展开，MenuRow 单勾，AX 同源锚点；UI-2 / ADR-054：任务行右侧改名/归档在悬停、行或动作聚焦、当前会话时露出（命中区 32×32=`RAIL_SESSION_ACTION_SIZE`，与时间戳共用 64px 槽；项目头计数进入同一悬停/焦点面；键盘/AX 可达；改名行内编辑 Enter 提交、Esc 取消、空白不提交） |
| `src/ui/text_input.rs` | ~1998 | `TextInput`（Composer / 终端 / Settings 共用，视口高度上限至少容纳当前字号单行与内边距）：内容 / 动态 placeholder / IME marked_range / UTF-16 映射 / 视口 max_h + overflow scroll（TextElement 按完整内容高布局，视口由父容器 max_h 兑现，caret 滚进视口按 ScrollHandle 容器高计算；鼠标与 IME 坐标映射基于归一化布局原点 content_bounds——prepaint 时 origin 只减输入框自身 ScrollHandle offset，保留祖先页面滚动位移——再减 scroll offset，行高取 paint 时 last_line_height）/ 选择复制剪切 / Undo Redo / reset_text 草稿恢复；UX-01 起 Composer（`TextInput::new`）按可用宽度软换行，GPUI 换行边界对齐完整 grapheme 并以最终显示字宽复核，测高 / 绘制 / 光标 / 选择 / 鼠标 / IME / 滚动共用同一显示行与原文 byte 起点，不改草稿原文；`with_placeholder` 的终端 / Settings 字段、secure 与只读 URL 不启用软换行；OAuth `read_only()` 模式保留选择 / Copy、长行横滚，阻止用户编辑与 IME，Enter 不冒泡发送；SET-010 secure 模式（SET-4）：渲染与 AX value 只发布 grapheme 掩码、Copy/Cut 不写剪贴板、粘贴 / AX set-value / IME 剔除 CR/LF（单行语义）、光标 / 选择 / 鼠标映射经 grapheme 偏移换算；文件编辑模式使用等宽字体、全高视口与横向滚动，Enter / Tab 不触发聊天；14 个测试 |
| `src/ui/u1_probe.rs` | ~322 | R1 Wave C U1 spike：真实 TextInput/Button/overflow 探针；R5 Wave B 增 SelectAll/Copy/Cut/Undo/Redo、IME commit 单次入栈（真实 EntityInputHandler 路径）与空输入不可发送、Wave B 键位（含 Shift-Enter）经 keystroke→keymap→action 真实链路覆盖；时钟推进与几何自测已删，debug_bounds 只服务点击/滚轮命中；10 个测试 |
| `src/ui/components/mod.rs` | ~19 | 组件族模块声明（含 `icon` / `empty_state`） |
| `src/ui/components/empty_state.rs` | ~87 | GUI4：居中空状态容器（可选图标 + 调用方标题 / 说明 / 动作 / 骨架）；不发布 AX，identifier 仍由调用方 shell 节点承担 |
| `src/ui/components/icon.rs` | ~281 | GUI3-05：`Icon` 枚举 41 项 + `Icon::path()`（`icons/<kebab>.svg`）/ `Icon::all()`；`icon()`（20px `flex_none` `svg()`）与 `icon_sized()`（16px），默认 `text.primary`（gpui 将 SVG 渲染为 alpha mask，必须在 Svg 上染色；调用方可再覆盖，如 Send/Cancel 的 `on_accent`）；`Assets` 以 `include_bytes!` 静态表实现 gpui `AssetSource`（`load` / `list("icons")`），资产在 `apps/desktop/assets/icons/`，不引入 rust-embed；`TestAppContext` 无资产源时 svg 静默占位；1 个测试（全部登记路径可加载、`<svg` 开头、数量一致、无重复） |
| `src/ui/components/switch.rs` | ~194 | `Switch`（OPT-3c / GUI3-08）：36×20 轨道 + 16px 滑块，On/Off 状态词由调用方并排渲染；disabled 无 Press；mouse / Enter / Space / AX Press 同源；GUI2-06 关态 hover 用 `surface.hover` 不走 `accent.hover`；状态切换 120ms ease-out 位移，首帧瞬移避免挂载滑入 |
| `src/ui/components/skeleton.rs` | ~29 | GUI3-08：Changes / Resources 加载骨架（4 条静态 hover 色条，不循环） |
| `src/ui/components/button.rs` | ~409 | `Button`：六 variant（Primary / Ghost / Danger / Success / Raised / IconCircle；IconCircle 为 Composer 32×32 圆形动作槽）的底色·文字色·hover/pressed 映射；默认 14px control / `CONTROL_RADIUS`；`child()` 承载自绘图标；焦点描边经 `focus_ring` 覆盖层（零布局参与）；`ButtonPadding` 四档及描边/尺寸/对齐 builder；disabled 无 pointer cursor、mouse press、hover 或 keyboard activate，仍保留可读文字与 tooltip |
| `src/ui/components/dropdown.rs` | ~262 | `Dropdown`（触发器 + `deferred(anchored())` 浮层；可选局部 Corner/Point 锚点，anchored 负责窗口碰撞）、`MenuPanel`（220–360px、8px padding、r6、strong border + menu-only shadow、默认最大高 240px、内部滚动、`occlude()` + 外点关闭）、`MenuRow`（34px；选中用 accent check + raised surface，不用整行亮蓝；hover/pressed/disabled/键盘高亮语义；长 label 单行截断）、`ANCHOR_GAP_Y`=8px |
...
| `src/ui/components/follow_scroll.rs` | ~94 | `FollowScroll`（`ScrollHandle` + 跟随位：贴底判定 / 脱钩 / 重挂；现仅终端使用）与 `BackToBottom` 回底控件容器（绝对定位右下） |
| `src/ui/components/label.rs` | ~48 | `Label`（单行文本，token 化字号 / 颜色） |
| `src/ui/components/list_row.rs` | ~188 | `ListRow`：Task 行与 ProjectHeader 行两形态，默认行高 44 + 垂直居中；`height()` 供两行 Settings 审批项使用 3.5rem（56/70/84px），不改变 TaskRail 默认；`radius()` 覆盖外壳与焦点描边圆角；`min_w_0` 保证子项 truncate 拿到确定宽度；selected/hover/pressed 使用 raised/hover/pressed surface，焦点描边经 `focus_ring` 覆盖层（零布局参与）；裸 Enter / Space 调用与 click 同一激活 handler |
| `src/ui/components/panel.rs` | ~85 | `Panel`：`side_right`（TaskRail，右描边 + gap/p-2）与 `side_left`（Inspector，左描边）固定宽面板壳；固定宽时 `flex_none`，禁止 Workspace 长内容把侧栏挤窄 |
| `src/ui/components/status_bar.rs` | ~101 | `StatusBar`：底部 30px 三栏（左项目 / 分支、中用量、右瞬态反馈或连接文案）；用量仍绝对居中，左右栏 `occlude` 避免挡住；AX 只发布居中 `run-status`；R6 Wave A 起不再承载 Inspector/Activity 动作 |
...
- **GUI2-01 Header**：最小 64px，上下各 12px 内边距，内容可撑高；保留任务标题、权威 branch / live 状态和 Activity / Inspector 入口。AX 使用实测 Header 高度划分 Timeline 与 Composer。GUI3-07：查找 / 回合入口为 36×36 Ghost 图标按钮（`⌕` / `≡`，20px，`text.emphasis`，tooltip / AX name 走 i18n）；branch 与 live 状态为 Raised 12px chip（r6，pad 6/4），无数据不渲染不留空洞。
- **GUI2-02 阅读与 Composer**：用户消息按内容收缩、右对齐，普通正文最大为阅读列 80%，代码 / 表格可使用整列；内边距纵向 12px / 横向 16px、圆角 12px。时间与菜单在气泡外，省去重复用户标题；助手保持开放排版。菜单悬停 / 聚焦 / 已打开时显现，键盘与 AX Press 始终可达，动作框读取实际布局。代码与表格在块内横向滚动，复制仍取原始内容。模型菜单底部固定「管理模型与供应商」，空目录 / 无结果也保留；鼠标、方向键 / Enter 和 AX 共用已有 Settings 导航，直接进入 Providers，保留任务、草稿和 Run。底部 30px StatusBar 在工作台常驻（Settings 壳仍不显示）；运行栏、live 页脚与终态页脚同源展示 ↑ 入 / ↓ 出 / 时长（有权威时长且 output>0 则 tok/s）。运行中按已上屏正文字符预览用量并走秒表，终态有持久化 usage 则回落权威读数并固定时长；render 与 AX 共用显示条件。
- **GUI2-04 对话查找与回合目录**：Header「查找 · ⌘F」/ `Cmd+F` 打开工作台内联查找栏，只查当前已加载用户 / 助手正文（忽略英文大小写，计数以匹配消息为单位），↑ / ↓、Enter / Shift+Enter 循环定位，当前命中整条消息以背景标识；分页中范围文案如实标「历史加载中 · 仅查找已加载正文」，无结果不访问 Host。多个用户回合且内容超出一屏才显示「回合」入口，摘要列表可滚动、选择后定位并关闭。定位键为协议消息 ID + run_id（事件 ID 兜底），流式转提交与 echo 转持久行后身份保持，渲染前重求虚拟行号；跳转脱离跟随，手动滚动解锚但保留阅读模式，显式回底才恢复跟随。切任务清空命中，关闭恢复入口焦点；协议与共享投影不变。
- **TaskRail（左侧栏）**
  ...
  - Timeline 分组 = 日期桶（Today / Yesterday / Previous 7 days / Earlier）→ 项目 → 任务；GUI3-06：桶内仅一个项目组且为 Unassigned 或等于当前 scope 时跳过项目头（`TaskRailDateGroup::skip_project_header`，render / focus stops / AX 同源），任务直接挂桶下。Projects 分组按 canonical workspace，缺 `workspace_id` 进 Unassigned（无「+」），始终显示头。
  - UI-2 日期桶头 12px medium secondary（GUI3-06 行高 24px）；项目头可点折叠（14px，chevron ▾/▸ + 名称 + 右对齐任务计数 + 定向「+」共用完整悬停 / 焦点面；「+」只建任务、不折叠；任务行 `w_full` 底色盖住时间槽与动作）；项目头计数 / 「+」与任务行改名 / 归档共用 64px 尾槽（两格 32×32，右缘 inset 8，`rail_trailing_origin_x` / `rail_trailing_plus_x` render / AX 同源）；项目级「+」按该 workspace 新建任务，同项目已有未开始任务时复用该条。
  - 任务行（UI-2：44px，GUI3-06 起 150% 字号为 36px（`rail_task_row_height`），选中 raised + 圆角 6）：状态点 Ø10 仅在有 live 语义时出现（Needs input=琥珀实心 = 有待审批；Running=accent 蓝实心 = 该 session 有 active run；Blocked=danger 红实心 = R3 Wave B live 派生的 failed / interrupted 终态，优先级 Needs input > Running > Blocked；GUI3-06：空闲不画点也不占位，AX 不发布 `status-dot-*`；wire 无每会话终态字段故不画终态绿点）、标题单行 `.truncate()` + 右侧 16px 渐隐（unread 时 SEMIBOLD，同字号同行高不改几何，不加 dot / 徽标）、14px 标题、8px 间隔、12px 相对时间右对齐（now / Nm / Nh / Nd）；时间与改名/归档共用 64px 尾槽，悬停、行或动作聚焦、当前会话时露出动作（标题宽度不变；150% 空闲行不保留尾槽，露出动作时标题槽 > 80px 且不被遮挡），归档为 16px 单色盒形图标（ADR-054，命中区 32×32；改名为行内编辑，Enter 提交 / Esc 取消 / 空白不提交；归档立即生效、仅隐藏不删除）；任务 click、行级 Enter、AX press、Cmd+Opt+↑/↓ 与 Cmd+Opt+N 都先关闭旧菜单并在切换后聚焦 Composer。激活当前 task 不重开 session 但仍关闭菜单；仅一个可见 task 时 cycling 不重开 session，仍聚焦 Composer。
  ...
  - 底部 Local / Settings 行显示实际连接状态与状态点（GUI3-07：断线 `Local · Disconnected`、连接中 `Local · Connecting`，不再嵌入原始错误；断线时行内提供 Ghost「Retry」`reconnect`，identifier / -17 Tab 档 / handler 不变，不再渲染列表上方全宽 Primary）；主区 recovery 继续提供断线原因（原文完整换行显示）、唯一 Primary `connection-retry`「Retry connection」和诊断。头像 / 姓名 / quota / 组织等无权威来源元素不画。侧栏新建、筛选、分组、连接和 Settings 的 AX 框读取实际布局。
...
- **Settings 认证后目录**：`AuthChanged::Succeeded` / `Removed` 与页级 Refresh 走 `refresh_models_authority`（`provider_auth_status` + 过滤 / 全量 `model_list`）。模型启用 / 全开全关回执按 Data 本地收敛弹层与 Composer，不再重探全通道（ADR-055 D2a）。Manage models 弹层头行常驻 Refresh，不限于空目录。
- **Inspector（cmd-i 开合；GUI2-05 起按 `shell_layout::InspectorPlacement` 放置）**：偏好关为 Hidden；宽窗（默认字号 > 1279 且 ≥ 1288，150% ≥ 1320）为 Side 440px 右栏；窄窗显式打开为 Center——同一面板占中央 Workspace（TaskRail 保留），顶部 36px「Back to conversation」`inspector-back`，不渲染 Timeline / Composer 且 AX 不发布，但保留实体、草稿、跟随 / 滚动偏移与连接状态；`pending_approval` 时面板顶部「Return to the conversation to handle the approval」`inspector-approval-hint`（点击即返回，不默认批准）；跨窄窗切换 `InspectorMotion::snap` 不动画。面板头 48px = 统一工具 / 文件 / PTY 标签栏与打开工具菜单 Dropdown（`inspector-panel`，value 当前面板，菜单 inspector-panel-menu 列实际接通的工具（含 Browser / Files / 子代理对话栏），`MenuKind::InspectorPanel`）+ 36×36 `inspector-collapse`，取代 UI-1 三页签；默认 Home 工具入口，Activity / Review 显式进入 Changes；Changes 内二级条 40px / 每项 96px / 12px 字；各页滚动状态独立保留。
  - Changes：Files / Summary 二级页签 + ↻ 手动刷新。Files = 32px 紧凑文件行（`M` 琥珀 / `A` 绿 / `D` 红色标，其余状态原样 · 路径 · 右对齐 `+A/−D`，≤200px 内滚动）+ DiffView（等宽 Menlo；hunk 头 raised 底 secondary 字并按 `@@ -a,b +c,d @@` 本地递推双列 32px 行号，畸形头空行号；addition 行 `diff.addition_bg` / deletion 行 `diff.deletion_bg` 整行浅底 / context 行 panel 底，不复用按钮语义色；长行 `overflow_x_scroll` 横滚不折行；binary 显示「Binary file — not rendered.」）。Summary = 七字段行（Session / Files / Lines / By status / Branch / Dirty files / Work dir，缺失显 unknown）。数据会话 ≠ 查看会话时顶端 banner 如实标注。
  - Terminal：与 Changes / Resources 共用顶层标签栏（选择 / 新建 / 就地关闭），正文为可直接输入的终端窗口；移除独立 TextInput、发送 / Start / Size 按钮和手动尺寸步进器。首次打开且无 PTY、无错误时自动创建；原生字符和 IME 提交直接走 `terminal_write`，Enter / Ctrl-C / Tab / 方向键交给 shell，Cmd-V 根据 shell 的 bracketed paste 模式粘贴；每个 PTY 独立串行写入。SGR 着色、光标位置、清屏与自动折行来自输出解析，预编辑只显示在光标处。尺寸随输出视口自动适配，仍经 Host 确认；关闭标签经 `terminal_close` 成功后才移除，关闭面板保留 PTY。失败、只读与断线在终端处说明，保留诊断入口。
  - Resources：MCP server 只读表（name + state 徽标，`failed` 红字；`transport · N tools[ · last_error]` 次行）+ ↻ 刷新。
- **StatusBar + ActivityPopover（R6 Wave A / P1-4；GUI4 三栏）**：底部 30px StatusBar 在工作台常驻。左栏仅在有绑定项目或权威 branch 时显示 `{project} · {branch}`（无项目不占空洞）；中栏展示当前或最近一轮的 ↑ / ↓ / 时长，空闲保留最后一轮固定读数，无 Run 时为 `—`；右栏优先 `status_hint`，其次字号反馈，否则连接文案。AX 仍只发布居中 `run-status`。历史用量仍在各轮页脚核查。OPT-4b（F6）起 Inspector 默认折叠（宽屏同样），显式动作（重开 / Activity 摘要 / Review changes）才展开。Inspector 展开态由面板内右上 `inspector-collapse`（36×36、20px 字形）折叠；折叠态 Workspace Header 右上 `inspector-toggle` Activity 触发器与最右 `inspector-expand` 重开按钮并存（各 40×37 槽、4px 间距），点击后在触发器下方以右缘对齐展开 Popover（内容宽 320px，内容高随字号为 144/180/216px）：标题 Activity、Changes 标题与权威摘要 `N file(s) · +A/−D` 或 `unavailable`；点击摘要或 Run Summary 的 Review changes 均展开 Inspector、定位 Changes 并聚焦当前选中的 Changes 顶层页签；仅有 latest 会话差异时加来源说明。Agent/Add tool 无 capability 不画，也不为其保留空白高度。
...
| Terminal | Enter / Tab / 方向键 / Ctrl 字母、Cmd-V | 控制键直通 PTY、粘贴；普通文字走原生输入法，组合态 Enter 留给输入法 |
...
| 根节点 | tab / shift-tab | focus_next / focus_prev——沿 tab_index 档位链走焦（GPUI 无默认 tab cycle；macOS 上 NSWindow 在 sendEvent 层吞掉裸 Tab，keyDown 路径收不到，终端聚焦时先直通 PTY，其余主机制是 `install_appkit_tab_monitor` 的 NSEvent 本地监听器在派发前截获并调 `window.focus_next()` / `focus_prev()`，根节点 on_key_down 的 Tab 分支保留作非 macOS / 监听器失效后备；带 cmd/ctrl/alt 的组合键放行不接管） |
...
UI-6b 命名账号（[ADR-059](../settings.md#adr-059ui-6b-命名账号与持久选择2026-09-08) / [ADR-061](../settings.md#adr-061账号默认名称与重命名2026-09-13)）：Settings 账号卡显示名称、kind、掩码、过期与后续请求选择；添加不再要求名称，默认名由 Host（或旧 minor 本地回退）生成。Rename 仅 minor ≥17，行内编辑 Enter 提交 / Esc 取消。行与 Use/Rename/Remove/确认按 credential ID 绑定，AX 与键盘动作同源，选中账号仍有其他记录时禁删。握手 minor ≥15 才显示新增账号能力；旧 Host 保留原操作与旧状态解码。controller 通过现有 pawork-client 通用命令发送，不新增业务依赖。
...
- 语言切换入口在 Settings → Appearance（`English` / `中文` 两个同源按钮，AX identifier `settings-language-en` / `settings-language-zh`），mouse / Enter / Space / AX Press 共用 `AppView::set_language`：更新全局语言与本地镜像、给出 status_hint 并整界面重渲染。
...
「子代理」设置按供应商展示完整 `model_catalog`，包括自定义供应商的目录模型，不按 GLM 或其他模型家族筛选。每个 `(provider_id, model_id)` 独立设置可发起、可充当和功能权限；运行仍要求供应商可用、模型已启用且规则允许。

2026-09-21 RV-08 / RV-09 修复：并发上限输入旁常显「允许范围 1–16」双语提示；非法非空输入（畸形 / 越界）在输入下方显示 danger 色本地化原因，render 与 AX 经 `subagent_concurrency_error` 每帧从输入文本同源推导，空输入按「待输入」只显示范围提示，恢复合法值后错误自然清除；Save 维持解析 fail-closed 禁用，已保存值显示与手动保存语义不变。全局开关说明与 tooltip 统一改为「对所有项目 / 新发起运行生效，进行中运行不受影响」，与 Global-only 配置语义对齐（不改配置存储层级）。
...
UI-6b G2：新增 `ui/settings/provider_quota.rs`；Controller 通过 Client 查询指定 provider/credential 的 Percent 三窗与写选择模式。投影按稳定 ID 和请求 epoch 存读数，结合 connection generation 拒绝迟到响应；离页/断线/认证变化清理。账号区视觉用进度条与已用百分比，重置倒计时与过期/加载标在条下；AX 仍保留完整已用/剩余/来源/获取时间。既有每秒时钟在 30 秒或 reset 后更新状态；仅新 Host/Go 开放自动切换开关。按钮/开关及 AX 共用实际布局，扩展既有 Provider AX 回归。
...
2026-09-21 图片与模型能力流程：Composer「+ / 添加」在首页、无项目与项目任务均可打开；本机文件 / 图片使用系统选择器，后台读取限制为最多 4 个、图片每个 8 MiB、UTF-8 文本每个 64 KiB。独立 `ComposerOptions` 按 session / 首页保存附件与 `web_search: Option<bool>`，切任务不串用；系统选择器迟到结果归属原草稿，失败保留，成功 RunStart 回执清除，发送期间禁止重复发送 / 修改附件。仅附件也可发送。图像 / 搜索能力不匹配时显示提示并禁发，不静默换模型。Controller 顺序发送 64 KiB `attachment_upload`，再带 `attachment_ids` 与可选 `web_search` 发 RunStart；旧 Host 在上传前拒绝。暂存不授予目录权限。项目相对引用仍有独立入口，Escape 清引用模式。附件移除、搜索状态与菜单均有键盘和 AX 操作；模型菜单能力筛选保留。其余缺口见 [剩余工作与验收](../../ROADMAP.md)。
...
4. 时间线每次变化 `timeline_changed()` 递增代次；render 前 `sync_list` 对 `ListState` 统一 `reset(len + pending_approval)`（projection 有条目替换语义，splice 不安全）。R4 Wave A 起为 Top 对齐 + 显式跟随：跟随态由 `timeline_following` 单一表达（滚动事件 `visible_range` 覆盖末项即贴底），reset 后跟随臂显式 `scroll_to` 末项底；脱钩读史恢复 reset 前偏移（item_ix 越界钳制），视口不跳；回底 = BackToBottom / 滚回底部重挂（仅内容溢出时绘制）。
...
- 八种浮层（`MenuKind`：Scope / ProjectTask / Model / SettingsRole / SettingsProviderModels / Entry(event_id) / Activity / InspectorPanel）共用单一 `Option<MenuKind>` 状态位：开新即关旧、至多一个打开；Grouping 不再是 MenuKind。
...
| 新建任务 | 同项目已有未开始任务则打开该条；否则 `session_create`（workspace_id 可缺省）→ 重取 snapshot | `Snapshot` + `SessionCreated`（复用路径不发 create） |
...
2026-09-15 终端交互修订：按用户要求改为直接输入的终端窗口，移除命令草稿及手动尺寸交互。回归 `terminal_bytes_queue_in_order_per_pty` 验证并发 PTY 的按键顺序及失败隔离；`blank_history_and_inline_composition_have_complete_text_runs` 防止空行着色导致正式窗口崩溃。AX 仅发布可聚焦的输出区域，不允许 SetValue 替换 shell 正文。
...
- **Settings Terminal 页（SET-6d / ADR-050）**：导航仅在 Host `terminal_settings` 至少成功解析一次后显示（失败/未知隐藏且不渲染写入口）。页面显示 Host 权威生效值：shell null 文案 `Not set (uses the platform default)`，以及 columns/rows；Save 全态回传三字段，Clear 清除 shell，回执后才更新。新建终端初始投影尺寸与创建后 resize 优先取面板实测列×行，否则取 terminal_settings 生效值（未查询回落 80×24）；只影响之后创建的终端。畸形载荷 fail-closed；断线 stale 保留最后只读结果并禁写，重连自动刷新。
- **依赖边界**：生产 `pawork-*` == `{pawork-client, pawork-terminal, pawork-browser}`（deny-list 测试）；`projection/` 零 gpui / tokio / OS import；协议类型只经 client re-export。Settings Data 缺 nullable 键 fail-closed（与 Host 必填键对齐），不把残缺帧当成未设置。
...
- **诚实显示**：Run tokens 终态取持久化累计用量；运行中的字符预览只作 live 指示，有权威读数即替换，缺失为 `—`。底栏常驻用量与时长，不再为 quota 占位；ContextMeter 只用 catalog 的 `context_window_tokens`；Changes / Resources 未拉取显 unavailable 而非 0；`now_ms` 由 UI 注入，投影层不读系统时钟。
- **终端约束**：`terminal_create` 的 cwd 只接受 workspace 相对路径；终端面不持有本地 PTY。输出区直接接受原生文字 / IME / 控制键，光标与预编辑嵌入 shell 实际位置；每个 PTY 的待写字节独立排队，成功回执后继续发送，失败清理该 PTY 队列并显示错误。创建仍受 Policy 约束，首次打开自动启动，失败不自动重试；标签关闭须由 Host 确认终止/清理。尺寸仅由输出视口自动适配，经去抖、在途去重与匹配回执确认；无尺寸草稿或 Apply 操作。原始 output 不变，解析支持 shell 行编辑、SGR、清屏与折行，CJK 列宽按等宽字体测量；完整 VT / 备用屏幕与复杂 emoji 字素簇仍未完整实现。
...
- **Accessibility 单一语义源（ADR-042）**：`AppView` 只从 canonical UI 状态与布局 metric 构建显式 `AxTree`；壳层几何与 render 共享 `shell_layout::resolve`（100% 窄窗 rail=240、150% rail=320）；字号反馈与 `status_hint` 只出现在 StatusBar 右栏视觉层，不发布独立 AX 节点（`run-status` 仍只承载用量串）；稳定 identifier 与本地化 label 分离，macOS bridge 只做 AppKit 映射。AX press / focus / set-value 必须回到既有 handler 与 enable gate，未知请求 fail-closed；disabled 控件不得发布可执行 action。Grouping AX Press 直接 toggle；其余触发器先移 GPUI 焦点再开菜单。Timeline AX 与 render 共享 rows + approval item 序列、tool group 折叠态和 Review gate；稳定帧读取真实 list bounds，首帧用共享公式回退，视口外条目不发布。Settings 普通树不得携带 API key 明文或 masked credential 片段；secure input 只发布等长掩码，stale 时输入与所有写动作 disabled 且无 Press。IME composing 中 AX Send 与键盘 Enter 同样不生效。新增可见交互须同批补节点、bounds、状态和 action 映射；非 macOS 当前为 no-op，不宣称已有平台 AX 实现。
- **平台显示偏好**：单一深色 palette 保持冻结 token，不读取系统显示偏好（macOS Increase Contrast 桥已于 2026-09-04 随功能移除）。GUI3-08 仅引入有限 oneshot 动效（Inspector 180ms、Switch 切换 120ms）；流式 caret 与加载骨架为静态，不循环闪动，因此 Reduce Motion 仍不需要分支。
...
| `pawork-terminal` | path 依赖 | 终端显示核心（行缓冲解析 / SGR 属性分段 / 按键→PTY 字节 / 尺寸估算）；零依赖纯库 |
| `pawork-browser` | path 依赖 | 系统网页视图、导航校验、页面状态、显示与释放；无 Core / 协议依赖 |
...
| `cocoa` / `objc` | macOS target only：`=0.26.0` / `0.2` | 原生 AX bridge 与 WebKit 子树兼容 |
| `raw-window-handle` | `0.6` | 从 GPUI 窗口借用原生句柄交给 browser |
...
| `controller/mod.rs` | 16 | 既有 wire/解析/安全回归；R6 Wave B 增 terminal create 失败 workspace 归属与 diff 内容 session id 生命周期字段。SET-6d 增 terminal_settings 查询/全态写 wire 与回执解析主路径一条（shell Some/null 两态）。删除 last_acked 单调性和生命周期字段自证；保留握手能力面钉板。 |
...
| `ui/input_area.rs` | 5 | Composer placeholder / 面板高度、伪二级分组、未连接过滤、全禁用空态；空 display_name 回退 id 并入真实分组菜单用例 |
