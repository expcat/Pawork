...
- **渲染面自动门禁尚未完整**：R1 已建立 U1 与真窗口 AX/screenshot 来源；R6 Wave B 已把 DiffView 长行真实横滚纳入 U2（offset `-720.0 / 2477.0`），不再只靠历史人工取证。R7 Wave A 的 State A hover/active/focus 九图已由用户签字，并依用户决定以原生 AX tree/action + 纯键盘 + U2 替代本波 VoiceOver；R7 Wave B 的 focus/menu/approval 等价路径已由两套主 U2 与一套审查边角 U2 收口。R7 Wave C 已在默认平台态完成 1080×720、CJK/emoji/超长内容、1024 行虚拟化、反复 resize、断线重连及单次性能基线。VoiceOver 未执行，屏幕朗读措辞 / 顺序未验证；字号放大与主动 reduced motion / 高对比态仍未完成，性能阈值也未冻结；三张定稿图分区 SSIM 与终局视觉签字留 R8。
