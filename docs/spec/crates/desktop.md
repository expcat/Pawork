...
| `src/ui/quick_search.rs` | ~956 | GUI2-03：当前 Snapshot 任务标题 / 项目名与已有页面 / 安全导航查找；独立输入、稳定目标选择、滚动实测 AX、模态键盘隔离与 Esc 回焦；GUI2-06 起页面与设置行条目从 `settings::search::settings_search_entries()` 派生（空查询只出八页标题，非空加入 `SearchTarget::SettingsRow`），断线仍只允许 Appearance / Advanced；2 个主路径 / 断线回归 |
