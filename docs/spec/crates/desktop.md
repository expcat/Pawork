...
| `src/ui/mod.rs` | ~3077 | `AppView` 宿主：Workspace Header、Timeline、Composer 与 Inspector 三栏装配；R6 Wave A 的 Header Activity 触发器/Popover；R6 Wave B 将 terminal draft、pending write/create 按 workspace/terminal 归属，任务切换保存/恢复输入，连接恢复后刷新已打开的 Changes/Resources，并消费携 workspace 的 terminal create 失败与携 session 的 diff 回执；键位、焦点链、菜单互斥、1s tick 与 barrier settle；15 个测试 |
...
R3 Wave B 起 [scripts/ui-r3-wave-b-nav.sh](../../../scripts/ui-r3-wave-b-nav.sh) 以真窗口键盘注入驱动 22 相位导航回归；注入经 [scripts/ui-key-event.swift](../../../scripts/ui-key-event.swift)（CGEvent HID tap，R6 Wave B 增真实横向滚轮事件）。R5 Wave B 的 [九场景 driver](../../../scripts/ui-r5-wave-b-states.sh) 覆盖 Composer。R6 Wave B 的 [九场景 driver](../../../scripts/ui-r6-wave-b-states.sh) 覆盖 Changes 真 diff/summary/长行横滚、Terminal create/write/output/resize/reconnect/policy 拒绝、Resources 三态、折叠恢复、task/latest-session scope 与断线重连；审查前完整矩阵 19 份 AX 断言全绿，证据见 [u2-rootfix-pass-20260830](../../ui-review/r6-wave-b/u2-rootfix-pass-20260830/)。审查后两项边角修复已通过 app/Desktop 定向测试，最终二进制 U2 补录因 macOS AX 注册 flake 在场景前 fail-closed，状态见 [Wave B notes](../../ui-review/r6-wave-b/notes.md)。
...
- **渲染面自动门禁尚未完整**：R1 已建立 U1 与真窗口 AX/screenshot 来源；R6 Wave B 已把 DiffView 长行真实横滚纳入 U2（offset `-720.0 / 2477.0`），不再只靠历史人工取证。三张定稿图分区 SSIM、完整 hover/VoiceOver、虚拟化与终局视觉签字仍留 R7/R8；Entry 菜单锚点卸载后的短暂失联也仍需收口。
