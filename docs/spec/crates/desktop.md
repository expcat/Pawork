...
31 个 `.rs` 文件、约 32.0k 行，全部在 `[[bin]] pawork-desktop` target 内（无 lib target、无 crate `tests/` 目录）。
...
| `src/controller.rs` | ~2990 | `DesktopController`（connect / 事件泵 / 空闲心跳 / 全部 Command·Query 构造与响应解析）；`open_workspace` 复用冻结 `workspace_add`，成功后刷新 snapshot 并发 `WorkspaceOpened{id,name}`；SET-3 增 `provider_auth_status` 查询（provider_id=None 全量）与 `ProviderStatusLoaded` 事件；SET-4 增认证写命令 `auth_start` / `auth_set_api_key`（ApiKeySecret 只在调用栈上构造，明文不进 projection/日志）/ `auth_cancel` / `auth_remove` 与 `AuthStarted` 回执（verification_url / user_code / expires_at）；SET-5 增 `set_default_model` 命令构造与 Data 回执解析（确认 pair 经 `DefaultModelConfirmed` 投递后自动重查 `provider_auth_status` 取回权威 default；Error / 传输失败走 OperationFailed，不动 UI 现有状态）；SET-6a 增 `general_settings` 查询与 `set_proxy_url` 命令（null=清除；Data 回执 `{proxy_url}` 即写后状态，经 `GeneralSettingsLoaded` / `ProxyUrlConfirmed` 投递，Error 取 Host 脱敏 message 原文）；SET-6b 增 `permissions_settings` 查询与 `set_approval_mode` / `workspace_trust` 命令（Data 回执即写后状态，经 `PermissionsSettingsLoaded` / `ApprovalModeConfirmed` / `WorkspaceTrustConfirmed` 投递；workspace_id 取 Host 查询透出的 attached id（ADR-048 实现期修订，校验方与发送方同源），匹配校验在 Host）；SET-6c 增 `mcp_test` / `mcp_server_remove` 命令与 `McpServersReceipt` 事件（回执与 mcp_list 同形状即权威，Error 取 Host 脱敏 message 走 OperationFailed，不动现有清单；回执 bump epoch 使在途 mcp_list 失效）；SET-6d 增 `terminal_settings` 查询与 `set_terminal_settings` 全态写命令（shell/columns/rows 三字段必填，null=清除 shell；Data 回执同形状即写后状态，经 `TerminalSettingsLoaded` / `TerminalSettingsConfirmed` 投递，Error 取 Host 脱敏 message 原文）；SET-6f 在连接结果保留非 Secret `DesktopHandshakeInfo`（Host runtime ID、协商 API、granted capabilities），不新增 wire；`ControllerEvent` 对 terminal create 失败携带 workspace、对 diff 内容携带 Host 实际 session id；ADR-045 `terminal_close` 命令构造与 `TerminalCloseSucceeded/Failed` 回执（成功不直接改 projection：running 终态由 live `TerminalExited` 刷新，exited 清理由 UI 回执后本地移除）；14 个测试 |
...
| `src/projection.rs` | ~5360 | `DesktopProjection` 渲染适配投影；`ConnectionState` / `ResumeState` / `ResumeApply` / `TerminalState` / `PendingApproval` / `ModelEntry` / `ActiveRun` / `SessionSummary` / `WorkspaceSummary` / `SessionLiveStatus`；SET-3 增只读供应商页状态 `SettingsProvidersState` / `ProviderStatusEntry`（`provider_auth_status` 载荷解析 fail-closed、render 与 AX 同源 label、断线 `mark_stale` 保留最后只读结果）；SET-4 增 `AuthChanged` 六态投影（`AuthChange` 解析 tag=type/content=data，畸形 fail-closed 只记 error 不落地状态；`OAuthWait` 等待信息；Succeeded 清等待并置 `pending_status_refresh` 提示 UI 再查认证状态；Replace 基线（写流程起点已 Connected）上非 Succeeded / Removed 终态不清旧凭证、改触发重查，Removed 仍复位 NotConnected 并置 pending_status_refresh（目录与 env 残留交权威重查））；SET-5 增顶层 `default` 解析（null=未设置；缺字段 / 非法形状 fail-closed）与 `SettingsProvidersData` / `SettingsProvidersState.default_model` 权威默认项、`confirm_default_model`（Host Data 确认后 Composer 同步 selected_model 并清 pending，不动会话/草稿/Run）、`default_model_unavailable`（默认 provider 未连接，或目录非空且明确不含默认 model→显式失效；目录为空未成功加载时抑制提示不误报，不静默切换）与 `group_models_by_provider`（render/AX 同源分组）；SET-6a 增 `SettingsGeneralState` / `parse_general_settings`（Host `{proxy_url:string|null}` 缺字段/非字符串非 null fail-closed；查询失败/未知 `available=false`；断线 `mark_stale` 保留最后只读值；`writes_enabled` 须接通且非 stale）；SET-6c 增 Tools 页消费 `McpServersReceipt`（`apply_authoritative_servers` 落地回执 + `available` 置位 + 解除 stale，复用 ResourcesPanelState epoch/stale 机制）；SET-6b 增 `ApprovalModeSetting` 五档 / `PermissionsSettingsData` / `SettingsPermissionsState` 与 `parse_permissions_settings`（四元组缺字段（含 workspace_id）/ 未知 mode / 类型错误 fail-closed；断线 mark_stale 保留最后只读值；写回执按字段确认）；SET-6d 增 `TerminalSettingsData` / `SettingsTerminalState` 与 `parse_terminal_settings`（shell/columns/rows 缺字段或类型错误 fail-closed；断线 mark_stale 保留最后只读值；回执即写后状态；`effective_size` 为新建终端初始尺寸来源，未查询到回落 80×24）与 `apply_terminal_initial_size`（create 回执后落位初始尺寸，不置 resize_confirmed，随后那次 terminal_resize 回执才确认）；R6 Wave B 按 workspace 保存多个 terminal 与 create 失败，占位/回执确定性归并，首段 output 在 create 回执给出权威 workspace 前只缓存不上屏，UpToDate 合并非事件权威 snapshot 且历史输出不复活 exited/killed 终态；P3 增快照 cwd 解析（缺键 unknown）、瞬态 IO 失败不锁死 running 终端与 create 回执补 cwd；ADR-045 `apply_terminal_exited`（live 终态与快照同口径：exited/killed/failed + availability 降 Stale，旧输出不复活）与 `remove_terminal`（Close 清理回执后本地移除，当前终端复位 not started）；P4 片 2F 修 D3——`apply_timeline_page` 不再让历史 responded/completed 改写 snapshot 权威 pending（同 run 早期工具也不能清后续工具审批），仅历史 run 终态按 run 清除；live ToolCompleted 仍按 run + tool_call_id 精确清除；Timeline/TaskRail/Run 状态与 F-13 谓词；63 个测试 |
| `src/ui/mod.rs` | ~4110 | `AppView` 宿主：Workspace Header、Timeline、Composer 与 Inspector 三栏装配；SET-3 增顶层路由 `AppRoute`（Settings 壳与工作台互斥渲染，工作台状态保存在字段、返回即恢复；进入拉取 provider 状态并断线 mark_stale）；SET-4 增 Settings 写操作宿主字段（secure 输入实体 / Replace 编辑器 / Remove 确认 / 动作焦点）、`AuthStarted` 消费、Succeeded 后再查 provider 状态、auth_start 失败回滚乐观态，离开 Settings 清空 secure 缓冲（含 undo 栈）；SET-5 增 `DefaultModelConfirmed` 消费（Composer 同步已确认默认）、页级 Refresh 入口（重查 provider_auth_status + model_list，失败保留旧列表并显示错误）、进入 Settings 即补拉模型目录（与 Refresh 对称、断线 no-op）与 ModelsLoaded 后「设为默认」按钮焦点回收；SET-6a 增 `SettingsPage`（通用页仅在 `general_settings` 查询成功后可选）、进入/Refresh 同拉 `general_settings`、断线 `mark_stale`、`GeneralSettingsLoaded` / `ProxyUrlConfirmed` 以回执为权威生效值（迟到响应仍重标 stale）；SET-6b 增 `SettingsPage::Permissions`（查询成功后可选）、进入/Refresh/重连同拉 `permissions_settings`、断线 mark_stale、`PermissionsSettingsLoaded` / `ApprovalModeConfirmed` / `WorkspaceTrustConfirmed` 以回执为权威生效值（迟到响应仍重标 stale）；SET-6c 增 `SettingsPage::Tools`（`mcp_list` 成功后可选，可用性=resources.available）、进入/Refresh/重连同拉 `refresh_resources`、`McpServersReceipt` 以回执为权威生效值、Remove 两步确认 `settings_mcp_remove_confirm`（离开 Settings 清空）；SET-6d 增 `SettingsPage::Terminal`（`terminal_settings` 查询成功后可选）、进入/Refresh/重连同拉 `terminal_settings`（重连同批预热新建终端初始尺寸查询缓存）、断线 mark_stale、`TerminalSettingsLoaded` / `TerminalSettingsConfirmed` 以回执为权威生效值并回填输入框（迟到响应仍重标 stale）、`TerminalCreated` 回执后投影初始尺寸与当次 `terminal_resize` 改用生效 columns/rows（未查询到回落 80×24）；SET-6e 增 `SettingsPage::Appearance`（本地外观页常在，导航焦点 + 三档字号 HashMap 焦点）；SET-6f 增 `SettingsPage::Advanced`、握手摘要与导航焦点；Connecting/断线清空摘要，连接成功刷新，避免旧 Host 信息冒充当前状态；Scope/WorkspaceConfirm 的 `Add project…` 通过 GPUI 系统目录选择器调用 controller `open_workspace`，成功后切换 scope、同步 terminal 并显示项目名；其余承载 Activity、按 workspace 隔离的 terminal 草稿、尺寸草稿/键盘 stepper、exited/killed 终端重建入口、ADR-045 Stop/Close 同槽谓词 `terminal_close_label`（running→Stop、已知 exited/killed/failed→Close，视觉/键盘/AX 三路径同源；failed 只 Close 后回到 Start）与 close pending/回执消费（发出时固定 Stop/Close 意图，不受 live 终态/回执先后影响）、焦点、字号、菜单、1s tick 与 barrier settle；P4 片 2F 让 WorkspaceConfirm 关闭后按真实来源恢复 add-task / header-new-task 焦点；16 个测试 |
...
| `src/ui/platform_preferences.rs` | ~123 | macOS Accessibility Display 偏好桥：读取 `NSWorkspace.accessibilityDisplayShouldIncreaseContrast`，监听 `NSWorkspaceAccessibilityDisplayOptionsDidChangeNotification` 并刷新窗口；状态保存在原子值中，非 macOS no-op。当前 UI 无动画，故 Reduce Motion 无渲染分支；无新增依赖、无主动修改系统设置。 |
...
| `src/ui/accessibility/app.rs` | ~4640 | R4 Wave A 增 `header_ax`（标题 / branch / live 状态）与 `timeline_row_ax` 五类节点；R6 Wave A 将折叠态 `inspector-toggle` 与 ActivityPopover 语义树迁至 Header；从 `AppView` canonical UI 状态与布局 metric 构建三栏语义树；壳层几何与 render 共享 `shell_layout::resolve`，R7 Wave C 同源发布 150% 的 320px rail 与 `composer-status-hint` 百分比；R9 同步 ActivityPopover heading / open-changes 两个受影响子节点的 inset/offset 断言；P3 增 Terminal 尺寸 stepper 组（4 按钮）与 Start/New/Size 单槽语义同 gate 同 id；ADR-045 增 `terminal-close` AX 节点（与可见按钮同谓词条件发布，Stop terminal / Close terminal 标签）；P4 片 3 起三处几何与 render 同源推导——stepper 五按钮走 `terminal_stepper_ax_rects`、审批卡高度走 `approval_card_height`（随 reason/detail 行数变化）、Timeline 行/approval 组成同一 item 序列，稳定帧优先读 `ListState::bounds_for_item` 的真实 item bounds，首帧才用 `timeline_row_height` / `row_top_gap` 公式回退；P4 片 2F 起七个菜单触发器的 AXPress 先与点击同源移焦（Enter 归根节点菜单裁决而非 composer SendMessage），Timeline 跟随保留部分可见首项，脱钩时不发布视口外 approval，按钮行随卡底内缩；TaskRail 几何、连接/会话文案、稳定 identifier、Composer/Send/Cancel、press/focus/set-value 白名单均与 render/业务 gate 同源；SET-3 增 settings rail / page AX 树（与工作台互斥渲染）与 open-settings / settings-back press 白名单；SET-4 增 settings 写动作按钮与 secure 输入 AX 节点（value 恒为掩码、TextArea role、Focus/SetValue；stale 时 enabled=false 且 permits 拒绝）及 press / set-value 派发（identifier 前缀解析 + provider 清单还原，未知 id fail-closed）；SET-5 增页级 Refresh 按钮与「模型与默认项」区 AX 节点（分组模型行 value 含 Default 徽标、失效提示行、Set default 按钮与 render 同 gate）及 settings-refresh / set-default press 派发；SET-6c 增 Tools 导航项（settings-nav-tools）与页面 AX（复用 resources_ax 行形状 + 每行 Test/Remove/确认按钮同 gate，stale 时 enabled=false 且 permits 拒绝）及 `settings-mcp-` 前缀动态派发（未知名 fail-closed）；SET-6a 增 General 导航项（查询失败/未知不发布）与通用页 AX（当前值 / 明文输入 / Save / Clear / 生效边界；stale 时 enabled=false 且 permits 拒绝）及 settings-nav-general / settings-nav-providers / settings-proxy-save / settings-proxy-clear / settings-proxy-input Focus·SetValue 派发；SET-6d 增终端导航项（settings-nav-terminal，查询失败/未知不发布）与终端页 AX（shell/columns/rows TextArea Focus·SetValue 同 gate、Save/Clear Press、生效边界；尺寸解析与 render 同源 parse_terminal_dimension）；SET-6e 增常在外观导航（settings-nav-appearance）与外观页 AX（三档 Button / selected / Press，宽高间距与 render 共用常量，未知 identifier fail-closed）；SET-6f 增常在高级导航、连接诊断只读节点与同源 Reconnect Press，离线不发布旧握手；13 个测试 |
...
| `src/ui/theme.rs` | ~679 | 深色主题 token：六组 29 色 + 以 16px 根字号表达的 `Rems` 字阶（100% 与原冻结 px 逐项相等）+ 100%/125%/150% `TextScale` + `metrics` 尺寸常量；R9 新增 56px rail meta、20/72/76px Changes 文件槽、36px diff header 与 24px gutter，并以既有 metrics 测试钉死；`dark()` 默认读取平台 Increase Contrast 状态，`dark_for` 高对比分支只增强辅助文字、surface、边界与选区，默认 palette 不变；10 个定向测试 |
| `src/ui/timeline.rs` | ~580 | Timeline 容器：gpui `list()` 变高虚拟化（R4 Wave A 自 Bottom 钉底改为 Top 对齐 + 显式跟随，F-06 短会话从 Header 下开始：贴底判定走滚动事件事实——`visible_range` 覆盖末项即贴底，handler 内不触 ListState 防 gpui 写借用重入 panic；`sync_list` 跟随态 scroll_to 末项、脱钩恢复钳制偏移）；`timeline_row_element` 五类渲染行组装（`timeline_rows()` 同源），R9 统一满宽 + max_w 618 可读列，独立 RunSummary 使用 40px entry gap、tool→summary 保留 12px 组内 gap；P4 片 3 增行高 / 可见窗口公式（`timeline_row_height` 按内容 + theme metrics + rem 档推导，`row_top_gap` / `timeline_visible_item_tops` / `timeline_following_window` 与 AX 共享，首项可部分可见、完全离屏项不发布）；空态引导（无 active session 且条目数为 0 时居中 tertiary 一句，R2 Wave B）；`install_scroll_follow`（脱钩检测）；`sync_list`（统一 reset、脱钩恢复钳制偏移、Entry 菜单 close-on-reset）；`TIMELINE_OVERDRAW`=200px；回底控件接线 |
| `src/ui/timeline_entry.rs` | ~837 | R4 Wave A 重写：F-07 消息层级（You/Pawork 标签行 + 时间、18px 正文行高默认 24 且随字号 rem 缩放、段落/• 列表两级切分）、F-08 `tool_group_element`（r5 描边面板、52 行高 / 2px 分隔线、状态 ✓/词诚实映射 succeeded→Completed 其余原文、无耗时字段不画）、`run_summary_element`（Ø40 状态圆随终态：Completed 绿 ✓ / Failed danger ✕ / Cancelled —；Ready for review + Review changes 主按钮，Open in editor 无 capability 不画）、`run_footer_element`、`error_entry_element`；R9 把消息时间、tool detail、Run footer 与 Running/Pending 提升至 secondary；`display_time`（epoch millis 串 → `relative_activity` 相对词 now/Nm/Nh/Nd，非法串原样兜底，render 与 AX 同源）；条目「···」fork 菜单迁入 message/error（identifier 冻结）；R7 Wave B 的 Review changes 展开后聚焦 Changes 选中页签，`on_fork` 入口级复核并在接受后聚焦 Composer；8 个测试 |
...
| `src/ui/input_area.rs` | ~584 | Composer（R5 Wave A 两行）：行 1 TextInput 单行常态 28px、多行向上增长并按面板 220 预算 clamp；行 2 footer（model Dropdown `provider / display_name` 触发器 max_w 220 truncate、只读 workspace Label max_w truncate、ContextMeter 文本、瞬态 `status_hint` Label、flex spacer、32×32 Send/Cancel 同槽 element id `composer-action`）；R9 让两行共属既有 panel surface，并仅在稳定的 unavailable 文案上使用 tertiary、可用 Context 保持 secondary；提示行删除，placeholder 只走状态机，Forked / 发送失败等瞬态反馈落 footer Label；WorkspaceConfirm 浮层保留；reasoning / 附件 / queue 诚实不画 |
...
| `src/ui/changes.rs` | ~987 | Changes 面：Files / Summary 二级页签固定 96×56、17px 字与 2px 选中下划线；`ChangesPanelState`（双 epoch 防过期）；R9 文件清单使用 20/72/76px glyph/status/delta 槽，DiffView 增横滚外 36px 只读路径 header、24px 语义 marker gutter 与中性正文，仍按最长行建立真实横滚 extent；latest-session mismatch fail-closed 与诚实 scope；折叠态 Header ActivityPopover 保持 320×320，标题下分隔并以 raised/bordered section 承载 Changes 权威摘要，不伪造 Agent；7 个测试 |
| `src/ui/resources.rs` | ~210 | Resources 页：MCP server 只读表 + `ResourcesPanelState`（epoch 防过期）+ 手动刷新；SET-6c 回执 `apply_authoritative_servers` bump epoch；2 个测试 |
| `src/ui/settings.rs` | ~2950 | SET-3 Settings 壳：Settings Rail（「← Back to workspace」+ 首页导航项「Models & providers」，未接通页面不显示）与全宽供应商页；SET-4 增认证写操作（全部 descriptor 驱动，禁按 provider 名分支）：`settings_auth_actions` 按 `auth_methods` + `auth.type` 推导动作——none/error 态 API key 内联 secure 输入 + Verify/Cancel、OAuth Connect；connecting 态显示等待详情（URL / user code / 到期）+ Cancel（api_key 验证中无取消）；connected 态每 method Replace + Remove（确认组）。`on_settings_action` 统一入口（`settings_action_enabled` + descriptor 复核，未知 provider fail-closed）；`settings_writes_enabled` 断线 / stale 一律禁写，空输入 Verify 同源禁用（可见 / 键盘 / AX / 入口四路径共用）；提交后清空输入缓冲含 undo 栈，明文只进 controller 调用栈；SET-5 增「模型与默认项」区（供应商列表下方）：按 provider 分组列出 `projection.models` 可运行模型（display_name · id），默认行带 Default 徽标，每行「Set default」按钮（provider 已连接、非 stale/断线、非当前默认才可用；identifier 为 `set-default-<provider>:<model>` 转义，派发按 projection.models 还原，未知 fail-closed），默认失效显式提示行（`SETTINGS_DEFAULT_UNAVAILABLE_NOTE`，render/AX 同源）与页级 Refresh 按钮（连接态 gate）；SET-6c 增「工具与 MCP」页（导航 `SettingsPage::Tools`，`mcp_list` 成功后显示）：清单行复用 Resources 渲染（name/state + transport·tools·last_error），每行 Test（发 `mcp_test`）与 Remove（两步确认后发 `mcp_server_remove`），回执即权威生效值（`apply_authoritative_servers`），`settings_tools_writes_enabled` = 连接 + available + 非 stale 三路径同 gate，生效边界文案如实标注同会话生效与进行中 Run 不回溯撤销；SET-6a 增「General」导航项（capability 与供应商页同源：`general_settings` 查询失败/未知则隐藏且不渲染写入口）与通用页：显示 Host 权威 `proxy_url`（null 文案「未设置（跟随系统环境变量）」）、内联明文输入 + Save/Clear（空 trim 禁 Save、已是 null 禁 Clear）、错误行、生效边界文案（新 OAuth/验证/目录探测同会话生效；当前活跃供应商的模型流量于切换供应商或重启 Host 后生效）；断线 stale 只读禁写，可见 / 键盘 / AX 同 `settings_general_writes_enabled`；SET-6d 增「终端」导航项（`terminal_settings` 查询失败/未知则隐藏且不渲染写入口）与终端页：显示 Host 权威生效值（shell null 文案「未设置（跟随平台默认）」+ columns/rows）、shell 输入 + columns/rows 数值输入（parse_terminal_dimension：u16 ∈ 2..=1000 与 Host 校验一致，畸形/越界禁 Save）+ Save（全态回传三字段，空 shell 映射为 null 以便只改尺寸）/ Clear（清除 shell，columns/rows 回传权威生效值）、生效边界文案（只影响之后创建的终端，已存在终端不变）；断线 stale 只读禁写，可见 / 键盘 / AX 同 `settings_terminal_writes_enabled`；SET-6e 增始终可用的本地「外观」页（不经 Host）：100%/125%/150% 复用唯一 `TextScale` / `set_text_scale`，当前档文字+Primary+AX selected 同步，按钮 render/AX 共用 112×28px 与 8px 间距，长说明按内容列换行；主题与 Increase Contrast 只读说明，字号仅当前 Desktop 会话生效、重启恢复 100%；SET-6f 增始终可用的本地「高级」页：连接状态、Host runtime ID、协商 API/capabilities、启动 endpoint、resume/ack 与断线 Reconnect；runtime ID 不冒充配置 instance，token/token path 不显示，原样 endpoint 不用于反推 data directory/配置 instance |
| `src/ui/task_rail.rs` | ~995 | Sessions 侧栏（F-03/F-04 定稿，R3 Wave A；R3 Wave B 增键盘导航与状态扩展）：顶部三行——标题行「Pawork」22px + ghost grouping 角标 28×28、全宽 raised scope 行（h36 / 1px 描边 / r4 / 18px 左对齐）、连接行（Ø10 状态点 + `connection_status_label` 17px + 28×28 全局「+」；R7 Wave C 将文案槽定为 rail 内容宽 − 全局「+」− 共享 8px 间隔，并在槽内 `overflow_hidden + truncate`，窄窗长断线原因显示省略号且不越过合同间隙，AX 仍提供完整状态值）；grouping / scope 菜单（MenuRow 键盘高亮）、日期桶头（18 medium secondary）与项目块（chevron + 名称 + 56px 右对齐计数槽 + 28×28 定向「+」）折叠、44px 任务行（状态点 + 标题 `.truncate()` + 56px 相对时间槽，active 标题 medium/primary；150%/320px rail 下不相撞；选中 raised + r4；Blocked=danger 红实心、unread 标题 SEMIBOLD 同字号不改几何）、Reconnect 按钮（仅 Disconnected / ConnectFailed，Connecting 不显示，R2 Wave B）、连接状态徽标、「Local」页脚（TR-12：不画头像 / 姓名 / quota；SET-3 起右侧 gear 为真实 Settings 入口，可见 / 键盘 / AX 同 gate）；桶头 / 项目头 / 任务行平铺为滚动容器直接子元素（`ScrollHandle::scroll_to_item` 拿行级 bounds，grouping/scope 切换后把 active task 滚到可见，折叠项目退回头部行）；rail 内键盘导航 `handle_rail_navigation_key`（↑/↓ 沿焦点链移焦 clamp 不 wrap，项目头 ←/→ 收起/展开已处目标态 no-op，Enter/Space 经 ListRow 行级 key_down 直接调激活 handler（Slice 4）；Slice 5 菜单打开时 rail 让位不接管；rail 聚焦 Button——grouping/scope/add-task/项目定向「+」——裸 Enter/Space 行级激活同 click 路径，菜单已开时让位给根节点菜单 Enter 接管），带修饰键不接管 |
...
> **SET-6e 模块增量（2026-09-03）**：`ui/mod.rs` 增 `SettingsPage::Appearance`、常在导航焦点与三档字号焦点状态；`ui/settings.rs` 增不经 Host 的外观页，100%/125%/150% 控件复用唯一 `TextScale` / `set_text_scale`，当前档以文字和视觉样式标记，长说明按内容列换行；`ui/accessibility/app.rs` 同步常在导航、三档 Button/selected 语义和 Press 派发，按钮宽高/间距与 render 共用常量，未知 identifier fail-closed。未改 controller、projection、protocol、配置或生产依赖。
>
...
- 行级键盘激活（design §3.6）：聚焦的 ListRow / Button 上裸 Enter / Space 在行级 `on_key_down` 直接调用与鼠标 click 同一激活 handler（task 行 `on_session_clicked`、项目头 `on_toggle_project`、grouping/scope/add-task/项目定向「+」按钮开菜单 / 新建），不走合成 click 兜底。物理键盘下该合成 click 仍会到达：`pending_row_key_activate`（行）/ `pending_button_key_activate`（按钮）衔接标记吞除；行键 / 按钮 id 不匹配同样吞除，鼠标真实 click 永不吞。带修饰键不接管；菜单打开时行级 / 按钮级让位，Enter 由根节点菜单接管。
...
- **Settings 默认模型（SET-5）**：默认项只在 Host `set_default_model` Data 确认后更新 Composer（`selected_model` 同步、清 pending），随后重查 `provider_auth_status` 落地权威 `default`；失败走 OperationFailed 不落地乐观状态。默认失效（provider 未连接，或目录非空且明确不含该 model）显式提示；目录为空（未成功加载）不判定不误报，不静默切换；「设为默认」要求 provider 已连接、非 stale、非当前默认（四路径同 gate）。刷新失败保留旧列表与默认项。
- **Settings 工具与 MCP 页（SET-6c / ADR-049）**：导航「工具与 MCP」仅在 Host `mcp_list` 至少成功一次后显示。清单复用 mcp_list（name/transport/state/tools/last_error）；每行 Test 发 `mcp_test{name}`、Remove 两步确认后发 `mcp_server_remove{name}`，回执 `{servers:[...]}` 即权威生效值（bump epoch，不另重查）；Error 保留旧清单并在本页显示失败文案（render/AX 同源 `action_error`，不再只进工作台 status_hint）；未知/畸形 fail-closed；断线 stale 只读禁写（render/键盘/AX 同 gate）。生效边界文案：remove 同会话生效（盘/密/内存三处一致），进行中 Run 已快照工具不回溯撤销，重启后与盘一致。
- **Settings 通用页（SET-6a / ADR-047）**：导航「General」仅在 Host `general_settings` 至少成功解析一次后显示（失败/未知隐藏且不渲染写入口）。页面显示 Host 权威 `proxy_url`；null 文案「未设置（跟随系统环境变量）」；Save/Clear 等回执 `{proxy_url}` 才改生效值，不另重查。断线 stale 保留最后只读结果并禁写（render / 键盘 / AX 同 gate）。生效边界文案不得宣称全局即时生效：新 OAuth/验证/目录探测同会话生效；当前活跃供应商的模型流量于切换供应商或重启 Host 后生效。proxy URL 非 Secret，输入明文。畸形载荷 fail-closed，不把残缺帧当成未设置。
- **禁动符号**（R8 冻结面，bin 内测试钉住内容）：`APP_VIEW_KEYBINDINGS`、`install_keybindings`、`MAIN_PATH_TAB_STOP_IDS`、`resolve_new_task_workspace`。
- **Settings 权限与审批页（SET-6b / ADR-048）**：导航「权限与审批」仅在 Host `permissions_settings` 至少成功解析一次后显示（失败/未知隐藏且不渲染写入口）。五档审批模式显式选择：当前值高亮只读，其余档位「选择」发 `set_approval_mode`，等 Data 回执才改生效值（回执即写后状态，不乐观更新）；会话信任开关发 `workspace_trust`（workspace_id 取 Host 查询透出的 attached id，缺 id 禁用，匹配校验在 Host fail-closed）；`trust_workspaces_global` 只读行（null 文案「未设置（默认不信任）」，本片不写 Global）；生效边界文案如实声明：仅当前会话生效、不持久化、重启 Host 回默认、进行中 Run 不受影响、之后启动的 Run 生效。未知 mode / 畸形载荷 fail-closed 不落地；断线 stale 保留最后只读结果并禁写（render / 键盘 / AX 同 gate）。
- **Settings 终端页（SET-6d / ADR-050）**：导航「终端」仅在 Host `terminal_settings` 至少成功解析一次后显示（失败/未知隐藏且不渲染写入口）。页面显示 Host 权威生效值：shell 持久值（null 文案「未设置（跟随平台默认）」）与 columns/rows 生效值；shell 文本输入 + columns/rows 数值输入（u16 ∈ 2..=1000，与 Host 校验一致，畸形/越界禁 Save）+ Save（`set_terminal_settings` 全态回传 shell/columns/rows 三字段，空 shell 映射为 null 以便只改尺寸）+ Clear（清除 shell=null，columns/rows 回传权威生效值）；回执即写后完整状态并回填输入框，不乐观更新。新建终端初始投影尺寸与创建后那次 `terminal_resize` 取生效 columns/rows（查询缓存；未查询到回落 80×24）。生效边界文案如实声明：只影响之后创建的终端，已存在终端不变。畸形载荷 fail-closed 不落地；断线 stale 保留最后只读结果并禁写，重连自动刷新（render / 键盘 / AX 同 gate，含 settings-nav-terminal AX Press 与焦点句柄）。
...
| `controller.rs` | 14 | 既有 wire/解析/安全回归；R6 Wave B 增 terminal create 失败 workspace 归属与 diff 内容 session id 生命周期字段。SET-6d 增 terminal_settings 查询/全态写 wire 与回执解析主路径一条（shell Some/null 两态）。 |
| `projection.rs` | 63 | 既有 snapshot/replay、Run/Timeline/TaskRail 与 Terminal 投影；R6 Wave B 增多 workspace terminal 选择、output-before-create 缓存、权威 workspace 回执前不上屏、create 失败占位回收、UpToDate snapshot 合并，以及 exited terminal 不被历史输出复活；P3 增快照 cwd/unknown 回退与瞬态 IO 失败不锁死 running 终端；ADR-045 增 live `TerminalExited` 即时降级且迟到输出不复活、Close 回执 `remove_terminal` 复位当前终端；P4 片 2F 增跨 run 与同 run 两条历史分页回归，钉住早期 responded/completed 不得清除 snapshot 中当前未决议审批（D3）；SET-3 增 provider_auth_status 主路径解析与畸形载荷 fail-closed 各一条；SET-4 增 AuthChanged 六态解析与应用主路径（含 Succeeded 再查询置位与 OAuth 等待清除）和畸形载荷 fail-closed 各一条；复审增 Connected 基线 Replace 终态不清旧凭证并触发重查一条；SET-5 增 default 主路径与畸形 fail-closed、确认后投影同步、失效默认标志（含目录为空时抑制误报）、刷新失败保留旧列表与默认项共 5 条；SET-6a 增 general_settings 解析主路径、畸形载荷 fail-closed、stale 保留最后值并禁写各一条；SET-6b 增 permissions_settings 四元组解析主路径（含五档 wire 串往返与回执确认）、畸形载荷 fail-closed、stale 保留最后值并禁写共三条。SET-6d 增 terminal_settings 主路径（解析应用/全态写确认串联/新建终端初始尺寸取生效值）与畸形载荷 fail-closed各一条。 |
...
| `ui/settings.rs` | 1 | SET-6d：空 shell Save 映射为 null、尺寸合法才可保存。 |
| `ui/mod.rs` | 16 | 既有键位、tab_stop、TaskRail 导航、Composer 与 per-session 草稿接线；R6 Wave A Header Activity；R6 Wave B Inspector 键盘目标、terminal gate 与 workspace 草稿/回执归属；R7 Wave C 钉字号放大/缩小/重置键位登记；P3 增已知 exited 终端 Start 重建 gate |
| `ui/accessibility.rs` | 3 | identifier 唯一与父子关系校验；focus 单一性；bounds hit-test 与无效树拒绝 |
| `ui/accessibility/app.rs` | 13 | 稳定 identifier、TaskRail/Timeline/审批/菜单焦点与几何同源回归；SET-4 secure 输入掩码与 stale gate；SET-6e/6f 共用一条本地 Settings 页 GPUI 回归：高级页覆盖离线导航、失败态 endpoint/unavailable 握手、连接后 runtime/API/capabilities/resume/last-ack 行、已连接态 Reconnect 最终 gate，以及 Disconnected 清空握手并恢复 Reconnect；随后继续覆盖外观页 AX Press 150%、根字号/selected/共用几何与未知 175% fail-closed。 |
...
| `ui/theme.rs` | 10 | 既有 WCAG、TaskRail、Header/Timeline 与 Composer token；R7 Wave C 增 rem 在 100% 与冻结 px 逐项相等、TextScale 档位和 Increase Contrast palette 强化断言；R9 增 rail/Changes/Diff 固定槽 metric 断言 |
...
**运行时验证资产**：`--probe`（连接 + snapshot + 模型目录一行摘要）与 `--probe-smoke`（流式回合 / 切模型 / 审批 / 取消 / 两次断线重连持久化 / `disconnect_survive`），配合隔离实例（`--instance` + `PAWORK_DATA_DIR`）在真实 host 上冒烟。[scripts/ui-ax-dump.swift](../../../scripts/ui-ax-dump.swift) 可做真窗口 AX tree / action 取证；端到端流程见 [../flows.md](../flows.md)，验证总策略见 [../README.md](../README.md)。
...
- **单主题**：仅深色 `dark()`；Increase Contrast 是同一深色主题的可访问 palette 变体，不是第二套主题。SET-6e 外观页只读陈述这一事实，不提供 light/system/custom theme 控件；`Theme: Global` 是未来运行时主题挂载点，当前未 `set_global`。
- **文件尺寸口径**：`ui/mod.rs` 约 4087 行；这是工程结构口径，不构成新 UI 视觉或交互放行条件。
