...
| `src/controller.rs` | ~2300 | `DesktopController`（connect / 事件泵 / 空闲心跳 / 全部 Command·Query 构造与响应解析）；`open_workspace` 复用冻结 `workspace_add`，成功后刷新 snapshot 并发 `WorkspaceOpened{id,name}`；SET-3 增 `provider_auth_status` 查询（provider_id=None 全量）与 `ProviderStatusLoaded` 事件；SET-4 增认证写命令 `auth_start` / `auth_set_api_key`（ApiKeySecret 只在调用栈上构造，明文不进 projection/日志）/ `auth_cancel` / `auth_remove` 与 `AuthStarted` 回执（verification_url / user_code / expires_at）；SET-5 增 `set_default_model` 命令构造与 Data 回执解析（确认 pair 经 `DefaultModelConfirmed` 投递后自动重查 `provider_auth_status` 取回权威 default；Error / 传输失败走 OperationFailed，不动 UI 现有状态）；`ControllerEvent` 对 terminal create 失败携带 workspace、对 diff 内容携带 Host 实际 session id；ADR-045 `terminal_close` 命令构造与 `TerminalCloseSucceeded/Failed` 回执（成功不直接改 projection：running 终态由 live `TerminalExited` 刷新，exited 清理由 UI 回执后本地移除）；12 个测试 |
...
173 个测试全部内嵌于 bin target（`#[cfg(test)]` 模块；无 crate `tests/` 目录），按文件分布：
...
- `--features gpui/runtime_shaders`：gpui 默认构建在编译期调用 Metal 着色器编译器；开发机仅有 Xcode CLT 时缺 Metal Toolchain 会构建失败，runtime_shaders 把着色器编译推迟到运行时使本机可闭环。当前 Desktop 定向门禁为 173/173（SET-5 起）。
