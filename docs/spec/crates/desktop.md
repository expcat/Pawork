...
30 个 `.rs` 文件、约 21.6k 行，全部在 `[[bin]] pawork-desktop` target 内（无 lib target、无 crate `tests/` 目录）。
...
| `src/controller.rs` | ~1900 | `DesktopController`（connect / 事件泵 / 空闲心跳 / 全部 Command·Query 构造与响应解析）；`open_workspace` 复用冻结 `workspace_add`，成功后刷新 snapshot 并发 `WorkspaceOpened{id,name}`；`ControllerEvent` 对 terminal create 失败携带 workspace、对 diff 内容携带 Host 实际 session id；ADR-045 `terminal_close` 命令构造与 `TerminalCloseSucceeded/Failed` 回执（成功不直接改 projection：running 终态由 live `TerminalExited` 刷新，exited 清理由 UI 回执后本地移除）；12 个测试 |
...
| `src/projection.rs` | ~3900 | `DesktopProjection` 渲染适配投影；`ConnectionState` / `ResumeState` / `ResumeApply` / `TerminalState` / `PendingApproval` / `ModelEntry` / `ActiveRun` / `SessionSummary` / `WorkspaceSummary` / `SessionLiveStatus`；R6 Wave B 按 workspace 保存多个 terminal 与 create 失败，占位/回执确定性归并，首段 output 在 create 回执给出权威 workspace 前只缓存不上屏，UpToDate 合并非事件权威 snapshot 且历史输出不复活 exited/killed 终态；P3 增快照 cwd 解析（缺键 unknown）、瞬态 IO 失败不锁死 running 终端与 create 回执补 cwd；ADR-045 `apply_terminal_exited`（live 终态与快照同口径：exited/killed/failed + availability 降 Stale，旧输出不复活）与 `remove_terminal`（Close 清理回执后本地移除，当前终端复位 not started）；Timeline/TaskRail/Run 状态与 F-13 谓词；43 个测试 |
| `src/ui/mod.rs` | ~3510 | `AppView` 宿主：Workspace Header、Timeline、Composer 与 Inspector 三栏装配；Scope/WorkspaceConfirm 的 `Add project…` 通过 GPUI 系统目录选择器调用 controller `open_workspace`，成功后切换 scope、同步 terminal 并显示项目名；其余承载 Activity、按 workspace 隔离的 terminal 草稿、尺寸草稿/键盘 stepper、exited 终端重建入口、ADR-045 Stop/Close 同槽谓词 `terminal_close_label`（running→Stop、已知 exited/killed→Close，视觉/键盘/AX 三路径同源）与 close pending/回执消费、焦点、字号、菜单、1s tick 与 barrier settle；16 个测试 |
...
| `src/ui/timeline.rs` | ~436 | Timeline 容器：gpui `list()` 变高虚拟化（R4 Wave A 自 Bottom 钉底改为 Top 对齐 + 显式跟随，F-06 短会话从 Header 下开始：贴底判定走滚动事件事实——`visible_range` 覆盖末项即贴底，handler 内不触 ListState 防 gpui 写借用重入 panic；`sync_list` 跟随态 scroll_to 末项、脱钩恢复钳制偏移）；`timeline_row_element` 五类渲染行组装（`timeline_rows()` 同源），R9 统一满宽 + max_w 618 可读列，独立 RunSummary 使用 40px entry gap、tool→summary 保留 12px 组内 gap；空态引导（无 active session 且条目数为 0 时居中 tertiary 一句，R2 Wave B）；`install_scroll_follow`（脱钩检测）；`sync_list`（统一 reset、脱钩恢复钳制偏移、Entry 菜单 close-on-reset）；`TIMELINE_OVERDRAW`=200px；回底控件接线 |
...
| `src/ui/inspector.rs` | ~670 | Inspector 面板：顶层 Changes / Terminal / Resources 三页签；Terminal 页含 cwd/尺寸 stepper 组（G1：本地草稿 ±步进、apply 走冻结 `terminal_resize`）、FollowScroll 输出、输入与 Start/New/Size 单槽（G2：已知 exited/killed 终端 Start 变「新建终端」入口）、ADR-045 Stop/Close 同槽按钮（running→Stop 真实 `terminal_close` 终止，已知 exited/killed→Close 清理 Host tombstone，在途禁用防连点）；`plain_terminal_output` 在可见文本与 AX 共用路径移除 ANSI/VT 控制序列并归一换行（纯文本视图，不冒充 VT emulator）；`ensure_terminal` 懒创建与 exited 重建共用 `begin_terminal_create`；3 个测试 |
...
- **Inspector（右栏，cmd-i 开合）**：顶层三页签，默认 Changes；顶层条 58px / 每项 100px / 18px 字，Changes 内二级条 56px / 每项 96px / 17px 字，选中态均为底部 2px accent 下划线；各页滚动状态独立保留。
  ...
  - Terminal：host 流式 `TerminalOutput` 滚动文本（非 VT100、无本地 PTY）+ cwd 与尺寸组（`−W +W [列×行] −H +H`：stepper 只改本地草稿并钳制在 20–500 列 / 6–200 行，可见值与 AX value 同源；尺寸按钮把草稿经 `terminal_resize` 下发，在途时禁重复提交，仅匹配当前终端与当前草稿的回执才清草稿，终端切换也复位；缺/空 cwd 事实显示 `unknown`，G3）+ 终端输入（Enter 写入，未启动时先懒创建；write/resize 瞬态失败在终端仍 running 时不锁死，仅 status_hint 报错，G2）+ Start / New / Size 单槽（未创建 Start；可操作 Size；已知 exited/killed 变 New——同 workspace/cwd 新建终端，旧终端只读保留；create/resize 在途时同 gate 禁用，G2）+ 脱钩回底控件。
...
- **终端约束**：`terminal_create` 的 cwd 只接受 workspace 相对路径（拒绝绝对路径、Windows 盘符前缀、`..` 分量）；终端面为滚动纯文本，无本地 PTY/VT emulator。显示层与 AX 同源过滤 CSI/OSC 等 ANSI/VT 控制序列并归一 CR 换行，不修改 Host 保存的原始 output。输入草稿和 create 失败按 workspace 隔离；冻结 wire 无 stop/close 命令与 live exit/failure 事件，因此不显示伪 Stop，也不宣称完整终端生命周期。尺寸变更只经冻结 `terminal_resize`（stepper 本地草稿、apply 下发、在途去重、匹配回执/切换复位）；已知 exited/killed 终端经 Start/New 走 `terminal_create` 重建（沿用 workspace 与可证 cwd，cwd 未知时回落工作区根），旧终端只读保留；write/resize 瞬态失败在 runtime_state==running 时保留可写、仅 status_hint 报错；快照 cwd 为 Host 权威，缺/空键诚实显示 `unknown`。
...
153 个测试全部内嵌于 bin target（`#[cfg(test)]` 模块；无 crate `tests/` 目录），按文件分布：
...
| `projection.rs` | 43 | 既有 snapshot/replay、Run/Timeline/TaskRail 与 Terminal 投影；R6 Wave B 增多 workspace terminal 选择、output-before-create 缓存、权威 workspace 回执前不上屏、create 失败占位回收、UpToDate snapshot 合并，以及 exited terminal 不被历史输出复活；P3 增快照 cwd/unknown 回退与瞬态 IO 失败不锁死 running 终端；ADR-045 增 live `TerminalExited` 即时降级且迟到输出不复活、Close 回执 `remove_terminal` 复位当前终端。 |
...
| `ui/accessibility/app.rs` | 6 | 动态 identifier 的转义稳定且无 escape-marker 碰撞；项目 identifier 的日期桶限定与 Projects 模式稳定；Timeline 摘要截尾保持 UTF-8 边界；会话行 AX description 携带状态词（Running / Needs input / Blocked 同源 + 「· Unread」unread 语义，R3 Wave A/B）；Composer AX 面板公式去掉 +68 漂移；Header Activity 触发器、Popover 与 R9 heading/open-changes 子节点锚点公式 |
...
- `--features gpui/runtime_shaders`：gpui 默认构建在编译期调用 Metal 着色器编译器；开发机仅有 Xcode CLT 时缺 Metal Toolchain 会构建失败，runtime_shaders 把着色器编译推迟到运行时使本机可闭环。当前 Desktop 定向门禁为 153/153（ADR-045 起）。
...
截至 2026-09-01，正式 Host/Desktop（无 fixture/seed/mock）已完成添加项目、真实 Provider 对话、审批写文件、Changes 与真实 Git 对照、PTY 命令、P1 多项目/会话归属双粒度重开、P2 六链路可靠性与 P3 三面板验收（含 ADR-045 Terminal 生命周期）。P3 隔离实例以 AX + `stty`/`pwd`/Git/SQLite/Host snapshot/ps 双证据验证尺寸变更、exited 重建、cwd 恢复、Changes/Resources 刷新与断线 stale；ADR-045 复验 Stop→无重连即时 killed（进程组击杀证据）、Close→复位 not started 且快照清空、`exit 7` 即时 exited、断线 stale 不回归；当前 Desktop 定向门禁 153/153。完整视觉/Accessibility 签字仍属 P4。
