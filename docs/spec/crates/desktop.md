...
| 路径 | 行数 | 承载内容 |
| --- | --- | --- |
| `src/controller/files.rs` | — | 项目目录 / 文本读取 / 显式保存；GUI 1.19 gate，回执带 workspace、路径和请求代次，正文不进日志 |
| `src/ui/files.rs` | — | Files 面板：右侧可收起的懒加载目录树（逐层展开、已加载范围筛选、刷新）、共享 PanelTab 文件标签与关闭确认、Markdown 预览 / 源码、等宽编辑、逐项目草稿、保存回执 / 冲突、重新载入确认、窗口关闭提醒与实测 AX |
| `src/controller/browser.rs` | — | 活动 Run 领取 Browser 请求，经主线程操作后返回真实回执 |
| `src/ui/browser.rs` | ~620 | 任务浏览器面板：地址输入、按钮与状态、原生视图裁剪 / 焦点桥接、session 切换保留、轮询和 AX |
| `src/main.rs` | ~670 | 入口与手动 argv 解析（非 clap）；GUI3-05 起 `Application::new().with_assets(ui::Assets)` 挂载 `include_bytes!` SVG 图标资产；`PAWORK_UI_BARRIER_DIR` env 读取（空值视同未设置，None 全程零开销）；`run_app`（1440×1024 居中窗口 + `WINDOW_MIN_SIZE` 1080×720 最小尺寸——R2 Wave B 设计响应式底线，再窄击穿 Workspace ≥560 合同——+ 沉浸式 titlebar：TitlebarOptions appears_transparent，traffic lights 悬浮深色壳、内容视口贯通全窗，R2 Wave A；`install_keybindings`、聚焦 Composer、安装 macOS AX bridge）；`run_probe` / `run_probe_smoke` 无窗冒烟模式及其 `wait_for_*` 事件等待器；1 个测试 |
...
| `src/controller/settings.rs` | ~920 | Settings 查询与写（provider_auth_status、auth_*、general/permissions/terminal、mcp_test/remove、set_provider_use_proxy；ADR-055 增 set_model_enabled / set_provider_models_enabled / set_default_role_model（conversation 角色复用既有 default 键，不再经独立 set_default_model），回执 `cleared_roles` 投影为一次性提示文案）；ADR-061 `auth_account_rename` 仅 minor ≥17 派出，成功后重查 provider 状态；断线不派出 |
...
| `src/ui/shell_layout.rs` | ~380 | R2 Wave A 壳层几何合同：`resolve`（唯一计算入口，render 与 AX 树共享）——宽窗 rail=288 / Inspector 打开时 440（OPT-4b 起 AppView 初始偏好为折叠，resolve 合同不变），窗口宽 ≤1279 时 rail=240；GUI2-05 起输出 `InspectorPlacement { Hidden, Side, Center }`（`can_fit_side_inspector` 不满足且偏好打开 → Center 中央呈现，`inspector_open` 仅表示 Side；`InspectorMotion::snap` 跨窄窗不动画）（Workspace ≥560）；R7 Wave C 在 150% 字号下改用 320px rail，宽度不足 1320 时保持 Inspector 折叠，1080 窗口保留 760px Workspace；固定侧栏 `flex_none`，防长文本 min-content 挤窄 Inspector；rail 顶部 36px traffic-light 安全区；4 个 GPUI 布局测试 |
| `src/ui/accessibility.rs` | ~410 | 平台无关 `AxTree` / `AxNode` / role / action / request / rect 模型；声明 Settings AX 子模块；3 个测试 |
| `src/ui/accessibility/app.rs` | ~8570 | 工作台三栏语义树与 Press 白名单（含 Settings identifier 派发、终端标签 / 新建标签）；P0-2 的 `task-rail-grouping` 发布目标动作 name + 当前视图 value，Press 直接切换，不再发布 expanded/menu child；P0-3 空态发布 title / description / 单一 New task action，disabled 时不发布 Press；P1 的 tool group / Review 与 Activity 几何、P2 的 credential summary 隐藏和 Settings 写 gate 与 render 同源；UX-05 model 菜单使用实际布局 / 内部列表滚动框裁剪 AX，打开定位当前项，搜索与键盘高亮共用筛选结果；Settings 页树拆到 `settings_*.rs`；OPT-3 增 Manage models 弹层（触发器 / 单模型 Switch / 全开全关 / 头行常驻 Refresh / 空目录）与四默认角色触发器·菜单的 identifier 派发；ADR-056 增展开 chevron（Press、value Collapsed/Expanded）与展开区 Credentials 凭证行 / Usage 空态节点；UI-2 会话行动作与项目头计数 AX 几何跟 render 同源；GUI2-01 增首页 / 宽窄字号实测布局回归；21 个测试 |
...
| `src/ui/accessibility/settings_providers.rs` | ~850 | Provider 64px 概览 AX；连接、目录、详情分层，普通 group value 不含 credential、endpoint、错误或 raw model id；列几何经 `settings_content_ax_width` 与 render 全宽内容列（`SETTINGS_CONTENT_PAD` 32×2，OPT-4c）同源（auth-methods 并入 name value 后 connection/catalog 平移至 +300/+440）；enabled 动作才发布 Press；OPT-3 增「Default models」四角色区（触发器 value 为当前对或「未设置」，候选 = 已连接 provider 的已启用模型，vision/search 标注只保存不接路由）与 Manage models 弹层 AX（Switch 状态词 On/Off、Enable all/Disable all、Refresh 常驻头行、空目录 fail-closed 文案） |
...
| `src/ui/input_area.rs` | ~1230 | UI-4 Composer：880px 居中列，两侧至少 28px、顶部 16px、底部 24px；输入卡片 16px 内边距、16px 圆角、轻阴影、聚焦 accent 描边、12px 输入 / 动作间距、110–220px 高度预算。模型 Raised chip 与 Send/Cancel 同为 36px 命中区，模型触发器随选中名称收缩（上限 220px，长名截断），菜单向上展开：已连接且有启用模型的供应商组头与模型同行展开（未连接 / 0 启用整组隐藏）。卡片外显示只读项目 chip 与 ContextMeter，UX-03 将无项目文件工具提示与项目新建入口并入元信息行，窄窗随宽度换行，status_hint 与字号反馈改落 StatusBar 右栏；render / AX 共用实测布局与说明。per-session 草稿、IME、发送 / 取消与空模型 gate 沿用；不新增附件或队列能力。6 个定向测试；实际 GPUI 布局与 AX 操作区对照位于 accessibility/app.rs |
| `src/ui/inspector.rs` | ~970 | Inspector 面板：48px 面板头统一展示工具、文件与当前项目 PTY 标签（`panel_tabs` 同源渲染 / AX、就地关闭、横向滚动）；`inspector-panel` / `inspector-panel-menu` 改为「+」打开工具菜单，候选 Changes / 新建 Terminal / Resources；无标签默认 Home 工具入口，`inspector-collapse` 收起面板；Side / Center 两种放置复用同一 `inspector_element`（Center 用 `Panel::fill()`、顶部 `inspector-back` 返回条、审批提示 `inspector-approval-hint`）；OPT-4a 起面板内 `inspector-collapse` 折叠按钮为 36×36 命中区 + 20px 字形；Terminal 页复用顶层同项目多 PTY 标签（切换 / 菜单新建）、cwd/尺寸 stepper 组（G1：本地草稿 ±步进、apply 走冻结 `terminal_resize`；面板输出面按像素自动 fit 列×行；P4 片 3 五按钮冻结 28/28/72/28/28×28 槽位，头部 px_2/py_1/gap_1 以共享 rem 常量表达并与 AX `terminal_stepper_ax_rects` / `terminal_chrome_top` 同源）、FollowScroll 等宽着色输出，输入落在输出面底部（无独立底栏）、Start/New/Size 单槽（G2：已知 exited/killed 终端 Start 变「新建终端」入口）、ADR-045 Stop/Close 同槽按钮（running→Stop 真实 `terminal_close` 终止，已知 exited/killed/failed→Close 清理 Host tombstone；failed 不直接 New，在途禁用防连点）；可见文本走 `terminal_view::render_terminal_lines`，AX 共用 `plain_terminal_output`（CR 覆盖 / 退格 / SGR，不冒充完整 VT emulator）；`ensure_terminal` 懒创建与 exited/killed 重建共用 `begin_terminal_create`；3 个测试 |
| `src/ui/terminal_view.rs` | ~125 | 终端显示的 gpui 胶水层：[pawork-terminal](terminal.md) 属性分段 → 主题色 StyledText runs、Keystroke → KeyEvent 转换、resize 防抖常量；行缓冲解析 / 按键映射 / 面板像素→列×行估算在 `crates/terminal`；1 个测试 |
| `src/ui/changes.rs` | ~1270 | Changes 面：Files / Summary、只读 DiffView、latest-session mismatch fail-closed 与诚实 scope；empty / unavailable / stale 使用分层占位；GUI3-08 加载态保留标题并加静态骨架。GUI2-05 / R-04：`parse_hunk_header` / `diff_hunk_line_numbers` 从 hunk 头本地递推双列行号（不改 wire，畸形头空行号）；整行 `diff.*` 浅底 token；32px 紧凑文件行 `changes_status_chip`（M / A / D 色标 + 路径 + `+A/−D`）。`has_reviewable_files_for` 要求 active session、Ready、非 stale 且文件非空，供 Run summary render/AX 同源门控。折叠态 Header ActivityPopover 为 320px 内容宽，内容高随字号为 144/180/216px，按当前唯一 Changes 内容收缩，不为未实现的 Agent 状态留空；7 个测试 |
| `src/ui/resources.rs` | ~370 | Resources 页：MCP server 只读表 + `ResourcesPanelState`（epoch 防过期）+ 手动刷新；empty / unavailable / error / stale 分层；GUI3-08 加载态加静态骨架；SET-6c 权威回执 bump epoch；3 个测试 |
| `src/ui/settings/mod.rs` | ~1500 | English / 中文 Settings 壳与稳定八页导航（GUI2-06：导航按「模型 / 工作台 / 系统」三组加 12px 组头 `settings-nav-group-*`，组内全隐则组头隐；查询非空时导航被查找结果替换；`settings_element` 支持定位行标识）；OPT-4c 起内容列用满 Rail 外可用宽度（`SETTINGS_CONTENT_PAD` 32×2，不再保留 820px 上限），共享脚手架单列 + 滚动层，受限高度内纵向滚动、切页归零；OPT-4d 导航选中/未选中共用同一外壳几何（UI-5：中性背景 + 字重；焦点描边经 focus_ring 覆盖层，零布局参与），文字坐标零位移（F4）；共享状态 / gate、descriptor 驱动认证动作与 Connect API key 编辑入口；4 个测试（空 shell Save 映射 null；OPT-3 角色区/弹层渲染 gate；角色控件 identifier 构造/解析互逆） |
| `src/ui/settings/search.rs` | ~850 | GUI2-06：静态目录 `settings_search_entries()`（page / row_id / 中英标题 / 说明 / 别名）与 `filter_settings_search_entries`（忽略英文大小写、按页面 gate 过滤）；Settings 左栏返回按钮下独立 `TextInput` 查找框（`settings-search-input`，不进草稿、不读 secure 缓冲，离开 Settings 清空）、结果列表（`settings-search-result-*`，↑/↓ / Enter / Esc）；`locate_settings_entry` → 切页 → `finish_settings_locate` 用实测框滚入目标行并约 2s 标识（`surface.hover` + 2px accent 左缘）、焦点落控件；只导航不写；供 `quick_search.rs` 共享页面 / 设置行标题；3 个测试 |
| `src/ui/settings/provider_quota.rs` | — | Go 账号三窗进度条 + 已用百分比、重置倒计时与过期/加载标注；AX 仍用完整已用/剩余/来源文本；刷新、选择模式控件与共享 AX 状态（ADR-060 / ADR-061 / UX-06） |
| `src/ui/settings/providers.rs` | ~1650 | provider 概览卡头（GUI2-06：固定 52px 单行五列——中性状态色点 · 名称 · 认证方式 · 「已连接 · N 个模型」chip · chevron，`settings-provider-header-{id}` 布局 id 钉 48–56px；原 64px 两列文字卡头作废）+ 独立认证操作详情行；普通层不显示 masked credential、endpoint、catalog error 或 raw model id；供应商列表后的「Default models」四角色区取代独立默认模型 section；Remove 保持二次确认；ADR-056 起卡片改为「64px 五列卡头 + chevron 展开区」（展开态按 provider_id 存 projection，有效展开 = 显式态 ∨ 流程态）：UX-06 展开区优先展示 Accounts（旧 Host 为 Credentials）区，其后是 Proxy / Manage models；账号区（ADR-061 紧凑卡：状态点 · 名称或行内重命名 · kind 徽标 · 当前选择 · Use/Rename/Remove；副标题 masked · 状态；添加不再要求手填名称，minor ≥17 发空串由 Host 生成默认名，旧 Host 本地回退脱敏 key / `OAuth`；空列表诚实空态，env fallback 不入列）与 Usage 行（Go 由 ADR-060/061 按账号展示三窗进度条与已用百分比；其它来源不可用时保留 unavailable）；展开区文本列一律 flex_row + flex_1 骨架（region 根 min_w_0 会以 Definite(0) 测量毒化 auto-basis 裸 flex_col 文本，真窗口实证）；ADR-052 provider 级代理 Switch：仅 Global `proxy_url` 已配置时可见，按生效值并排 On/Off 状态词，writes 总闸关闭（断线 stale）时 disabled；mouse / Enter / Space / AX Press 共用 `on_settings_toggle_provider_use_proxy`（OPT-3c 起控件形态为 `Switch`）；ADR-055 增供应商列表后的「Default models」四角色区（角色菜单候选 = 已连接且已启用，含 Clear；候选为空仍保留 Clear（鼠标/键盘/AX 同源）；vision/search 标注 save-only）与每 provider「Manage models」弹层（`MenuPanel` 内单模型 Switch 行 + Enable all/Disable all + 头行常驻 Refresh，禁用命中角色默认对时显示 cleared_roles 提示；目录为空显示诚实空态，Refresh 常驻头行，不渲染全开假按钮） |
| `src/ui/settings/general.rs` | ~220 | Network / HTTP proxy（wire 保持 General 兼容名） |
| `src/ui/settings/permissions.rs` | ~360 | 五档整行 radio（GUI2-06：Ø16 圆环 1.5px `text.secondary`、选中 Ø8 accent 实心，取代 `●` / `○` 字形）+ 会话信任；row click / Enter / Space / AX Press 共用 handler；`ApprovalModeWire` English 标签在 `approval_labels.rs` |
...
| `src/ui/components/mod.rs` | ~16 | 组件族模块声明（含 `icon` / `empty_state`） |
| `src/ui/components/empty_state.rs` | ~90 | GUI4：居中空状态容器（可选图标 + 调用方标题 / 说明 / 动作 / 骨架）；不发布 AX，identifier 仍由调用方 shell 节点承担 |
| `src/ui/components/icon.rs` | ~270 | GUI3-05：`Icon` 枚举 41 项 + `Icon::path()`（`icons/<kebab>.svg`）/ `Icon::all()`；`icon()`（20px `flex_none` `svg()`）与 `icon_sized()`（16px），默认 `text.primary`（gpui 将 SVG 渲染为 alpha mask，必须在 Svg 上染色；调用方可再覆盖，如 Send/Cancel 的 `on_accent`）；`Assets` 以 `include_bytes!` 静态表实现 gpui `AssetSource`（`load` / `list("icons")`），资产在 `apps/desktop/assets/icons/`，不引入 rust-embed；`TestAppContext` 无资产源时 svg 静默占位；1 个测试（全部登记路径可加载、`<svg` 开头、数量一致、无重复） |
| `src/ui/components/switch.rs` | ~200 | `Switch`（OPT-3c / GUI3-08）：36×20 轨道 + 16px 滑块，On/Off 状态词由调用方并排渲染；disabled 无 Press；mouse / Enter / Space / AX Press 同源；GUI2-06 关态 hover 用 `surface.hover` 不走 `accent.hover`；状态切换 120ms ease-out 位移，首帧瞬移避免挂载滑入 |
| `src/ui/components/skeleton.rs` | ~30 | GUI3-08：Changes / Resources 加载骨架（4 条静态 hover 色条，不循环） |
...
| `src/ui/components/label.rs` | ~55 | `Label`（单行文本，token 化字号 / 颜色） |
...
| `src/ui/components/status_bar.rs` | ~100 | `StatusBar`：底部 30px 三栏（左项目 / 分支、中用量、右瞬态反馈或连接文案）；用量仍绝对居中，左右栏 `occlude` 避免挡住；AX 只发布居中 `run-status`；R6 Wave A 起不再承载 Inspector/Activity 动作 |
...
| `projection/tests.rs` | 73 | 既有 snapshot/replay、Run/Timeline/TaskRail 与 Terminal 投影；未开始任务钉顶 / 复用、live 活动清钉顶、多标签 Close 回落与 create 失败保兄弟标签；Settings Data 走 protocol 类型 fail-closed（缺 nullable 键 / 类型错误）；SET-3～6d 解析与 stale/禁写回归仍在。 |
...
| `ui/mod.rs` | 17 | 既有键位、tab_stop、TaskRail 导航、Composer 与 per-session 草稿接线；进入/Refresh/重连 `refresh_all_settings`；R6 Wave A Header Activity；R6 Wave B Inspector 键盘目标、terminal gate 与 workspace 草稿/回执归属；R7 Wave C 钉字号放大/缩小/重置键位登记；终态关闭 gate；GUI4 终端按键直通与自动 fit resize |
