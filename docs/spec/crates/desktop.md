...
UI-2 收口实跑 214 个测试，全部内嵌于 bin target（`#[cfg(test)]` 模块；无 crate `tests/` 目录），下表保留历次测试资产分布，本批总数以实跑日志为准：
