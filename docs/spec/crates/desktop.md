...
| `src/main.rs` | ~723 | 入口与手动 argv 解析（非 clap）；开窗前 `ui::theme::install` 注入窗口主题；GUI3-05 起 `Application::new().with_assets(ui::Assets)` 挂载 `include_bytes!` SVG 图标资产；`PAWORK_UI_BARRIER_DIR` env 读取（空值视同未设置，None 全程零开销）；`run_app`（1440×1024 居中窗口 + `WINDOW_MIN_SIZE` 1080×720 最小尺寸——R2 Wave B 设计响应式底线，再窄击穿 Workspace ≥560 合同——+ 沉浸式 titlebar：TitlebarOptions appears_transparent，traffic lights 悬浮深色壳、内容视口贯通全窗，R2 Wave A；`install_keybindings`、聚焦 Composer、安装 macOS AX bridge）；`run_probe` / `run_probe_smoke` 无窗冒烟模式及其 `wait_for_*` 事件等待器 |
...
| `src/ui/accessibility/settings_subagents.rs` | ~380 | 子代理页 AX：开关 / 输入 / Save / 模型规则控件与 render 同 gate 同 identifier；ADR-063 目录同源过滤已连接供应商的启用模型，强度默认 / 范围 chip 节点与 render identifier 同源；RV-08 并发范围提示常发、错误原因仅出现时发布（StaticText） |
...
| `src/ui/settings/subagents.rs` | ~760 | GUI 1.20「子代理」页：全局开关 + 并发上限（1..=16）+ 按模型规则（可发起 / 可充当 + 7 项功能权限 chip + 重置默认）；ADR-063 目录只列已连接供应商的启用模型，能力徽标（图像 / 搜索）+ 默认强度 cycle + 可选范围 chips；全部即时全态写、回执收敛不乐观更新，写在途禁用防重复提交；RV-08 输入旁常显 1–16 范围提示、非法非空输入显示 danger 原因（render / AX 同源逐帧推导，空输入按待输入）；RV-09 全局开关文案改为对所有项目 / 新发起运行生效 |
