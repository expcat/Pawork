...
31 个 `.rs` 文件、约 31.5k 行，全部在 `[[bin]] pawork-desktop` target 内（无 lib target、无 crate `tests/` 目录）。
...
| `ui/accessibility/app.rs` | 13 | 稳定 identifier、TaskRail/Timeline/审批/菜单焦点与几何同源回归；SET-4 secure 输入掩码与 stale gate；SET-6e 新增离线外观导航 → AX Press 150% 主路径，断言 `TextScale::Percent150`、窗口根字号 24px、三档唯一 selected、AX bounds 使用 render 共用宽高/间距，并锁定未知 175% identifier 不获 action 许可。 |
...
- `--features gpui/runtime_shaders`：gpui 默认构建在编译期调用 Metal 着色器编译器；开发机仅有 Xcode CLT 时缺 Metal Toolchain 会构建失败，runtime_shaders 把着色器编译推迟到运行时使本机可闭环。当前 Desktop 定向门禁为 186/186（SET-6e 起）。
