...
2026-09-15 右侧面板：统一工具 / PTY 标签栏与空面板入口已实现；终端后台关闭保持当前页，回执后才移除标签。键盘方向切换、滚动裁剪与关闭回退由定向用例验证；本批自动检查及真窗口结果在交付时记录，用户人工验收未完成。
