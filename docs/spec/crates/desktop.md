...
66 个 `.rs` 文件，全部在 `[[bin]] pawork-desktop` target 内（无 lib target、无 crate `tests/` 目录）。
...
| `src/ui/timeline.rs` | ~730 | Timeline 容器：gpui `list()` 变高虚拟化与显式跟随；`timeline_rows()` 同源组装五类行，按既有 run/order 把连续 tool 聚合并由 terminal summary 吸收重复相位；P1 以首个 tool event id 作为稳定折叠 key，live / replay 共用结构，折叠态进入共享行高 / 可见窗口 / AX 公式；880px 居中阅读列、短会话 Top 对齐、首页按无任务 / 空任务区分下一步，`TIMELINE_OVERDRAW`=200px 合同不变 |
...
- **TaskRail（左侧栏）**
  ...
  - 底部 Local / Settings 行显示实际连接状态与状态点；主区继续提供断线原因、重试和诊断。头像 / 姓名 / quota / 组织等无权威来源元素不画。侧栏新建、筛选、分组、连接和 Settings 的 AX 框读取实际布局。
- **Timeline（中栏上）**：虚拟化列表渲染五类条目——靠右内容宽用户气泡、`Pawork:` 助手消息（流式增量合并为一条）、连续工具 group（标题汇总数量 / 状态，可折叠）、运行状态 / 唯一 terminal summary、`Error:` 错误行。GUI2-01 首页在无条目、无审批、无运行且未加载历史时居中显示 `Start a task`。无 active session 提示从侧栏选择或创建，并显示唯一 Primary `New task`；已有空任务提示在底部输入，首页不再要求创建任务。首页 title / hint / action 的 AX 读取实测框，Composer 始终保留原底部位置。无 active session 时 Header 不重复显示新建按钮。按钮与 Header 路径共用 `header-new-task` focus / handler，AX 同步 title / description / enabled action；Disconnected 保留旧条目时不显示空态。每条右侧「···」菜单含 Fork（仅 reducer 判定的闭合 run 边界可用；不可用时灰字禁用行；接受后聚焦 Composer）。用户上滚脱钩后右下浮出 `↓ Back to bottom`。
