...
| `src/controller.rs` | ~2640 | `DesktopController`（connect / 事件泵 / 空闲心跳 / 全部 Command·Query 构造与响应解析）；`open_workspace` 复用冻结 `workspace_add`，成功后刷新 snapshot 并发 `WorkspaceOpened{id,name}`；SET-3 增 `provider_auth_status` 查询（provider_id=None 全量）与 `ProviderStatusLoaded` 事件；SET-4 增认证写命令 `auth_start` / `auth_set_api_key`（ApiKeySecret 只在调用栈上构造，明文不进 projection/日志）/ `auth_cancel` / `auth_remove` 与 `AuthStarted` 回执（verification_url / user_code / expires_at）；SET-5 增 `set_default_model` 命令构造与 Data 回执解析（确认 pair 经 `DefaultModelConfirmed` 投递后自动重查 `provider_auth_status` 取回权威 default；Error / 传输失败走 OperationFailed，不动 UI 现有状态）；SET-6a 增 `general_settings` 查询与 `set_proxy_url` 命令（null=清除；Data 回执 `{proxy_url}` 即写后状态，经 `GeneralSettingsLoaded` / `ProxyUrlConfirmed` 投递，Error 取 Host 脱敏 message 原文）；SET-6b 增 `permissions_settings` 查询与 `set_approval_mode` / `workspace_trust` 命令（Data 回执即写后状态，经 `PermissionsSettingsLoaded` / `ApprovalModeConfirmed` / `WorkspaceTrustConfirmed` 投递；workspace_id 取 Host 查询透出的 attached id（ADR-048 实现期修订，校验方与发送方同源），匹配校验在 Host）；SET-6c 增 `mcp_test` / `mcp_server_remove` 命令与 `McpServersReceipt` 事件（回执与 mcp_list 同形状即权威，Error 取 Host 脱敏 message 走 OperationFailed，不动现有清单；回执 bump epoch 使在途 mcp_list 失效）；`ControllerEvent` 对 terminal create 失败携带 workspace、对 diff 内容携带 Host 实际 session id；ADR-045 `terminal_close` 命令构造与 `TerminalCloseSucceeded/Failed` 回执（成功不直接改 projection：running 终态由 live `TerminalExited` 刷新，exited 清理由 UI 回执后本地移除）；13 个测试 |
...
179 个测试全部内嵌于 bin target（`#[cfg(test)]` 模块；无 crate `tests/` 目录），按文件分布：
...
- `--features gpui/runtime_shaders`：gpui 默认构建在编译期调用 Metal 着色器编译器；开发机仅有 Xcode CLT 时缺 Metal Toolchain 会构建失败，runtime_shaders 把着色器编译推迟到运行时使本机可闭环。当前 Desktop 定向门禁为 179/179（SET-6b 起）。
