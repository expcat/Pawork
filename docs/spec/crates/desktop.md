...
> 本机单窗口 GPUI Agent 壳（TaskRail + Timeline + Composer + Inspector）：独立进程经 GUI Connection Protocol 连接 `pawork gui serve`，业务依赖为 [pawork-client](client.md) 与 [pawork-terminal](terminal.md)（终端显示核心纯库）；协议 / 传输类型仍只经 desktop → client →（re-export）获取，不被任何包依赖。
...
- **依赖 deny-list**：生产 `pawork-*` 依赖恰好 `{pawork-client, pawork-terminal}`。由 `platform.rs` 内测试 `desktop_production_pawork_deps_stay_client_only` 解析本包 `Cargo.toml` 断言，扫描器覆盖 `[dependencies]`、`[target.'cfg(...)'.dependencies]`、`[dependencies.<alias>]` 与 `package = "..."` 重命名形态；dev-dependencies 不计入。
...
71 个 `.rs` 文件，全部在 `[[bin]] pawork-desktop` target 内（无 lib target、无 crate `tests/` 目录）。
...
- **Inspector（cmd-i 开合；GUI2-05 起按 `shell_layout::InspectorPlacement` 放置）**：偏好关为 Hidden；宽窗（默认字号 > 1279 且 ≥ 1288，150% ≥ 1320）为 Side 440px 右栏；窄窗显式打开为 Center——同一面板占中央 Workspace（TaskRail 保留），顶部 36px「Back to conversation」`inspector-back`，不渲染 Timeline / Composer 且 AX 不发布，但保留实体、草稿、跟随 / 滚动偏移与连接状态；`pending_approval` 时面板顶部「Return to the conversation to handle the approval」`inspector-approval-hint`（点击即返回，不默认批准）；跨窄窗切换 `InspectorMotion::snap` 不动画。面板头 48px = 名称选择器 Dropdown（`inspector-panel`，value 当前面板，菜单 `inspector-panel-menu` 列实际接通的 Changes / Terminal / Resources，`MenuKind::InspectorPanel`）+ 36×36 `inspector-collapse`，取代 UI-1 三页签；默认 Changes；Changes 内二级条 40px / 每项 96px / 12px 字；各页滚动状态独立保留。
...
- **依赖边界**：生产 `pawork-*` == `{pawork-client, pawork-terminal}`（deny-list 测试）；`projection/` 零 gpui / tokio / OS import；协议类型只经 client re-export。Settings Data 缺 nullable 键 fail-closed（与 Host 必填键对齐），不把残缺帧当成未设置。
