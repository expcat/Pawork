...
| `src/controller.rs` | ~1900 | `DesktopController`（connect / 事件泵 / 空闲心跳 / 全部 Command·Query 构造与响应解析）；`open_workspace` 复用冻结 `workspace_add`，成功后刷新 snapshot 并发 `WorkspaceOpened{id,name}`；SET-3 增 `provider_auth_status` 查询（provider_id=None 全量）与 `ProviderStatusLoaded` 事件；`ControllerEvent` 对 terminal create 失败携带 workspace、对 diff 内容携带 Host 实际 session id；ADR-045 `terminal_close` 命令构造与 `TerminalCloseSucceeded/Failed` 回执（成功不直接改 projection：running 终态由 live `TerminalExited` 刷新，exited 清理由 UI 回执后本地移除）；12 个测试 |
...
| `src/ui/settings.rs` | ~223 | SET-3 Settings 壳：Settings Rail（「← Back to workspace」+ 首页导航项「Models & providers」，未接通页面不显示）与全宽只读供应商页（卡片只呈现 Host 权威名称 / 认证方式 / 连接状态 / 目录来源；stale / loading / error 可见文案；无写 capability 不画操作按钮） |
...
160 个测试全部内嵌于 bin target（`#[cfg(test)]` 模块；无 crate `tests/` 目录），按文件分布：
...
- `--features gpui/runtime_shaders`：gpui 默认构建在编译期调用 Metal 着色器编译器；开发机仅有 Xcode CLT 时缺 Metal Toolchain 会构建失败，runtime_shaders 把着色器编译推迟到运行时使本机可闭环。当前 Desktop 定向门禁为 160/160（P4 片 2F 起）。
