...
## 6. 与当前 Settings 活动线的关系

当前已选择 **Desktop Settings / 模型与供应商** 作为活动线，但没有创建发布版本。下列候选仍不得被自动并入 Settings：

1. **本机多账户与成本效率**：G1–G6；Settings 首期只允许每个 Provider 一个活动连接；
2. **Desktop 完整编码工作台**：Git 写入、`@` 补全、规则可见性等；
3. **分发与集成**：IDE/CI/Web/安装发布等 D 类候选。

首批 Settings 收口后，如用户选择其中一类，再创建独立 Feature Spec/任务切片，明确不做项、成功指标与证据预算。发布继续维持 BK-RELEASE-01 未授权状态。
