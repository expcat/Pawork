...
> 基线日期：2026-08-25。状态词汇见 [README.md](README.md#3-状态词汇)。本表记录生产可见面，不以“代码存在”替代当前阶段复验或发布证明。
...
| CAP-SESSION-01 | 会话列表、查看、恢复、导出/导入、分支 | `sessions`、`--resume` | 已实现 | envelope v1、DB v12、export v3；损坏/Secret 导入 fail-closed。 |
...
| CAP-AUTH-01 | API key、OAuth 登录/刷新、脱敏状态 | `auth list/set-key/login/logout` | 已实现；部分待人工验收 | ChatGPT/xAI 自然临期 refresh 仍属 R9 人工验收。OS Keychain 不在当前实现。 |
| CAP-CONTEXT-01 | 上下文预算、compaction、用量/定价 | Run、`usage` | 已实现 | usage 幂等冲突与哨兵口径仍有 ROADMAP 登记，R9 复核未完成。 |
...
| CAP-GUI-01 | 本机 GUI server 与 GPUI Desktop | `gui serve`、`pawork-desktop` | 已实现；待人工验收 | K-03 十一项未签字；断线不取消 Run，15s heartbeat 对 30s host timeout。 |
...
| `usage` | 用量查询 | 已实现；R9 仍需复核既有 usage 登记项。 |
...
| --- | --- | --- |
| `chatgpt` | OAuth | 已实现；自然临期 refresh 待 R9 人工验收。 |
| `xai` | OAuth | 已实现；自然临期 refresh 待 R9 人工验收。 |
...
| 面 | 当前能力 | 状态 |
| --- | --- | --- |
| TaskRail | 会话/任务选择、新建、长标题截断 | 已实现；K-03 人工验收待签。 |
| Timeline | 变高虚拟化、流式条目、审批、fork 边界、回底 | 已实现；长会话/菜单锚点/hover 人工项待签。 |
| Composer | 输入、发送、`@` host 展开 | 部分实现；IME/多行粘贴待人工验收，`@` 候选浮层未实现。 |
| Changes | Files/Summary/DiffView/ActivityPopover | 已实现为只读；stage/unstage/hunk 未实现。 |
| Terminal | 创建、输入、resize、输出 | 已实现；创建受 Policy，响应安全说明尚未形成完整渲染面。 |
| Resources | MCP server/tool 状态只读列表 | 已实现；已加载 AGENTS/Skills 分区无 host query。 |
