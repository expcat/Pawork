...
> 本目录是对整个仓库（24 成员：22 库 + 2 应用，约 25.2 万行 Rust）的完整代码级 Review 产出，面向后续分析、重构与新人上手。
>
...
2. 按关注面选读 [backend/](backend/)（22 个库 + CLI 宿主）或 [frontend/](frontend/)（GPUI Desktop，按状态层 / 视图层拆分）。
...
| 包 | 文档 | 规模 | 一句话 |
| --- | --- | --- | --- |
| pawork-domain | [backend/domain.md](backend/domain.md) | 18 文件 / 5.3k 行 | canonical 领域类型 + Provider/Tool 契约 + 事件信封 v1（纯净红线） |
| pawork-transport | [backend/transport.md](backend/transport.md) | 6 文件 / 1.7k 行 | framed 字节传输：local（UDS / named pipe）+ memory |
| pawork-terminal | [backend/terminal.md](backend/terminal.md) | 1 文件 / 0.8k 行 | 终端显示核心纯库：行缓冲滚动文本解析（CR / 退格 / SGR）与按键映射（非完整 VT emulator） |
| pawork-browser | [backend/browser.md](backend/browser.md) | 3 文件 / 0.9k 行 | 系统 WebView 浏览器：四路径导航闸、无凭据 HTTP(S) only、无页面→宿主桥 |
| pawork-computer-use | [backend/computer-use.md](backend/computer-use.md) | 3 文件 / 1.2k 行 | 纯 Rust RFB 控制专用容器虚拟桌面（127.0.0.1:5905），不触本机 HID（在途） |

### L1 契约层

| 包 | 文档 | 规模 | 一句话 |
| --- | --- | --- | --- |
| pawork-protocol | [backend/protocol.md](backend/protocol.md) | 38 文件 / 15.6k 行 | GUI 帧 / headless-json / core-api / typegen + 三通道 registry + 共享投影 reducer |
...
| pawork-auth | [backend/auth.md](backend/auth.md) | 13 文件 / 6.0k 行 | Secret 后端 / OAuth / 多账号索引与选择 / 脱敏解析链（locator 单一事实源） |
| pawork-providers | [backend/providers.md](backend/providers.md) | 35 文件 / 15.2k 行 | net/（http/sse/retry）+ registry/pricing/usage/negotiate/reasoning + 八通道 channels/ |
...
| pawork-control-plane | [backend/control-plane.md](backend/control-plane.md) | 16 文件 / 13.9k 行 | 控制面 core + quota/ + credential/（lease/pool） |
...
| pawork-workspace | [backend/workspace.md](backend/workspace.md) | 35 文件 / 15.4k 行 | workspace service + file_index + resources + config 六层矩阵 + 五来源 import |
...
| 包 | 文档 | 规模 | 一句话 |
| --- | --- | --- | --- |
| pawork-tools | [backend/tools.md](backend/tools.md) | 21 文件 / 10.4k 行 | 九工具 + ToolScheduler + mcp/ |
...
| 包 | 文档 | 规模 | 一句话 |
| --- | --- | --- | --- |
| pawork-app | [backend/app.md](backend/app.md) | 70 文件 / 35.9k 行 | 装配宿主 AppCore + gui_server/ + gui_host/ 分发表（最大库） |

### L6 接口层与宿主

| 包 | 文档 | 规模 | 一句话 |
| --- | --- | --- | --- |
| pawork-cli | [backend/cli.md](backend/cli.md) | 30 文件 / 12.2k 行 | 21 子命令 + channels/acp/（AcpHost） |
| pawork-client | [backend/client.md](backend/client.md) | 15 文件 / 6.5k 行 | framed 连接面 + headless/ SDK |
...
Desktop 共 75 文件 / 60.0k 行，按四层职责拆为四篇：
