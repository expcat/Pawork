...
| `auth_start` / `auth_set_api_key` / `auth_cancel` / `auth_remove` / `auth_account_*` | settings.rs | 认证与账号命令；API key 明文只在本次调用栈转成 wire 命令后即弃（不写日志 / 事件 / 状态）；成功态收敛走 AuthChanged 事件；`auth_account_rename` 仅 minor ≥17 |
...
| `set_pending_model` / `effective_model` / `context_meter_label` / `run_status_label` / `run_usage_display` | 方法 | 模型切换 pending / 生效口径 / ContextMeter（未知显示 unavailable）/ RunStatusBar（↑ / ↓ / 时长；终态权威 usage，运行中按已上屏正文预览，时长 `now_ms` 注入） |
