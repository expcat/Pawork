...
统计（2026-09-17 源码口径）：`apps/desktop/src` 共 **75 个 .rs 文件、59,995 行**，全部在单一 `[[bin]] pawork-desktop` target 内（无 lib target）。四层规模：`ui/` 47,724 行、`projection/` 6,905 行（含 tests.rs 3,691）、`controller/` 4,246 行、platform.rs + platform/ 397 行、main.rs 723 行。生产 `pawork-*` 依赖恰好 `{pawork-client, pawork-terminal, pawork-browser}`，由 `platform.rs` 内测试钉死。
...
4. `run_app` 进入真实窗口路径：`Application::new().run` → `install_keybindings` → 打开 1440×1024 居中窗口（`WINDOW_MIN_SIZE` 钉 1080×720，窗口系统级限制，测试锁定）→ 透明 titlebar（macOS `NSFullSizeContentView`，traffic lights 悬浮于 TaskRail 顶部安全区）→ `AppView::new(Arc<Platform>, socket, barrier_dir)` → `restore_appearance`（首帧前从 `desktop.json` 恢复语言与字号）→ `install_files_close_guard`（文件面板未保存草稿的关窗提醒）→ `install_accessibility`（AX 请求经 gpui foreground executor spawn 回到 `window.update` → `handle_accessibility_request`）→ 聚焦 Composer → `cx.activate(true)`。
...
Desktop 不拉起、不发现进程，只做**路径发现 + 鉴权文件读取 + 主动连接**：Host 由 `pawork gui serve` 启动并写 `gui.token`；Desktop 按 socket 文件名推导同目录 token（`pawork-gui-dev.sock` → `gui-dev.token`），token 文件缺失 / 空 / 非 UTF-8 一律 fail-closed 拒连。能力宣告为 `Events / Snapshots / Approvals / TerminalStreaming / BrowserControl` 五项（不宣告 `ArtifactStreaming`；BrowserControl 为 2026-09-16 浏览器批次新增，同时是 `poll_browser` 的 gate 之一）；`ConnectOptions` 固定 10s 超时、1 MiB max frame、client_label `pawork-desktop`。
...
- **client 帧匹配的 request_id 纪律**（crates/client `FrameWant::matches`）：`Response / Snapshot / Resume / SubscriptionBarrier` 只接同 request_id 的 Error 帧；常驻事件泵的 `Event` want 只接 `request_id=None` 的连接级错误——带 request_id 的命令错误不会被泵抢走导致等待方误报断线。2026-09-17 复核该修法仍在。
...
**deny-list 断言**：`platform.rs` 测试 `desktop_production_pawork_deps_stay_client_only` 用 `include_str!` 读本包 `Cargo.toml`，断言生产 `pawork-*` 依赖恰为 `{pawork-client, pawork-terminal, pawork-browser}`。扫描器覆盖 `[dependencies]`、`[target.'cfg(...)'.dependencies]`、`[dependencies.<alias>]` 与 `package = "..."` 重命名形态；dev-dependencies（`tempfile`、gpui test-support）不计入，另有负例测试钉死扫描器语义。macOS target 的 `cocoa` / `objc` 仅为 ADR-042 原生 AX bridge 服务，`raw-window-handle` 从 GPUI 窗口借用原生句柄交给 browser WebView。
...
| client 事件泵抢占命令错误帧 | `FrameWant::Event` 若匹配所有 Error 会抢走带 request_id 的命令错误；Event 只接 request_id=None，Response/Snapshot/Resume 只接同 id | AGENTS.md §11「client 事件泵抢占命令错误帧」 |
