...
> Agent Engine 执行核：把 canonical 消息与工具定义装配成 `CanonicalModelRequest`，驱动多轮工具循环并把全过程事件化。共 18 个 .rs 文件 / 6,516 行（src 6,307 + tests 209，共 70 个测试）；主要模块：`tool_loop`（多轮循环）、`session_turn`（单轮事件化）、`appender`（流折叠）、`context`（预算/压缩/估算/裁剪）、`cancel`（run 级取消）、`event`（事件出口）。
...
- **红线**：生产依赖仅 `pawork-domain`（`tests/domain_only.rs` 解析 Cargo.toml 断言，含 alias/target 表）；`src/` 不得出现任何 provider 名串（`tests/no_provider_branch.rs`，名单从 `CHANNEL_REGISTRY` 派生 + 14 个固化基线别名，dev-only 依赖 `pawork-providers`；rg 独立复查 src/ 零命中）。
...
| 相对路径 | 行数 | 职责 |
| --- | --- | --- |
| src/lib.rs | 410 | crate 门面与 re-export；`assemble_request(_with_tools)`；`pub(crate) run_turn`；内联单测（6 个） |
| src/event.rs | 251 | `EngineError`、`AgentEventSink`、`EventEmitter`/`LoopEventEmitter`/`LoopSink`、`map_provider_event` |
...
| src/session_turn.rs | 643 | `SessionTurn`、`run_session_turn` 单轮事件化、`now_timestamp` |
| src/tool_loop/mod.rs | 348 | `run_session` 编排、`LoopContext` trait、闸门/快照/压缩回调数据类型 |
| src/tool_loop/round.rs | 97 | 单轮流收集 `collect_stream_round`、助手消息装配、usage 饱和加法 |
...
| src/tool_loop/compaction.rs | 499 | 资源层注入、输入估算、软限压缩链、硬限截断、`run_manual_compaction` |
| src/tool_loop/tests.rs | 2,200 | `run_session`/`run_manual_compaction` 全量定向测试（24 个 tokio 测试，`#[cfg(test)]`） |
...
| 名称 | 种类 | 功能/语义 |
| --- | --- | --- |
| `assemble_request` | pub fn | 用冻结契约默认值填满 `CanonicalModelRequest`：tools/hosted/extensions/stop_sequences 空、`ToolChoice::Auto`、`ResponseFormat::Text`、`PromptCachePreference::Automatic`、`RequestBudget::default()`、thinking/reasoning/temperature/max_output_tokens/trace_id/session_id 全 None、provider_options 空 BTreeMap |
...
| `EventEmitter` | pub(crate) struct | sequence 分配器 + 信封封装：`emit` 以 `AtomicU64::fetch_add` 从 start_sequence 起分配，`event_id = "evt-{run_id}-{sequence}"`；**timestamp 逐事件取 emit 时刻墙钟**（`now_timestamp()`，event.rs:66-83——同轮共享时间戳会让终态 run 的首末事件差恒为 0，已改为逐事件取值） |
...
| `CancelHandle` | pub struct（Clone） | `new(run_id, cleaner)`；`token()`/`child_token()` 返回同一根 `CancellationToken`（传 provider stream 与 tool execute）；`cancel(reason)` 用 `AtomicBool::compare_exchange` 原子门控：首次先 `token.cancel()` 再 `cleaner.cleanup` 并记录 `processes_killed`，重复调用返回 `already_cancelled=true` 且不再清理；`reason` 参数被忽略（仅供调用方写事件/日志，不进 RunCancelled 信封）；另有 `is_cancelled()`/`processes_killed()` 查询 |
...
| `run_session` | pub async fn | 多轮事件化主循环（行为详见 §5.1）；覆写 request.session_id 为 turn 真实值，工具续轮与摘要请求保留此身份 |

`LoopContext` 方法签名与语义：

| 方法 | 语义 |
| --- | --- |
| `async execute_tools(calls, events, cancel) -> Vec<ToolCallResult>` | 执行已放行调用；执行期间可经 `events.emit_tool_event` 转发工具输出流；返回结果不要求与 calls 等长同序——engine 用 `align_tool_results` 按 invocation 顺序对齐，缺失回填 `NotFound("missing tool result")` |
| `async request_approval(calls, already_approved_for_run, events, cancel) -> Result<Vec<ApprovalGate>, EngineError>` | 对整批调用逐个给出闸门，返回向量必须与 calls 等长；实现契约：每次阻塞等待决策前必须 emit `ToolApprovalRequested`（reason 逐字 ``tool `{name}` requires approval``，含 batch 已批准短路路径）；`already_approved_for_run=true` 时不应再询问 |
...
| `truncate_for_budget` | 私有 fn | 每条消息一次计数后从最旧非 System 消息逐条丢弃直到估算 ≤ max_input 或只剩 floor 条；截断后就地刷新调用方的 `InputEstimate`（不再二次全量重估）；返回（丢弃条数, 截断后估算） |
| `run_manual_compaction` | pub async fn | REPL `/compact` 入口：不是 run，不发 RunStarted/RunCancelled；校验 start_sequence 与「nothing to compact」；reason 固定 `AutoCompactionReason::Manual`；摘要请求保留 turn.session_id；返回重建后的消息列表 |
...
3. 覆写 request.session_id 为 turn 真实值；注入资源层（injected_layers 非空时拼 System 前缀 + `Diagnostic("resources.injected")`）。
4. 每轮：轮首取消检查 → `estimate_input` → `ContextPrepared{message_count, estimated}` → `apply_context_limits`（软限压缩/硬限截断）→ `ProviderRequestStarted` → `collect_stream_round`（`&current` 借用传递，2026-09-18 R-07 起零拷贝）。
...
- 内部摘要请求不进 AgentEventSink、usage 不计入 run_usage、保留原请求 session_id；失败/空摘要降级结构性摘要而非失败。
- 截断永不丢最后 `retained_messages` 条（默认 4）与 System 消息。

### 5.5 事件可持久化可重放 / Provider 中立

- 所有 AgentEvent 经 `AgentEventSink` 出口，sequence 从 start_sequence 起连续原子分配，`event_id = evt-{run_id}-{sequence}`，timestamp 逐事件取 emit 时刻墙钟；排序与重放以 sequence 为准；persist 失败后不再补终态（磁盘停在最后一条成功 append，恢复由重放完成）。
- `map_provider_event` 与全循环只消费 canonical `ProviderStreamEvent`/`ModelProvider`；`no_provider_branch` 测试扫描 src/ 全部 .rs 禁止通道 id 与基线别名出现（rg 独立复查零命中）。新增通道自动进入守护名单（从 providers CHANNEL_REGISTRY 派生），无需改 engine。

### 5.6 与 Spec 的一致性

`docs/spec/crates/engine.md` 与源码逐条核对一致：模块树、API 面、事件序、K-02 审批对、依赖红线（Cargo.toml 生产依赖恰为 pawork-domain）、feature 无（default=[] 无具名 feature）、测试资产（tool_loop 24 个 tokio 测试）均吻合；行数量级为约数（如 tool_loop/tests.rs ~2180 vs 实际 2,200），不构成行为差异。

### 5.7 已知低效点（质量扫描记录，非行为缺陷）

- 原「`run_session` 每轮 `current.clone()`」已于 2026-09-18 R-07 根治：`ModelProvider::stream` 改收 `&CanonicalModelRequest`（domain 公开 trait，全部 adapter 本就只借用），engine 每轮零拷贝；公开入口 `run_session` / `run_session_turn` / `run_manual_compaction` 仍按值收请求。

## 6. 测试资产

| 文件 | 验证点 |
| --- | --- |
| src/lib.rs 内联（6） | assemble 默认值、run_turn 事件透传（含未消费变体）、预取消不调 provider、流中取消 |
| src/event.rs 内联（1） | map_provider_event 七类映射与 ResponseCompleted 不映射 |
| src/cancel.rs 内联（3） | cancel 触发 token+cleaner、幂等、child_token 随根取消 |
| src/appender.rs 内联（4） | 文本+工具调用按序折叠、空/非法参数 → Null、reasoning item 安全折叠、tool_results_message |
| src/session_turn.rs 内联（5） | 单轮事件序、预取消、流中取消不提交助手、ProviderError → RunFailed、persist 失败停发终态且 resume 续 sequence |
| src/context/budget.rs 内联（3） | 预留扣减、饱和、serde round-trip |
| src/context/compaction.rs 内联（5） | serde snake_case、硬限优先、软限触发、无触发路径 |
| src/context/token.rs 内联（7） | chars/token 向上取整、CJK 分开计数、framing 常量、消息/schema/嵌套 ToolResult/reasoning summary 计数 |
| src/context/tool_result_trim.rs 内联（8） | 四级分类边界（闭区间）、Medium 头尾+说明、Large 摘要+ArtifactRef、Huge 仅引用、空结果、纯图片不误判 Small |
...
| tests/domain_only.rs（2） | 生产依赖 domain-only 红线 + 解析器 alias/target 覆盖 |
| tests/no_provider_branch.rs（2） | 守护名单派生自 CHANNEL_REGISTRY + src/ 无 provider 名串 |
