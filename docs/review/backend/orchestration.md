...
> 多 Agent 编排:在控制面(租约/用量/租户策略)之上运行 `AgentSupervisor`——spawn worker、预算闸门、取消整棵子树、任务依赖 DAG、可选 git worktree 隔离与 patch merge。13 个 .rs 文件、共 7,488 行(内联测试约 3,775 行,非测试逻辑约 3,713 行,`supervisor/mod.rs` 内嵌 2,382 行/46 个测试);依赖 `pawork-domain` + `pawork-policy`(路径内核) + `pawork-control-plane`(关 default features)+ optional `pawork-git`(feature `git`),禁止依赖 `pawork-workflow`(Cargo.toml 核实无该依赖,仅 pawork-app 依赖本包)。
...
- **产品接线现状**:`pawork-app` 以 `default-features = false` 依赖本包(git feature 关闭),且 app 侧未注入 `PatchMerger`——`propose_patch`/`approve_patch` 与 `GitWorktreeAllocator` 当前在产品路径不可达,merge/worktree 为纯库能力。
...
| `pawork-control-plane`(default-features=false,不拉 rusqlite) | `CredentialPool`/`LeaseGuard`/`LeaseOutcome`/`AcquireRequest`、`TenantPolicyEngine`/`Permission`/`PolicyGate`/`PolicyDecisionEvent`、`UsageLedger`/`UsageRecord`/`UsageQuery`/`AUTO_RECORD_ID_PREFIX` |
| `pawork-policy` | `resolve_workspace_path` 路径内核(merge collect / detect_conflicts / merge 越界防护共用) |
...
| src/merge.rs | 620 | `DiffProvider` trait、`PatchMerger`(collect/detect_conflicts/merge)、`resolve_relative` 越界防护、`atomic_write` |
| src/supervisor/mod.rs | 2,728 | `AgentSupervisor` 结构与 `SupervisorConfig`/`SupervisorError`(11 变体);complete/fail/retry_task/propose_patch/approve_patch;内嵌 45 个测试 |
...
| src/supervisor/cancel_tree.rs | 171 | `cancel_tree` 自根向下收集整棵子树并逐节点取消、`CancelTreeReceipt` |
...
| `WorkerStateMachine` | struct | `from_state`(恢复用)/`state()`/`apply(t) -> Result<WorkerState, LifecycleError>`(2026-09-18 起事件映射死轨 EventHint 已删除,事件由 supervisor 手工构造单轨) |
...
- `check()`:硬超限判定是 **`used >= limit`**(相等即超限;budget.rs:257 附近);源内两处 doc 注释(BudgetReport 字段注释与 check() 注释)仍写「> limit」,与实现不一致(以实现与测试为准)。软告警 `used >= ratio×limit` 用 ppm 整数比较(`u128` 中间量,`used×1e6 >= limit×soft_ratio_ppm`),全程无 f64 精度误差。
- `diff_hard_exceeded(report)`:只读传入报告不重算;「已告警」记忆集合跨 Clone 共享——同一维度持续超限只返回一次,用量回落(不再硬超限)后从记忆移除、可再告警。**注意**:现有测试用 110>100 触发硬超限,未覆盖 used==limit 的等值边界。
- `flush_to_ledger`:async mutex 序列化;每条 record 内容 = `目标快照 - last_committed` 增量,`record_id = auto-rec-budget-{controller_id}-{input}-{output}-{cost_micros}`(budget.rs:357-361;前缀复用 control-plane 保留前缀 `AUTO_RECORD_ID_PREFIX = "auto-rec-"`,usage.rs:66——内容寻址幂等键,重试重放同 record 不重复计账);pending 在 await 前保存,ledger Ok(含幂等重放)才推进游标;失败/取消保留完全相同 pending;无增量空操作;cost-only 增量单独成条;循环处理「旧 pending + 新增长」聚合提交。cache_read/cache_write 恒写 0(注释 P14-7:cache 通路未贯通,明确不写误导值)。
...
| `resolve_relative` | fn(cfg(any(test, feature="git"))) | 拒绝空路径、绝对分量(Prefix/RootDir)与 `..` 穿越;**仅被 `GitDiffProvider::file_content` 使用**——`PatchMerger::detect_conflicts` 读父侧(merge.rs:239)与 `merge` 原子写(merge.rs:301)直接 `parent_path.join(file)`,不经过该校验(见 §5) |
| `atomic_write` | fn(私有, merge.rs:335-358) | 同目录 `.merge-tmp-{pid}-{counter}`(进程级静态 AtomicU64 保证并发唯一)+ `fs::write` + `rename`,失败清理临时文件;无 fsync(原子性依赖同目录 rename)。与 storage blob 的 `atomic_write_bytes` 属跨包重复实现(主代理已登记跨包发现) |
...
| `AgentSupervisor` | struct | 内部状态:workers / cancel_tokens / children / reservations(在途并发预约)/ pool / policy / ledger / event_log / next_agent_id / budget(controllers)/ config / parent_workspace? / worktree_allocator? / task_graph? / patch_merger? / pending_patches / flush_ctx(终态失败归属缓存)/ flush_in_flight |
...
- `fail(&AgentId, reason)`:同 complete,但 lease 标记 `Failed`(计入连续失败)、TaskGraph fail+`TaskFailed{reason}`、事件 `WorkerFailed{reason}`。与 complete 约 60 行同构(见 §5 冗余记录)。
- `cancel_tree(&AgentId) -> CancelTreeReceipt`(cancel_tree.rs):先确认 agent 存在;以**栈式遍历**(Vec::pop + extend,深度优先序;源码注释写 BFS,与实现不符)收集全部后代;每节点:取消令牌 → 锁内 `BeginCancel`+`Cancel`(终态跳过)→ 发 `WorkerCancelling`+`WorkerCancelled` → 释放 worktree → TaskGraph cancel+`TaskCancelled` → lease 标记 `Cancelled`(幂等释放,**不惩罚账号健康**,只累加取消计数)→ flush。幂等:重复调用对终态节点全部跳过。任一 flush 失败 → `Err(CancelTreeFlushPending{receipt, pending})`(取消本身完成)。
- `record_usage(&AgentId, input, output, cost_micros)`(budget_gate.rs):终态拒绝(`WorkerTerminal`);累加进表内 controller(直接 `get_mut`,不经 Clone);`check` + `diff_hard_exceeded` 对新进入硬超限维度逐个发 `BudgetExceeded{dimension, used, limit}`。
...
spawn 步骤(顺序即代码序):0 parent 准入 → 1 生成 canonical `agent-N` id + 深度闸门(超限发 `ConcurrencyDenied{kind:"depth"}`+审计拒绝)→ 原子并发预约(全局拒绝发 `ConcurrencyDenied{kind:"agents"}`;租户拒绝只审计不发事件)→ 2 策略闸门链(deny-first):主体角色 `Permission::AgentSpawn` → 模型白名单 `check_model`(仅 req.model 有值)→ acquire 存在时校验 AcquireRequest 外层一致性(tenant/principal/session 错配拒绝)+ `Permission::LeaseAcquire` + provider/account 白名单 → 日 token/cost 预算(租户配置任一预算维度才查账本;`ledger.aggregate` 失败 **fail-closed** 拒绝)→ 记录 allow 决策 → 3 建 AgentInstance(worktree_path 先取请求值)→ 4 Admit(折叠进 spawn)→ 5 worktree 分配(仅配置了 allocator+parent_workspace 且请求未自带路径;分支名 = agent_id;失败 → Failed 标记+注册+`WorkerFailed`+归还预约,返回 `PoolAcquire`)→ 6 发 `WorkerCreated`(worktree 用分配后真实路径)+`WorkerAdmitted` → 7 lease 申请(canonical 覆写 agent_id 与外层三标识;成功后 **validate_lease_scope** 校验,错配 → `into_lease` 取走守卫 + 显式 `release(Released)`(不惩罚账号健康)+ 释放 worktree + Failed 注册 + `PolicyDenied`;acquire 失败 → 释放 worktree + Failed 注册 + `PoolAcquire`)→ 8 Start+`WorkerStarted` → 9 注册 children 与 cancel token → 10 TaskGraph 注册(发 `TaskCreated`;Ready 即 `TaskAssigned`+`start`;Blocked 等待依赖完成,由外部 ready_tasks+mark_ready 推进;失败 → 取消本次已加任务 + `abort_spawn_as_failed` 收口,返回 `PolicyDenied`)→ 11 原子兑现预约(reservations 移除与 workers 插入同一临界区)+ 注册预算 controller。

**失败路径统一收口**:已准入后的全部失败路径(worktree 分配、lease 申请/作用域错配、TaskGraph `add_task`/`assign`/`start`)都经 `abort_spawn_as_failed`(spawn.rs:491):标记 Failed → 移除 children 边与 cancel token → 注册 WorkerEntry 并发 `WorkerFailed` → worktree 显式释放、lease 以 `LeaseOutcome::Released` 释放(不惩罚账号健康);重复 id 时只取消本次成功加入的任务(不波及既有任务),返回 `PolicyDenied`。与 Spec §5「spawn 中途失败也把 worker 标记 Failed 并注册」一致,跨租户 task_deps 触发路径有 `task_graph_rejection_releases_resources_and_records_failed_worker` 定向回归。

## 5. 关键行为与契约

- **禁止依赖 `pawork-workflow`**(依赖方向红线;plan/task 与编排的装配在 app;Cargo.toml 核实)。
- **取消覆盖整棵 worker 树**:自根向下收集全部后代无孤儿(栈式遍历,深度优先序;注释/Spec 写 BFS,与实现不符,集合等价仅顺序不同);`LeaseOutcome::Cancelled` 释放只累加取消计数,不惩罚账号健康;幂等重复调用。
- **预算超限走控制面 ledger**:本包不写 SQLite;`BudgetExceeded` 同维度持续超限只发一次,回落可再告警;硬超限边界是 `>=`(相等即超,`hard_limit_is_ge_boundary` 直接断言 used==limit)。
- **flush 不吞 pending**:终态 flush 失败必显式返回(`UsageFlushPending`/`CancelTreeFlushPending{receipt,pending}`),controller 与归属 ctx 成对保留;提交游标 + 内容寻址幂等 record_id(`auto-rec-budget-{controller_id}-{totals}`)保证重试不重复计账;FlushTicket 在途标记防并发假成功;cache 维度恒 0(未贯通,不写误导值)。
- **spawn 原子并发**:预约与兑现都在「先 reservations 后 workers」单一锁序临界区,无 check-then-act 窗口;RAII 保证任一失败路径归还槽位;已准入后的失败经 `abort_spawn_as_failed` 统一清理(见 §4.8)。
...
- **事件流一致**:worktree / lease / TaskGraph 注册失败路径都把 worker 标记 Failed 并注册(`WorkerFailed` 落日志,见 §4.8)。
- **deny-first 策略链**:任何一层拒绝不可被上层覆盖;日预算查询失败即拒绝(账本不可用绝不静默放行)。
- **recover_report 是 report-only**:不重建可操作状态、不 emit 事件;崩溃后要继续操作需重新 spawn。
- **worktree 释放安全**:只经 allocator(git 侧校验受管 worktree),绝不递归删除用户数据;Guard Drop 不隐式释放(避免无 runtime panic),须显式 `release`;complete/fail/cancel_tree 及 spawn 的全部失败路径(含 TaskGraph 注册)均已覆盖显式释放。
- **merge 路径安全**:`resolve_relative` 委托 `pawork_policy::resolve_workspace_path`,拒绝绝对分量、`..`、`.git` 与 symlink 逃逸;`collect`、`detect_conflicts` 读父侧与 `merge` 写入主路径全部经该校验,主路径越界拒绝有 `public_merge_paths_reject_unsafe_files_before_writing` 定向回归(当前产品接线未注入 PatchMerger,默认不可达)。写入原子(tmp+rename);冲突绝不自动合并。
...
**冗余与低效记录(质量扫描)**:2026-09-18 R-06 已收口——complete/fail 同构块收敛为 `finish_terminal` + `TerminalOutcome`;lifecycle `EventHint` 死轨删除(仅自测引用);`now_ms` 合一保留 budget.rs 一份 pub(crate)。(spawn.rs 失败注册块已统一为 `abort_spawn_as_failed`。)

## 6. 测试资产

无 tests/ 目录,全部内联 `#[cfg(test)]`(共 87 个测试):

| 位置 | 覆盖点 |
| --- | --- |
| supervisor/mod.rs(46 个 tokio 测试) | spawn 生命周期事件序;lease 持有到 complete/fail 的 outcome;cancel_tree 递归/幂等/lease Cancelled 不惩罚健康/深树;recover_report 只读;parent 准入矩阵(缺失/跨租户/跨 session/终态);租户并发/本地并发(ConcurrencyDenied);未知 agent;事件快照重放 round-trip;worktree 分配;TaskGraph 联动(Ready 直启/Blocked 等待/retry/注册失败收口释放 lease、worktree 与并发槽位);complete 真实归属 flush;patch propose→approve;provider/account/model 白名单、角色 deny-first、每租户并发;并发 spawn 不超全局/池上限;日预算 fail-closed;AcquireRequest 错配拒绝;恶意 pool lease 校验释放;canonical agent_id;深度闸门(兄弟放行孙代拒);终态 flush 失败保留可重试;BudgetExceeded 去重/恢复再告警/used==limit 等值硬超限;cancel_tree flush pending 透传;flush_usage 活动拒绝/ctx 配对/并发无假成功无双计/与终态 flush 并发;fail+cancel flush 与 complete 一致 |
| lifecycle.rs(8) | 合法转换全表;Complete 三源态;任意活动态 cancel/fail;终态拒绝一切;状态推进(原 EventHint 断言随死轨删除改为状态断言);重放重建与未知事件忽略;终态后迟到事件忽略;事件 serde snake_case tagged |
| budget.rs(9) | 维度上限;flush 归属/cost-only/同快照幂等/累计增量(10→20 聚合到 20)/失败重试同 record/失败后增长聚合两次 delta/中途取消重放不双计/并发 flush 共享游标(used==limit 等值边界由 supervisor/mod.rs `hard_limit_is_ge_boundary` 覆盖) |
| task_graph.rs(7) | DAG 顺序;拒环;跨租户依赖拒绝;依赖完成转 Ready;retry 计数;幂等 complete;重复 id 拒绝 |
| worktree.rs(6) | 分配隔离(worker 写不动 parent 文件);guard release;Drop 不派发任务;into_inner 转移;非受管释放记录但保留数据 |
| merge.rs(8) | collect;冲突检测(父侧漂移才冲突);干净文件原子合入;冲突绝不自动合并;Reject/NeedsConflictResolution 零写入;相对路径穿越拒绝(含 collect/detect_conflicts/merge 主路径越界回归) |
| identity.rs(3) | worker/parent 构造;serde snake_case |
...
graph LR
    ...
    policy[pawork-policy<br/>resolve_workspace_path] --> orch
    git[pawork-git<br/>仅 feature git,当前无成员启用] --> orch
    orch --> app[pawork-app<br/>orchestration host,default-features=false]
