...
> 工具层:九个内置 Agent 工具(4 只读 + 3 写 + run_command + computer)、最小调度器(ToolRegistry + ToolScheduler:Policy 闸门、审批解析、全局并发信号量、超时)与 MCP 客户端子系统(`mcp/`,rmcp SDK 被隔离在 codec.rs 单文件)。21 个 .rs 文件、共 10 376 行(12 个顶层模块 + 9 个 mcp 子模块),无独立 tests/ 目录,全部回归内联 `#[cfg(test)]`。

## 1. 职责与边界

- **内置工具**:九个 `pawork_domain::AgentTool` 实现;文件类工具的一切路径输入都是 `workspace_id + relative_path`,统一经 `pawork-policy` 的 `resolve_workspace_path` 解析,模型无法用绝对路径直达文件系统。computer 工具经 `pawork-computer-use` 访问进程共享的隔离虚拟桌面,不触达宿主键鼠。
- **调度**:`ToolRegistry` 是唯一注册表(内置与 MCP 工具同表);`ToolScheduler::execute_named` 串起「查表 → Policy 裁决 → 审批解析 → 全局信号量 → 超时包裹 → 执行」。
- **MCP**:配置解析与校验、受管客户端(惰性连接/指数退避/请求超时/取消)、能力发现与 `{server}.{tool}` 命名空间注册(非法字符折叠为 `_`)、stdio 服务器强制经 Sandbox Runtime 托管、Secret 只存 locator(`SecretRef`)、PKCE OAuth。
...
| `pawork-computer-use` | `Computer`/`Action`/`Backend`(隔离虚拟桌面会话,computer 工具后端) |

**被哪些包依赖**:仅 `pawork-app`(crates/app;app_core.rs / approval.rs / extensions.rs / loop_ctx.rs / gui_host 引用 `pawork_tools`)。engine 不直接依赖本包(全仓 Cargo.toml 扫描确认)。
...
| `base64` | computer 截图 JPEG → Base64 编码 |
...
| 路径 | 行数 | 职责 |
| --- | --- | --- |
| src/lib.rs | 30 | 门面:12 个模块声明 + re-export(九工具、scheduler 全家、`pub mod mcp`) |
| src/common.rs | 183 | 公共层:`BuiltinToolError` → `ToolError` 集中映射;取参 helper;`workspace_roots`;`resolve_write_rel`;`atomic_write` |
| src/computer.rs | 309 | `ComputerTool`:进程共享隔离桌面(OnceLock)、按 action 必填参数、跨 run 观察隔离、JPEG → canonical Image、取消即停后台任务 |
| src/read_file.rs | 480 | `ReadFileTool`:行号视图、offset/limit、编码探测、二进制检测、读/输出上限 |
| src/list_directory.rs | 453 | `ListDirectoryTool`:目录优先字典序、BinaryHeap 单扫描分页、symlink 信息 |
...
| src/scheduler.rs | 1157 | `ToolRegistry`/`ToolScheduler`/`ApprovalResolver` 闸门流水线(约 420 行逻辑 + 测试) |
| src/mcp/mod.rs | 247 | MCP 边界类型(`McpError`/`McpToolInfo`/`McpPeer`)+「公开源码不得出现 rmcp」守卫测试 |
| src/mcp/capabilities.rs | 740 | 能力桥:discover、`McpToolAdapter`(MCP→AgentTool)、注册与白名单闸 |
...
| `opt_str` / `opt_u64` / `opt_bool` | fn | 取可选字段(缺省/显式 null → None;类型不符 → `InvalidField`) |
| `workspace_roots(service, id)` | fn | 未知 workspace → `WorkspaceError::NotFound` |
| `resolve_write_rel(roots, rel)` | fn | 全部文件/命令工具(含只读)解析路径的统一入口,返回绝对路径 |
...
### 4.2 九个内置工具(descriptor 语义与 execute 行为)

统一形状:实现 `AgentTool`(`descriptor()` + `execute(request, context, sink, cancel)`);文件/命令工具构造函数均 `new(WorkspaceService)`,computer 为 `new(Arc<Computer>)` / `default()`(进程共享 `Computer::isolated()`)。八个文件/命令工具全部 `requires_approval=false`(是否询问完全由 scheduler 按 ApprovalMode + capability + trusted 裁决);四个只读工具 `allowed_in_untrusted_workspace=true`,四个副作用工具为 false。
...
| `run_command` | Process | 否 | **30s(descriptor 自带,见 §5)** / 8 MiB | 见下方专列 |
| `computer` | ExternalPlugin | 否 | 15s / 768 KiB | `action` + 按 action 必填字段;**requires_approval=true**;输入上限 16 KiB |
...
- **list_directory**(私有 `list_dir`,`spawn_blocking` 执行):非目录 → `Other` 错误。单次 `read_dir` 扫描,BinaryHeap 只保留前 `offset+limit` 个最小项(大目录内存 O(offset+limit))仍得准确 total;排序 `entry_cmp`=目录优先→名字字典序→kind。symlink:`symlink_metadata` 判定,follow 成功=`symlink`(size 取目标),失败=`broken_symlink`(size 取 lmeta);`safe_symlink_target` 把目标 canonicalize 后相对化到某个 root,越 root 省略。行格式 `{size:>8}  {kind}  {name}[ -> target]`;metadata.entries 为结构化数组。
- **find_files**(私有 `find`,`spawn_blocking` 执行):`WalkBuilder` follow_links(false) + ignore/git_ignore/git_exclude(true)(**默认跳过隐藏文件**);每 64 个 entry 查一次取消;命中 glob(rel 或完整 path 两形态)后逐项用 `resolve_write_rel` 复核,`.git` 与逃逸 symlink 静默跳过;结果字典序稳定排序;`max_results` 达到即 truncated。
- **search_text**(私有 `search`,`spawn_blocking`):`hidden(false)`(**搜隐藏文件**但仍尊重 .gitignore);glob 过滤同样 rel/path 双形态;逐文件 `read_to_string`(非 UTF-8 跳过);`Matcher` = Regex 或 Fixed(大小写不敏感时 lowercase);非法 regex/glob → InvalidInput;命中输出 `{rel}:{line}:{marker}{text}` + context 行,256 KiB 全局预算耗尽即提前返回已累积部分。
- **write_file**(私有 `write`):require path+content → resolve → `atomic_write`;metadata `{path, bytes}`。
- **edit_file**(私有 `edit`,`spawn_blocking` 执行):解析 segments(`edits[]` 优先,否则单段);`edits[]` 为空数组 → "no edits provided";任一 `old==new` → Conflict;全部段先在内存 `count_and_replace` 预演(0 命中 NotFound、>1 命中 Conflict 报命中数),任一段失败整体不落盘;净变化为零 → NotFound;成功一次 `atomic_write`。fuzzy 匹配:行对齐 whitespace 归一化(行 span 切分 + token 序列 KMP),命中区间是行首到行 content_end(保留行尾换行)。metadata = `EditReport{path, applied[], bytes}`。
- **apply_patch**(私有 `apply`,`spawn_blocking` 执行):`parse_ops`(未知 op → InvalidField);先对全部 op 的 path 与 to resolve(任一非法整体失败);dry_run 只返回 `PlannedChange` 清单不落盘;执行前 `snapshot_involved_paths` 做字节备份;逐 op 执行(create/update 走 atomic_write、delete 幂等、rename 建父目录);任一 op 失败 → `restore_backups` 恢复全部备份,以 `ApplyPatchError::Partial{failed_op, message, applied}` 报出已应用清单。
- **run_command**(私有 `run`):
  ...
  - env:`env_clear=true`;`default_env_allowlist()` + 宿主配置白名单 + 显式 `env{}` 全部进入 spec.env;`process_env_denylist()` 通配 deny(`*TOKEN*`/`*KEY*`/`*SECRET*`/`*PASS*`/`*AUTH*`/`*COOKIE*`/`*CREDENTIAL*`/`LD_PRELOAD`/`DYLD_*`/`NODE_OPTIONS`/`BASH_ENV`/`ENV`/`BASH_FUNC_*` 等,通配命中优先于 allowlist)。
  - 沙箱:手工构造 `SandboxPolicy`(filesystem read/write roots=workspace roots、deny=default_secret_paths()、network Enforce、allow_spawn、ResourceLimits 含 wall_time_ms=timeout_ms);`RunCommandTool::with_runtime` 供测试注入。
  - 取消:`bridge_exec_cancel` 把 domain token 桥到 exec token。
  - 执行:`SandboxSelector::pick()` 选后端(优先硬隔离),消费 ProcessEvent 流:Stdout/Stderr chunk 实时累计并经 sink emit `OutputDelta`(真流式),Exit 收口;成功 = exit 0。输出 stdout + 非空 `[stderr]` 段 + 非零 `[exit N]`;metadata 含 exit_code/字节数/truncated 与 **sandbox 块**(backend id、isolation、fallback、note、attempted、limits 全量)。
- **computer**(computer.rs):JSON 严格校验(`additionalProperties:false` + oneOf 按 action 必填);解析失败返回可纠正的参数要求且**不触达 backend**;输入 >16 KiB 拒绝。`spawn_blocking` 执行;future 超时/取消经 CancelOnDrop 局部 token 停止后台任务(虚拟桌面锁保持到后端释放输入)。scope = (workspace_id, run_id) 跨 run 观察隔离。成功截图返回 Text 元数据 + `ImageSource::Base64` JPEG(输出预算 768 KiB);每次输入返回投递事实,效果需新截图复验。

每个文件工具各带私有错误 enum(`ReadFileError`/`ListDirError`/…/`RunCommandError`),Cancelled 变体在 execute 顶层映射为 `ToolError::cancelled`,其余经 `BuiltinToolError` 集中映射;find_files/search_text 的 spawn_blocking JoinError → Internal;computer 直接映射 `pawork_computer_use::Error`(Permission→PermissionDenied、Invalid→InvalidInput、Stale→Conflict、Cancelled→Cancelled、Unsupported/Backend→ExecutionFailed)。
...
| `ToolScheduler` | struct | `new(registry, config)` / `tool_count` / `approval_mode` / `workspace_trusted` / `with_approval_snapshot(mode, trusted)`(克隆工具表建新实例,宿主 Arc-swap 用)/ `with_tools`(克隆既有 registry 与策略配置并追加 Host 工具)/ `execute_named(name, request, context, cancel, approval, sink)` |
| `ApprovalOutcome` | enum | `Approved` / `Denied`(**无 reason 字段**,拒绝文案由闸门统一生成) |
...
| `AutoApproveResolver` | struct | `can_resolve_policy_prompt()=false`(不能满足 policy AskUser);resolve 恒 Approved |
| `NoopToolEventSink` | struct | 丢事件 sink,测试与最小宿主用 |
| `GateOutcome` | enum(私有) | `Denied{reason}` / `Approved{asked_user}`——check_gate 的中间形态(spec 旧称 SchedulerError,源码无此类型) |

私有实现要点:

- `execute_with_tool` 主流程:取 executor → `check_gate`(Denied → `Ok(failure ToolResult, category=Authorization)`)→ **S2 审批钩子**(仅在 check_gate 为 Allow/AllowWithConstraints 且未问过用户时;调用方传入 resolver 即对**所有工具**再问一次,AutoApprove 恒过、DenyAll 全拒)→ cancel 预检 → `acquire` 全局 Semaphore 许可(`max_concurrent.max(1)`)→ descriptor 有 `default_timeout_ms` 则 `tokio::time::timeout` 包裹(超时 → `ToolErrorKind::Timeout`)→ 执行。
- `check_gate`:`PolicyEngine::decide(PolicyInput{capability, input, trusted, allowed_in_untrusted_workspace, approval_mode})`;若 descriptor `requires_approval=true` 且裁决非 Deny → **升级为 AskUser**;AskUser 分支要求 resolver `can_resolve_policy_prompt()==true` 否则 fail-closed 拒绝("automatic approval is forbidden";无 resolver 时 fallback 为 AutoApproveResolver → 同样拒绝);AllowWithConstraints → `apply_execution_constraints` 把 `timeout_ms`/`max_output_bytes` 注入 input(与已有值取更严者);Deny → 拒绝结果。
- `ToolHandle` RAII 持有 `OwnedSemaphorePermit`。
- `ProviderHosted`/`ProviderExtension` kind → `not_locally_executable` 错误。

**并发规则事实**:scheduler 执行层是**单一全局 Semaphore**,对所有 capability 一视同仁,不按 capability 限并发;"仅 ReadOnly 可并发"体现在 descriptor 层——`ToolCapability::permits_concurrent_execution()`(domain)只有 ReadOnly 为 true,MCP adapter 用它填 `supports_concurrency`,内置四只读 true、四副作用 false;`supports_concurrency` 在本包与 engine/app 中均无消费方,纯对外声明。
...
| 名称 | 种类 | 功能/语义 |
| --- | --- | --- |
| `namespaced_name(server, tool)` | fn | `{server}.{tool}` 拼接后把 `[A-Za-z0-9_-]` 之外字符折叠为 `_`(服务器名禁 `.` 保证无歧义;上游 Provider 拒绝带 `.` 的工具名) |
...
| `register_discovered_tools` | fn | 同步变体;注册期 `trusted &&= host_trusted`(MCP 配置 trusted 不得越过宿主信任地板) |

execute 行为:调用期双重校验——`allowed_workspaces` 非空时按 `context.workspace_id` 查,违规返回 Authorization 失败结果;`allowed_tools` 非空时按原始工具名查(与注册期过滤叠加);两次 cancel 检查;非 object input → InvalidInput;`peer.call_tool` 的 Cancelled → `ToolError::cancelled`、Timeout → Timeout(retryable=true)、其余 → 失败结果(category=Tool);最后 `apply_tool_result_budget` 限输出。
...
| `ClientPeer` | struct(Clone) | `server_capabilities()`/`list_tools()`(read_only 取 annotations.read_only_hint)/`ping()`/`call_tool(call, timeout, cancel)`(cancel 时对端发 MCP cancel 通知) |
| `tool_to_info` / `call_to_params` / `call_result_to_tool_result` | fn | 模型转换:text/image/resource-text 三类 ContentBlock;structured_content 进 `metadata.mcp.structured_content`;is_error → success=false + ErrorContext |
| `server_result_to_tool_result` | fn | InputRequiredResult → **fail-closed** Protocol 错误(不支持追加输入) |
| `apply_output_cap` / `apply_tool_result_budget` | fn | 硬字节上限,文本在 UTF-8 char boundary 截断,超限丢弃后续 part 并置 truncated;structured_content 编码后放不进剩余预算则整体丢弃 |
| `serve_stdio` / `serve_http` / `build_http_transport_config` / `validate_http_transport_config` | fn | 握手建立;http 配置校验(空 URL/空 token、auth_token 与 Authorization 冲突、明文 http 携密非 loopback 拒绝、重复 header) |
| `map_service_error` / `should_retry` / `timed` | fn | ServiceError → McpError 映射;只有 Disconnected 可重试;请求级超时包装 |
| `test_support::InProcessConnector` | mod(cfg(test)) | echo/slow/failing/delayed 四种 in-process 服务器 |
...
| `TransportSpec` | enum(serde tag=kind) | `Stdio{command, args, env: BTreeMap<String, SecretRef>}` / `Http{url, headers}`;校验:command 非空;http/https only、拒 URL userinfo 与 fragment、携密 header 明文 http 仅 loopback |
| `StdioSandboxRuntime` | struct | `{backend, policy, workspace_roots}`;roots 为空拒绝;`into_spawner` 建唯一 spawner(初次连接与重连复用) |
| `SecretResolvingConnector` | struct(impl McpConnector) | 每次 connect 解析 SecretRef → 分派 sandboxed_stdio 或 http;stdio 无 runtime fail-closed |
| `RestartPolicy` | struct | max_attempts(默认 1)/base_delay_ms(200)/max_delay_ms(10_000);校验 max≥1、base>0、max≥base |
| `McpPermissions` | struct | `allowed_tools`/`allowed_workspaces`(空集=不限制)、`max_output_bytes`(默认 1 MiB) |
| `validate_server_name` / `is_loopback_url` | fn | 服务器名非空且禁 `.`;loopback 判定 |
...
| `ManagedMcpClient` | struct(impl McpPeer) | `options()`/`health()`/`ping()`/`shutdown()`(5s 优雅关闭);lifecycle Mutex 串行化连接路径 |

私有实现要点:惰性连接——首个请求才 connect;`peer_with_cancel` 先查 connector `should_reconnect_before_request()`(OAuth bearer 轮换用);`reconnect` 循环指数退避(封顶防溢出),失败计数达 `max_attempts` 返回 Disconnected,耗尽后冷却 `4×max_delay` 才复位;connect 被请求超时约束;一切等待点同时 select shutdown 与调用方 cancel。
...
| `McpBearerProvider` | struct | `bearer()`:按 skew 自动 refresh 并回写轮换 token |
| `OAuthHttpConnector` | struct(impl McpConnector) | connect 前 `authorized_config`(拒绝已有 Authorization,注入 Bearer);bearer 轮换检测 → 强制重建 transport |
...
| `SandboxedStdioSpawner` | struct | 生产实现:走 `SandboxBackend::spawn_interactive`;cwd = working_dir 或第一 root;stdout 预算 8 MiB + 2×8192 slack |
| `SpawnedStdio{read, write}` | struct | `SandboxStdoutReader`(mpsc ProcessEvent → 字节流,stdout+stderr 共享 8 MiB 预算,超限 fail-closed 断连)/ `SandboxStdinWriter` |
| `apply_mcp_stdio_env_hygiene(&mut SandboxPolicy)` | fn | env_clear=true + untrusted allowlist + deny 追加 `PAWORK_API_KEY_*`;**不改 network_mode** |

#### mcp/security.rs / mcp/transport.rs

| 名称 | 种类 | 功能/语义 |
| --- | --- | --- |
| `SecretRef{service, account}` | struct | 只序列化 locator;`resolve(backend)`:service 必须 `pawork.mcp.*` 前缀(裸 stem 也拒),Provider/OAuth 域 fail-closed |
| `ResolvedSecret` | struct | `expose_secret()`;Debug/Display 恒 `[REDACTED]`;Drop 清零 String |
| `StdioTransportConfig` / `HttpTransportConfig` / `TransportConfig` | struct/enum(私有 mod) | 携密字段 Debug 手写 redact;`safe_url_for_debug` 打码 userinfo/query/fragment |
| `McpConnector` | trait(pub(crate)) | `transport_name()` / `should_reconnect_before_request()`(默认 false)/ `connect()` |
| `DefaultConnector` | struct | `http(config)` 公开;`sandboxed_stdio(config, spawner)` 仅 crate 内 |
...
- **审批语义(源码为准,与 Spec §4.1 存在差异)**:实际流水线是 ① policy decide → ② `requires_approval=true` 且非 Deny 时**升级为 AskUser** → ③ AskUser 必须由 `can_resolve_policy_prompt()==true` 的真实通道放行(`approval=None` 时 fallback AutoApproveResolver → 拒绝;**AutoApproveResolver 无法批准任何 requires_approval 工具**)→ ④ policy Allow/AllowWithConstraints 放行后,**只要调用方传了 resolver,S2 钩子对所有工具再问一次**(AutoApprove 恒过,DenyAll 则全部拒绝)——即"宿主注入 resolver = 每次调用都过 resolver"。Spec 所述"叠加闸仅 requires_approval=true 时生效、AutoApproveResolver 在此闸有效"与当前源码不符;Spec 亦未载 S2 通用钩子。
- **拒绝不是 Err**:policy Deny 与审批拒绝返回 `Ok(ToolResult{success:false, category:Authorization})`,模型可见、Agent loop 可继续;超时/取消/未知工具才是 `Err`。
- **超时与约束**:scheduler 用 descriptor.default_timeout_ms 静态值包裹执行;AllowWithConstraints 注入的 timeout_ms/max_output_bytes 只收紧不放宽。注意 `run_command` descriptor 自带 30s scheduler 超时,**即使 input 请求 600s,整个调用仍会在 30s 被 scheduler 切断**(Spec §3.1 写"descriptor 不设",源码为 `Some(30_000)`,以源码为准);exec 层 timeout_ms 到点则产出带 `timed_out` 标记的失败结果,两层语义不同。
...
- **Spec 与源码差异汇总**(以源码为准):① find_files timeout 10s(Spec 表写 30s);② search_text timeout 15s(Spec 写 30s);③ list_directory max_output_bytes 128 KiB(Spec 表写 256 KiB);④ run_command descriptor 设 30s(Spec 写不设);⑤ 审批流水线语义(上文,Spec §4.1 第 4 步与 §3.4 ApprovalOutcome/叠加闸描述);⑥ Spec 提及的 `SchedulerError` 类型不存在(实为私有 `GateOutcome`);⑦ Spec §6 称被 pawork-engine 依赖,实际仅 pawork-app 依赖本包;⑧ Spec §7 scheduler 测试清单中的「灾难地板命令 Deny」不在本包(shell 灾难地板断言位于 pawork-policy/src/shell.rs)。
...
| computer.rs | 批准后截图返回真实图像且序列化往返不重执行;缺字段 click 报可纠正参数且不触后端;未信任/只读/自动批准均不触碰虚拟桌面后端 |
...
| apply_patch.rs | 多文件 create;dry_run 不写;delete+rename;部分失败回滚;create/update/delete 各自恢复;proptest 字节精确回滚;op 路径穿越拒绝 |
...
| scheduler.rs | 只读并发峰值;全局并发上限强制串行;未知工具 NotFound;取消传播;超时映射;DenyAll 返回失败结果不执行;AutoApprove 不能过 policy prompt;registry kind/hosting 校验;snapshot 保旧改新;untrusted 写在 NeverAsk 下仍拒;AskForWrites 不能 AutoApprove 绕过;ReadOnly 模式 trusted 也拒写;NeverAsk+trusted 注入约束 |
| mcp/mod.rs | 内置与 MCP 同表;`public_sources_do_not_mention_rmcp` |
| mcp/capabilities.rs | 命名空间注册(非法字符折叠);只读透传;写工具 approval+untrusted 地板;trusted 需 host 双确认;预取消;输出截断;非 object 拒绝;structured_content 预算内保留;workspace 白名单;工具白名单注册期+调用期;is_error 转换;host 不信任钳制;未广播能力跳过 |
...
graph LR
    ...
    computeruse[pawork-computer-use<br/>Computer/Backend] --> tools
