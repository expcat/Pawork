...
Phase 4 remediation 后，索引键为 `run_id + tool_call_id`，不同 Run 的同名 Tool Call 不会碰撞。Run→change→Blob/path 映射以版本化 JSON 原子写入 Artifact Store 根目录；`CheckpointService::open` 在宿主恢复时重建映射。快照读取、冲突检查与恢复写入使用 Tokio 文件 IO，异步路径不再同步读取整文件。
