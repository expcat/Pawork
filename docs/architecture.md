...
| GUI 协议 | 帧格式带版本协商；`SUPPORTED_API_VERSIONS` 1.0–1.11（1.3 Terminal 生命周期 `terminal_close` + `TerminalExited`，按协商 minor 门控推送；1.4 Settings 认证；1.5 通用页；1.6 权限与审批；1.7 工具与 MCP；1.8 终端设置；1.9 Accepted 握手可选 `host_data_dir`；1.10 供应商级代理开关 `set_provider_use_proxy`；1.11 会话生命周期 `session_rename`/`session_archive`/`SessionMetaChanged`、`session_create.workspace_id` 可选化（ADR-054））；typegen 检入 [`schemas/`](../schemas/)（core-api/gui-protocol/headless-json）；三通道可用性单源 `protocol::app::registry`，未登记 fail-closed | 帧 golden + typegen 断言（`crates/protocol`） |
