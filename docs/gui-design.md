...
**尚未完成**：当前 GUI 历史投影 `join_text` 只取 `ContentPart::Text`，协议虽已有 `AppEvent::ThinkingDelta`，但 Host 转发、共享 reducer 与历史条目没有接通；`Thinking` 未单独传给 Desktop。因此本批不猜测哪些正文是思考，不宣称已支持思考折叠。需要另行明确思考内容的 GUI 投影契约；若演进冻结 wire/schema，按工程约定先确认。实现、自动检查、代理真窗口检查和用户人工验收分别见 [路线图](ROADMAP.md)。
