...
> 2026-09-02 SET-6a 已落地「通用」页（ADR-047）：Settings Rail 第二项，
> `general_settings` 查询成功解析后才显示；承载 Global 层 `proxy_url`
> 的读取 / 内联设置 / 清除，未设置时显示「未设置（跟随系统环境变量）」；
> 页面如实标注生效边界（新 OAuth / 验证 / 目录探测同会话生效，当前活跃
> 供应商的模型流量于切换供应商或重启 Host 后生效）；断线 stale 只读禁写，
> 重连后自动刷新恢复；可见 / 键盘 / AX 三路径同 gate。
>
> 2026-09-03 SET-6b 已落地「权限与审批」页（ADR-048）：查询成功后才显示；
...
- [x] Reconnect 恢复（P1 片 1/2C）：Session→Workspace、Task/Timeline/Changes 跨 Host 重启恢复；Terminal 进程不恢复但 workspace/cwd 诚实恢复。多项目注册表与按会话路由已由 P1 片 2B 落地，Desktop 五流程接线经片 2C 审计确认无缺口并通过真窗口验收（双粒度重开 + 断线诚实性），P1 收口；进行中 Run 的全应用生命周期进入 P2
