...
### UX-09 文案、字号反馈与终端信息（2026-09-10）

中文界面的回复作者、工具组、变更摘要与终端动作统一使用中文产品术语，设置页以 Pawork 服务 / 项目解释运行范围；保留诊断命令、原始错误与用户内容。新任务的宿主占位标题仅作显示本地化，不写回历史标题。

MCP 空态给出平台全局配置路径、配置节与重启刷新步骤；配置了服务器后再解释测试 / 移除。字号设置成功后的提示保留 3 秒，连续调整从最后一次重新计时；收起回收行高，不清除错误和项目限制，当前档位留在外观页。

终端将所属项目、工作目录与状态放到可滚动正文，避免挤压顶栏尺寸控件；输入与启动 / 关闭控件的无障碍命中框使用实际布局。沿用现有主题和焦点环；验证完成度见 [活动路线图 UX-09](ROADMAP.md#ux-09-视觉文案与可访问性一致性)。
