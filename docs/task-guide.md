...
以下 1–2 适用于 S0–S11 实现与 S13 整改任务。S12 按其任务书只检查审查报告、finding 证据、ROADMAP 回写和文档 diff，不运行测试或冒烟。
...
- **当前 S0–S13 不做**：Workspace Full Gate、L0–L3 分级、clippy/fmt 门禁、schema drift CI、三平台矩阵、覆盖率。S12 按 [任务书](../plan/S12-project-code-review.md) 只做静态审查与 finding 登记，不执行这些门禁；S13 整改沿用 S0–S11 定向约定，同样不执行；未来若需要发布，另立任务重新定义。
- **平台策略**：S0–S11 与 S13 整改项在当前可用平台实测；其他平台代码随迁移进入但只做交叉 `cargo check`（可选）。三平台实跑不再默认挂靠 S12。
- **与根 [../AGENTS.md](../AGENTS.md) 的关系**：根文件 §5 已明确 S0–S11 的定向验证、S12 的只读边界与 S13 的整改约定；其余章节（架构红线、命名、提交与分支、安全与权限、子代理使用）全量适用。
...
- **S13 收尾**：逐项回写 ROADMAP §3.2 状态列（🟢 + 日期 + 证据链接）；全部收口后 §3.1 归档一条 S13 记录，未完成决策/延期项转 §4。S13 已于 2026-08-18 收口，事实源为 ROADMAP §3.1「S13 收口」行。
