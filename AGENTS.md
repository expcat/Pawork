...
- **按写入集加载 Spec**：进某包前读 [docs/spec/crates/](docs/spec/README.md) 该包一篇；不要一次读完全部 Spec。跨包链路（Agent loop / GUI 连接 / 事件持久化与重放 / 凭证与脱敏）读 [docs/spec/flows.md](docs/spec/flows.md) 对应一条。
...
- `apps/desktop` 直接 `pawork-*` 依赖只允许 `pawork-client`、`pawork-terminal`（终端显示核心纯库）与 `pawork-browser`（系统网页视图）。
