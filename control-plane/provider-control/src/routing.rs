...
use pawork_api::{ModelCapabilities, ReasoningStateDescriptor};
use serde::{Deserialize, Serialize};

use crate::health::{CooldownKey, FailureContext, HealthRuntime, HealthState};

/// 路由所需能力（canonical，可自 [`pawork_api::ModelCapabilities`] 映射）。
...
mod tests {
    ...
    impl Clock for MutableClock {
        fn now(&self) -> pawork_domain::Timestamp {
            pawork_domain::Timestamp::from_unix_millis(self.0.load(Ordering::Relaxed))
        }
    }
