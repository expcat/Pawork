...
//!
//! 本模块只依赖 `pawork-sqlite` 迁移运行器与 rusqlite，不把控制面行为类型
//! 拉进 schema（扁平行 + SQL；领域类型仍在本 crate 其它模块）。

use std::path::Path;

use pawork_sqlite::{
    migrate as run_migrate, schema_version as read_schema_version, DatabaseActor, Migration,
...
pub async fn schema_version(database: &DatabaseActor) -> Result<u32, MigrationError> {
    read_schema_version(database, CONTROL_PLANE_MIGRATIONS_TABLE).await
}
