...
//! - reasoning protector **复用** `pawork_provider_core::ReasoningProtector`（删除本模块
...
//! - 不扩张 [`pawork_api::ModelProvider`] contract（不新增账号 / 租户 / 客户端职责）。

use std::collections::HashMap;
use std::sync::atomic::{AtomicU64, Ordering};
use std::sync::{Arc, Mutex};

use pawork_domain::{AccountId, ProtectedBlobRef, ProviderId, SessionId, TenantId};
use async_trait::async_trait;
use pawork_api::{ModelDefinition, ModelProvider, ResolvedCredential};
...
/// `pawork_provider_core::ReasoningProtector`。本 trait 只表达「按 scope 构造」的
...
impl ReasoningProtector for InMemorySessionProtector {
    async fn protect(
        &self,
        payload: &[u8],
    ) -> Result<ProtectedBlobRef, pawork_provider_core::ReasoningProtectError> {
    ...
    ) -> Result<Vec<u8>, pawork_provider_core::ReasoningProtectError> {
        // V2 `ReasoningProtector::resolve` 返回 `Vec<u8>`（V1 返回 `ProtectedBlob`）。
        // 需要零化包装时由调用方用 `pawork_blob_store::ProtectedBlob::new`；
        // 不静默丢掉该类型，见 [`wrap_protected_blob`]。
        let expected = Self::prefix(&self.scope);
        if !blob_ref.as_str().starts_with(&expected) {
            // 跨 scope 访问：fail-closed（不泄漏存在性）。
            return Err(pawork_provider_core::ReasoningProtectError::Unavailable);
        }
        self.blobs
            .lock()
            .expect("protector blobs mutex poisoned")
            .get(blob_ref)
            .cloned()
            .ok_or(pawork_provider_core::ReasoningProtectError::Unavailable)
    }
}
...
fn map_credential_kind(kind: CredentialKind) -> pawork_api::CredentialKind {
    match kind {
        CredentialKind::ApiKey => pawork_api::CredentialKind::ApiKey,
        CredentialKind::OAuth => pawork_api::CredentialKind::OAuthBearer,
        CredentialKind::Other => pawork_api::CredentialKind::SessionToken,
    }
}

/// V2 `ReasoningProtector::resolve` 返回 `Vec<u8>`；需要 V1 零化包装时用此辅助。
///
/// 不静默丢掉 [`pawork_blob_store::ProtectedBlob`]：trait 边界已接上 V2，
/// 该类型仍作为可选包装保留。
pub fn wrap_protected_blob(bytes: Vec<u8>) -> pawork_blob_store::ProtectedBlob {
    pawork_blob_store::ProtectedBlob::new(bytes)
}
...
mod tests {
    ...
    fn lease(provider: &str, session: &str) -> CredentialLease {
        CredentialLease {
            ...
            credential_id: pawork_domain::CredentialId::new("cred-1"),
            account_id: pawork_domain::AccountId::new("acct-1"),
            provider_id: ProviderId::new(provider),
            agent_id: pawork_domain::AgentId::new("agent-1"),
            session_id: SessionId::new(session),
            principal_id: pawork_domain::PrincipalId::new("principal-1"),
            tenant_id: pawork_domain::TenantId::new("tenant-a"),
            acquired_at_ms: 0,
            expires_at_ms: 0,
            version: 2,
        }
    ...
    fn descriptor(provider: &str) -> ProviderDescriptor {
        ProviderDescriptor {
            ...
            builtin_models: vec![ModelDefinition {
                id: pawork_domain::ModelId::new("model-1"),
    ...
    impl ModelProvider for NoopProvider {
        ...
        ) -> Result<pawork_api::ModelResponseSummary, ProviderError> {
    ...
    fn factory(
        ...
        let factory = InMemoryProviderFactory::new(
            ...
            Arc::new(crate::account::FixedClock::new(
                pawork_domain::Timestamp::from_unix_millis(1_000_000),
            )),
        );
    ...
    fn openai_metadata() -> crate::account::CredentialMetadata {
        ...
        m.tenant_id = pawork_domain::TenantId::new("tenant-a");
        m.account_id = pawork_domain::AccountId::new("acct-1");
        m.provider_id = ProviderId::new("openai");
        m.credential_id = pawork_domain::CredentialId::new("cred-1");
        m.synthetic = false;
    ...
    async fn distinct_sessions_never_share_a_global_protector_scope() {
        ...
        assert_eq!(
            wrap_protected_blob(resolved).expose(),
            b"reasoning-continuation-bytes"
        );
    }
    ...
    async fn compose_rejects_missing_descriptor_builder_and_cross_tenant() {
        ...
        let lease_mistral = CredentialLease {
            ...
            schema_version: crate::CONTROL_PLANE_SCHEMA_VERSION,
            credential_id: pawork_domain::CredentialId::new("cred-x"),
            account_id: pawork_domain::AccountId::new("acct-1"),
        ...
        cross.tenant_id = pawork_domain::TenantId::new("tenant-other");
    ...
    async fn compose_rejects_credential_not_usable_for_take() {
        ...
        expired.expires_at = Some(pawork_domain::Timestamp::from_unix_millis(999_999));
        assert!(matches!(
