//! Pawork 长期记忆：只读提炼、canonical EmbeddingProvider 检索注入。
//!
//! **experimental**：本 crate 提供 Provider 无关的记忆抽象与进程内存储，但尚无
//! 真实 Embedding 实现，也未接入 context-engine / host / CLI。激活条件由主代理
//! 登记 ROADMAP §4。本包**不**以 default-off feature 门控——定向测试
//! `cargo test -p pawork-memory` 默认可跑（测试内 Mock Embedding）。
//!
//! 跨会话长期记忆 service。记忆从历史 canonical 事件只读提炼，**绝不修改 / 删除
//! 任何输入事件**；记忆文本经 canonical [`EmbeddingProvider`](pawork_api::EmbeddingProvider)
...
//! - 记忆记录 / 失效为 canonical [`MemoryEvent`](pawork_domain::MemoryEvent)，可持久化、
//!   可重放；失效为 `valid=false` 而非删除，保留可追溯。
//! - 跨 workspace 默认不共享：`WorkspaceLocal` 受 `workspace_id` 过滤，`Shareable` 可跨。
//!
//! 具体领域类型与事件载荷由 `pawork-domain` 的 workflow 与 [`AgentEvent`](pawork_domain::AgentEvent) 提供。
