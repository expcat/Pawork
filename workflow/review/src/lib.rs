//! Pawork Review Engine：行锚点评审、re-anchor 与 resolution 生命周期。
//!
//! 职责：
//! - 行锚点评审意见（`file:line`）：编辑后按邻近行内容指纹 re-anchor，
//!   漂移时标 `stale` 而非静默失效；
//! - resolution 生命周期 `open → addressed → resolved | wontfix`，以
//!   `pawork_domain::ReviewEvent` 持久化（经 `pawork_domain::AgentEvent::Review`
...
/// canonical 评审类型重导出（来自 `pawork-domain`，只读消费）。
