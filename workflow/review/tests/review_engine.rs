...
use pawork_domain::{
    AgentEvent, ReviewAnchor, ReviewFindingId, ReviewResolution, ReviewSessionId, ReviewSeverity,
    WorkspaceId,
};
use pawork_git::diff::{DiffFile, DiffHunk, DiffLine, FileStatus, HunkId, LineKind};
use pawork_review::*;
...
fn replay_rejects_invalid_event_sequences() {
    ...
    let orphan = ReviewEvent::FindingOpened {
        ...
        finding_id: ReviewFindingId::new("f1"),
        anchor: ReviewAnchor {
            file: "a.rs".to_string(),
            line: 1,
            end_line: None,
...
fn review_event_round_trips_through_agent_event_wrapper() {
    ...
    let wrapped = AgentEvent::Review(event.clone());
    ...
    let wrapped = AgentEvent::Review(resolved);
    let json = serde_json::to_string(&wrapped).unwrap();
