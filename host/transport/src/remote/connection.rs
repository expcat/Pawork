...
//!   （见 [`super::session::SendWindow`]）。
//! - Ack 载荷携带被确认帧的 payload 摘要（[`super::wire::encode_ack`]）；
//!   服务端只接受本连接实际发送且摘要一致的确认，恶意 / 跨会话确认按协议
//!   违规断开（见 [`super::session::AckError`]）。
...
use crate::{ConnectionInfo, TransportError, TransportErrorKind, TransportFrame};

use super::session::{AckError, EndpointState, SendWindow};
use super::wire::{self, connection_closed, transport_error, FrameKind};
...
/// [`super::RealRemoteTransportConfig::max_buffered_bytes`] 提供）。
#[derive(Debug)]
...
impl crate::GuiConnection for ServerConnection {
...
#[async_trait::async_trait]
impl crate::GuiConnection for ClientConnection {
    async fn send(&self, frame: TransportFrame) -> Result<(), TransportError> {
        let _send_guard = self.send_lock.lock().await;
