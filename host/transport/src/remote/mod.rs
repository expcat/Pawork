...
use pawork_protocol::client_auth::{ClientAuthError, Token, TokenStore};
...
use crate::{
    ...
    TransportEndpoint, TransportError, TransportErrorKind,
};
use crate::{
...
const DEFAULT_MAX_FRAME_BYTES: u64 = crate::DEFAULT_MAX_FRAME_BYTES;
/// 默认有界重放窗口（帧数）。
...
fn parse_endpoint_address(address: &str) -> Result<ParsedAddress, TransportError> {
    ...
    let fingerprint = tls::parse_fingerprint(fingerprint_hex)
...
impl RealRemoteTransport {
    ...
    pub(crate) async fn publish_endpoint(
        ...
        loop {
            ...
            let identity = tls::generate_identity(&id)?;
            let address = format!(
                "real://{id}?fp={}#tcp={}:{}",
...
mod tests {
    ...
    fn endpoint_address_round_trip_and_rejections() {
        let identity = tls::generate_identity("addr").expect("identity");
        let address = format!(
        ...
        assert_eq!(
            parsed.fingerprint,
            tls::parse_fingerprint(&identity.fingerprint_hex).expect("fp")
        );
