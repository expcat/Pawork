...
use pawork_channels::acp::wire::{ERROR_INVALID_PARAMS, ERROR_INVALID_REQUEST, ERROR_METHOD_NOT_FOUND};
use pawork_channels::acp::{AcpClientAdapterFactory, CwdResolver, JsonRpcMessage, SessionResolver};
...
async fn negotiated_adapter() -> Arc<pawork_channels::acp::AcpClientAdapter> {
    ...
    let factory = AcpClientAdapterFactory::new(
        ...
        pawork_channels::acp::wire::Implementation {
...
async fn golden_session_new_request_matches_fixture() {
    ...
    let params = serde_json::from_value::<pawork_channels::acp::wire::SessionNewParams>(
...
fn golden_session_prompt_request_matches_fixture() {
    ...
    let params = serde_json::from_value::<pawork_channels::acp::wire::SessionPromptParams>(
        message.params.clone().unwrap_or(Value::Null),
    )
...
async fn golden_permission_response_selected_matches_fixture() {
    ...
    assert_eq!(
        decision,
        pawork_channels::acp::PermissionDecision::Selected {
...
async fn golden_permission_response_cancelled_matches_fixture() {
    ...
    assert_eq!(decision, pawork_channels::acp::PermissionDecision::Cancelled);
}
...
async fn session_resume_omitted_builder_defaults_are_accepted() {
    ...
    let params = serde_json::from_value::<pawork_channels::acp::wire::SessionResumeParams>(
        message.params.clone().unwrap_or(Value::Null),
    )
