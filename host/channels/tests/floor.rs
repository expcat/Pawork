...
use pawork_channels::acp::wire::{ERROR_INVALID_REQUEST, ERROR_RESOURCE_NOT_FOUND, PROTOCOL_VERSION};
use pawork_channels::acp::{AcpCommandHost, AcpHost, JsonRpcError};
...
async fn session_new_rejects_cwd_outside_registered_workspaces() {
    ...
    assert_eq!(error.code, pawork_channels::acp::wire::ERROR_INTERNAL);
    assert!(
        error
            .message
...
async fn drained_outbox_barriers_must_be_released_after_partial_write() {
    use pawork_channels::acp::{OutboxItem, PromptResolution};
