...
Pawork 用 Rust 从零实现一个编码智能体（Coding Agent）平台核心。它以 Pi 的功能、工作流与交互习惯为参考，但**不复用**其 TypeScript 实现：**CLI 与 Rust Core 是同一个程序和进程边界**（二进制 `pawork`），GUI（后续 Tauri + React）作为独立进程，经 CLI 暴露的 GUI Connection Protocol 连接 Core。
