...
use pawork_sdk::mock::MockTransport;
use pawork_sdk::{BackpressurePolicy, PaworkClient, PaworkOptions, SdkError, SdkErrorKind};
...
async fn connect(mock: &MockTransport) -> PaworkClient {
    PaworkClient::from_transport(
        Box::new(mock.clone()),
        PaworkOptions {
            client_name: "pawork-sdk-test".into(),
...
async fn handshake_exposes_version_instance_and_capabilities() {
    ...
    assert!(capabilities.contains(&pawork_protocol::headless::SdkCapability::Sessions));
    assert!(capabilities.contains(&pawork_protocol::headless::SdkCapability::CompatImport));
    ...
    assert_eq!(hello["client_name"], "pawork-sdk-test");
}
