...
> 状态：🟢 R4 已收口（Wave A 🟢 2026-08-28 / Wave B 🟢 2026-08-28；拍板 1：State A/B 分区 SSIM ≥0.99 移交 R8；wire 演进仍开放）· 🟢 R5 已收口（Wave A 🟢 2026-08-28 / Wave B + U2 🟢 2026-08-29；Composer SSIM 经用户确认移交 R8）· 🔵 R6 进行中（Wave A render / AX / U1 🟢 2026-08-29；Connected State A/B 证据 🔴 环境阻塞）
...
> 波次状态：
> - **Wave A（🔵 2026-08-29）**：实现侧已收敛 Inspector 顶层 strip 与 Changes Files/Summary 二级 strip 的层级/默认落点，并将折叠态 Activity 从 StatusBar 迁到 Workspace Header 右上向下展开约 320px；render/AX/U1、长内容不挤窄 Inspector 与 Header Activity AX 锚点公式回归 132/132 通过，GLM 只读审查无 P0/P1、唯一 P2 已补回归，只展示权威 Changes 摘要，不伪造 Add tool / Agent capability。自动 State A 两次因外部 AX 只返回递归 `AXApplication` 超时，手工 GUI 未进入 Connected；须按 [证据说明](../docs/ui-review/r6-wave-a/notes.md) 补齐同 fixture State A/B 后收口。
