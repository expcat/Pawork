...
> 状态：🟢 R4 已收口（Wave A 🟢 2026-08-28 / Wave B 🟢 2026-08-28；拍板 1：State A/B 分区 SSIM ≥0.99 移交 R8；wire 演进仍开放）· 🟢 R5 已收口（Wave A 🟢 2026-08-28 / Wave B + U2 🟢 2026-08-29；Composer SSIM 经用户确认移交 R8）· 🔵 R6 进行中（Wave A 🟢 2026-08-29；Wave B 实现/定向门禁完成，最终二进制 U2 补录受 AX flake 阻断）
...
> - **Wave B（🔵 2026-08-30）**：冻结 wire 内实现已收口：Changes latest-session/scope/真实横滚、Terminal 多 workspace 草稿与 snapshot/replay/reconnect、Resources stale/刷新、Inspector 键盘/AX，以及 Host 重启后的 GuiClient 请求幂等碰撞。app 178、Desktop 144、client 41、driver unittest 6 全绿；审查前 U2 九场景 19 断言全 PASS。审查补修两个边角后，唯一完整复跑在业务场景前被既有 macOS AX 注册 flake fail-closed，故保持蓝态等待补录最终二进制证据。详见 [r6-wave-b](../docs/ui-review/r6-wave-b/notes.md)。
...
- [x] Changes/Terminal/Resources 的展示范围与真实 capability 一致，无越权、假动作或错误 scope（审查前 U2 九场景 + 审查后定向回归）。
