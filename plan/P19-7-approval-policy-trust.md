...
**涉及范围**：Approval Center、inline approval card、Policy explanation、Workspace Trust/Tenant policy UI、revision conflict handling

## 细分步骤

1. **审批模型** —— 展示 tool/command、capability、workspace/tenant/account scope、requested by、风险理由与过期时间。目的：决策有上下文。
...
7. **a11y/security tests** —— focus trap、倒计时、陈旧审批、恶意长文本、Secret redaction、多窗口 race。目的：高风险路径可复核。
...
- [ ] shell/env/URL/tool args 的 Secret 在 DOM、日志与截图 fixture 中脱敏
