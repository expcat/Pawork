...
2. **安装 / 更新 / 卸载** —— 目的：install 拉取归档→校验→解包到 workspace / global；update 比对版本→按策略升级；uninstall 清理资源并注销，受 `resource-loader` 作用域约束。
...
7. **定向 / Mock 测试** —— 目的：安装 / 更新 / 卸载流程、签名失败拒绝、pin 生效、team policy 拦截越权安装、多 source 聚合。仅定向 + Mock（用 mock registry）。
