...
> Phase 14 · 模型用量与额度监控 · 状态：🟡未开始 · 依赖：P2-9、P2-7、P6-4
...
2. **定义额度快照领域模型** —— 目的：统一「剩余 / 已用 / 总量 / 重置时间 / 数据时间戳」口径，覆盖请求次数、token、金额三种计量维度。包含 `QuotaSnapshot`、`QuotaMeasure`（Count / Token / Cost）、`QuotaUnit`、`Remaining`（含无限额度与未知总量的语义）。
