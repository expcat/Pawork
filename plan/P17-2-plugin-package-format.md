...
**最终目的**：定义统一的 Plugin Package 格式——一个可安装包（manifest + 归档）可聚合多种扩展类型：Skills、Agents（profile）、Hooks（用户钩子）、MCP server 声明、LSP server 声明。让一次安装即可交付一个完整能力组合，避免用户手动逐项配置。Package 仅做**聚合、校验、作用域绑定**，复用各类型既有的子 manifest，不重定义其语义。

**涉及范围**：新增 `plugin-package`；复用 `resource-loader`（子资源加载）、`agent-domain`（类型）

## 细分步骤

1. **Package manifest schema** —— 目的：定义 package 清单（id / version / license / entrypoint），子段声明 skills / agents / hooks / mcp / lsp 各自的相对路径或内联清单，复用各子类型既有 schema 不重定义。
...
5. **与各 loader 集成** —— 目的：解包后把子资源分发到对应 loader（skills→P8-3、hooks→P17-1、mcp→P9、agents→P8-5、lsp→P17-4），单一入口安装。
...
- [ ] 一个 Package 可包含 Skills / Agents / Hooks / MCP / LSP 五类并一次安装
- [ ] 归档带内容 hash 校验，损坏 / 篡改可检测
- [ ] 跨类型 / 跨包冲突可检测并报错
- [ ] 子资源正确分发到各 loader，作用域与 `resource-loader` 一致

**相关文档**：[plugins](../docs/features/plugins.md) · [skills](../docs/features/skills.md) · [P17-1 User Hooks](P17-1-user-hooks.md) · [mcp](../docs/features/mcp.md) · [P17-4 LSP Runtime](P17-4-lsp-runtime.md) · [ROADMAP](../ROADMAP.md)
