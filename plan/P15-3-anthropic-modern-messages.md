...
- [ ] thinking `signature` 只经 Protected Blob Store 引用往返，多轮 extended thinking 连续（P15-7 / ADR-032）
- [ ] server tool 续接走 `ProviderTranscript`；只有客户端 `tool_use` 的 Core 结果可映射 `tool_result`
