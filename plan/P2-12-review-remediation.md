...
13. **plan/docs 同步**：11 篇 `plan/P2-*.md` 状态回填 🟢、19 个验收框勾选；`providers.md` 补 `include_usage` 与 stop reason 语义说明。目的：纠正违反 AGENTS.md §4 的流程偏差。
...
- [ ] **V1**：慢速长流（>60s）不被超时掐断（契约用例）
- [ ] **V2**：预取消不发出请求（wiremock 命中计数 0）；mid-stream 取消用例读到 delta 后取消
- [ ] **V3**：请求体含 `stream_options.include_usage`，尾部 usage-only chunk 正确归一（断言请求体字段）
- [ ] **V4**：`list_models` 请求头含 Authorization（契约断言）
- [ ] **V5**：`[DONE]` 无 finish_reason 时 stop_reason 非 Error（用例）
- [ ] **V6**：provider_options 命中保留键被忽略并告警（测试）
- [ ] **V7**：解析器 buf 超 1 MiB 发错误并重置；非法字节批量移除（不退化 O(n²)）
- [ ] **V8**：`ExponentialBackoff` 已删除，生产退避仅 agent-engine 一处
- [ ] **V9**：`resolve("GPT-4O")` 与 `resolve("gpt-4o")` 等价（测试）
- [ ] **V10**：`tests/contract.rs` 无调试 println
