...
| D13 | K-08 `ArtifactStreaming` 宣告 | 双端停止宣告 + client `experimental` 门控删除;artifact 流式转候选,R3 registry 就位后接线 | 服务端 `host/gui-server/src/session.rs:462` `ArtifactRead` 固定 unsupported,而握手在 `host/cli/src/gui.rs:71` 宣告能力、`clients/gui-client` `capabilities`:79 无条件包含;desktop/probe(`apps/protocol-probe`)均未启用门控(2026-08-18 重验) |
...
| D15 | `control-plane/core` OTel exporter(AuditExporter/OtelAuditExporter/InMemoryOtelExporter/TracingAuditExporter)、identity_schema(484 行)、rbac 的 PermissionProfile/PrincipalRole(~150 行) | OTel/identity_schema 归档;rbac 保留 `Permission`(orchestration `spawn.rs:11` 在用)其余归档 | 外部零生产调用;`PermissionProfile`/`PrincipalRole` 被 orchestration 测试引用(`supervisor/mod.rs:457`),归档须同步改测试(2026-08-18 重验) |
