...
**涉及范围**：Desktop Settings、Provider/Model catalog、ProviderAccount/Auth flows、Health/Route explanation、Usage/Quota views、Tenant policy presentation
...
3. **Auth flows** —— API Key 通过受控一次性输入提交，OAuth PKCE/device/callback 显示 state/expiry/cancel；成功后清空 DOM。目的：Secret 不停留 renderer。
...
7. **Secret/security tests** —— DOM/localStorage/log/screenshot/crash fixture 扫描，过期 OAuth、多账号 race、stale quota。目的：高风险验收。
...
- [ ] API Key/OAuth 成功或取消后敏感输入从 DOM/state/log 清除
