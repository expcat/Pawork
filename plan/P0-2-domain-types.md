...
5. **守住纯领域约束** —— 默认构建仅依赖 serde/serde_json + 标准库，无 IO/HTTP/DB/GUI framework；`ts-rs` 仅作为 `typegen` 可选 feature。目的：满足架构红线与 ADR-001，并允许协议类型生成。
...
- [x] 类型覆盖 文本/图片/Thinking/ToolCall/ToolResult/ArtifactRef
- [x] 无任何禁依赖（GPUI/Tauri/SQLite/HTTP/Git/具体 Provider）
- [x] 全部类型可 serde 序列化
