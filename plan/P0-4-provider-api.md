...
3. **定义 ModelProvider Trait** —— `async fn stream(req) -> Stream<ProviderStreamEvent>`。目的：所有 provider 的统一抽象。
...
- [ ] Trait 不绑定 HTTP/Tauri
- [ ] canonical 覆盖 text/tool/image/thinking/usage/stop
- [ ] 错误可分类（可重试/不可重试/超时/限流/鉴权）
