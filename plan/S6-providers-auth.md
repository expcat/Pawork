...
把 S0–S5 的两条开发测试通道扩展为六条首发产品通道：ChatGPT OAuth、xAI Grok OAuth、Z.AI GLM Coding Plan API key、OpenCode Go API key、Qwen Token Plan API key、DeepSeek API key。`pawork models` 聚合已配置通道；API key 与 OAuth 凭证进入 `$PAWORK_HOME/auth.json` / `~/.pawork/auth.json` 文件后端（0600、跨进程 write/refresh 锁、独立临时文件+rename 原子写；OAuth 多条目批量单次提交；损坏 fail-closed）；环境变量只作 headless/CI fallback；全局脱敏确保 secret 不入日志。
...
| `pawork-auth`（providers/auth） | 激活：auth 文件后端、OAuth PKCE/Device/refresh/callback、credential、masked；凭证解析链为 auth 文件 → env fallback → 无凭证 | 波 B 迁移 V1 `auth-service`，收口按用户决策用文件后端替代 Keychain，并只接六条首发通道 |
...
2. **auth 链路**：API key 的 `set-key` 与 ChatGPT/xAI OAuth 登录/刷新都落 auth 文件；env fallback 只在文件无条目时启用，并在 `auth list` 标注来源。
...
- [ ] ChatGPT/xAI OAuth refresh：在不伪造 mock 冒烟、不破坏现有 refresh token 的前提下，以临期真实凭证各触发一次请求前刷新；确认轮换后的 access/refresh/default meta 原子写回、旧会话可续用，永久失败明确要求重新登录。
- [x] 四条 API-key 通道：`pawork auth set-key <provider>` 后清除对应 env，分别完成一次流式工具任务。（2026-08-15 完成：glm-coding / opencode-go 此前已通过；qwen-token-plan / deepseek 从 `.env` 注入后 `set-key` → 清 env → `auth list` file 来源；`qwen3.8-max` 完成 `read_file` 工具任务（cache read 2048）与纯文本流式对话；`deepseek-chat` 完成 `read_file` 工具任务（cache read 2560）与纯文本流式对话。Token Plan 线上目录已换代，静态条目由过期的 `qwen3-coder-plus` 更新为 `qwen3.8-max`）
- [x] `pawork auth list` 只显示掩码与来源，不显示 token/key/account secret。（auth 文件命中显示掩码，env/none 不显示值）
...
- 决策前历史：macOS 未签名 dev 构建每次重编后二进制 cdhash 变化，读取此前创建的 Keychain 条目会触发 SecurityServer GUI 授权框并在 headless 下阻塞；签名发布构建不受影响。冒烟中创建的 `pawork.glm-coding` / `pawork.opencode-go` 两个 default 条目已于 2026-08-15 用 `security delete-generic-password` 删除并验证：`auth list` 恢复 env 来源、无 env 时 fail-closed，且经 env 直通完成一次真实流式对话。
- 决策前历史：wave B 的 keyring 依赖最初未启用平台后端（默认 mock 存储，store 后 get 即 NotFound）；曾按 macOS/Linux/Windows target 分别启用 `apple-native` / `sync-secret-service` / `windows-native` 并完成跨进程实测，随后由下一条用户决策整体取代。
- 用户决策（2026-08-15）：不使用 macOS Keychain 及任何系统机密存储，凭证统一走文件后端 `$PAWORK_HOME/auth.json` / `~/.pawork/auth.json`（JSON v1、0600、独立临时文件+rename 原子写、跨进程 write/refresh 锁、损坏 fail-closed，形态对齐 Codex CLI auth.json）；keyring 依赖已整体移除，旧 Keychain 条目已清理（验证 0 残留）。
...
- `cargo test -p pawork-auth`：auth 文件、OAuth PKCE/Device/refresh/callback、masked。
- `cargo test -p pawork-diagnostics`：全局脱敏规则与 token tracing 断言。
- config/app 的凭证优先级与六通道装配定向测试。

## 退出标准

- [x] 六条首发通道完成正式装配与首次登录/对话真实冒烟；延期厂商没有伪 feature、空 adapter 或预埋分支。（2026-08-15 完成：ChatGPT / xAI / glm-coding / opencode-go / qwen-token-plan / deepseek 均完成真实凭证冒烟）
- [x] auth 文件为主、env 为显式 fallback；OAuth refresh 与 credential kind fail-closed 回归通过。（2026-08-16：auth 59 通过 + 1 个跨进程 helper 按设计 ignored；app 42 通过 + 1 个真实 smoke ignored；default refresh 的进程内 singleflight、双进程旧快照复用、空 refresh 拒写、批量原子回写与永久失效重登提示均有回归；文件后端按用户决策替代 Keychain）
...
- [ ] ChatGPT 与 xAI 请求前 OAuth refresh 各完成一次真实临期凭证冒烟；轮换 token 与 default meta 持久化正确，失败时 fail-closed 要求重登。
...
- 收口（串行）：生产请求路径接入 refresh singleflight + auth 文件术语一致性 + ChatGPT/xAI 临期真实 refresh 冒烟；完成前阶段保持 🔵。
