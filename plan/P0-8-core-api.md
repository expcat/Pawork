...
> Phase 0 · 架构与协议冻结 · 状态：🟢已完成 · 依赖：P0-3
...
6. **定义 Transport 抽象 trait** —— 使用 `async-trait` 定义 GuiTransportServer/GuiListener/GuiConnection/GuiTransportClient，只搬运 opaque `TransportFrame` 字节。目的：本地与远程 GUI 复用同一协议，差异只在传输层。
...
- [x] 类型即 schema source（可生成 TS）
- [x] 覆盖关键命令、查询与事件
- [x] 命令带来源、身份与 idempotency；事件带来源与全局/流 sequence
- [x] 含 GUI 接入协议帧与 Transport 抽象 trait
- [x] 含 request id 与 API version
