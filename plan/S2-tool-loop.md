...
| `pawork-tools`（execution/tools） | 激活：V1 `builtin-tools` 的 `common` + `read_file`/`list_directory`/`search_text`/`find_files` 四模块迁移；V1 `tool-runtime` 的 scheduler 最小迁移（注册、并发上限、超时、输出截断） | 直接迁移（[archive/M1](archive/README.md) pawork-tools 节；写工具 S3、run_command S4、`tool_search` 冻结不迁） |
...
- [x] 在本仓库运行：「`ROADMAP.md` 里 S4 阶段的真实验收要点是什么？」——Agent 应 read_file 后据实回答。（2026-08-14 三通道均据实答出 cargo check 闭环 / Ctrl-C 进程树 / fail-closed；OpenCode Go 首次 `StreamInterrupted`，重试通过）
...
- [archive/M1-execution-security.md](archive/README.md)（pawork-tools/pawork-workspace 迁移细则）
- [archive/M2-providers.md](archive/README.md)（provider-anthropic 归属与 feature 组织）
- [archive/M4-engine-closed-loop.md](archive/README.md)（provider_loop 拆分目标形态——S2 起逐步对齐）
