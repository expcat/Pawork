...
4. **Citation / reasoning 回灌 fuzz** —— 目的：用 proptest/arbitrary 对畸形或边界的 citation 列表、reasoning 凭证 blob、interleaved 顺序做 fuzz，断言归一不 panic、不丢字段、凭证不被解码/落日志。
...
6. **`no_provider_branch` 守护** —— 目的：扩展 `agent-engine/tests/no_provider_branch.rs`（P6 既有），覆盖 Phase 15 新增的 hosted_tools / ReasoningItem / 协商记录不含 Provider 名称分支。
...
- [ ] fuzz 覆盖畸形 citation / reasoning 凭证 / interleaved 顺序，归一不 panic、凭证不落日志
- [ ] 旧路径（Chat Completions / P6 Messages / xAI Chat）在 v2 后行为不变（兼容性差分断言）
- [ ] `no_provider_branch` 守护覆盖 hosted_tools / ReasoningItem / 协商记录
