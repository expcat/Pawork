...
隔离目录：`PAWORK_DATA_DIR` + workspace `.pawork/config.toml`（`fixtures/config/config.example.toml`）；凭证从 `.env` 注入，未写入仓库。
...
- [archive/M3-storage-session.md](archive/README.md)（session-store 迁移细则、DDL、触发器、migration 序列）
- [archive/M0-skeleton-foundation.md](archive/README.md)（pawork-sqlite 纯化细则）
