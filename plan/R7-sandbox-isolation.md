...
> 对应 [ROADMAP.md](../ROADMAP.md) §2 R7 行。根因:S4 沙箱以「诚实标签」交付(能力有限但如实上报),V2 期间未再加深。2026-08-23 波 0 实态:macOS Seatbelt 已是 deny-default 写白名单、读因 Darwin 25+ 放开整盘;`network_allow_hosts` 是无生产赋值的内存字段而非配置项(K-09);PTY 会话不经沙箱与审批闸(gui_host 直连 `pawork-exec` PTY;波 B 创建动作已入 policy 闸);shell 风险分类靠固定词表 + 空白 tokenize(波 B 已改手写 tokenizer)。本阶段把执行面从「标签诚实」升级为「语义诚实」。安全域改动,全程定向回归护航,fail-closed 语义只紧不松。
