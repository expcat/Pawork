...
| 迁移 | schema v11:v10 数据一次性迁移(回填即校验),旧库升级 golden;信封 wire 形状不变(envelope v1 保持) | — |
| 压缩语义 | compaction 按分支水位独立计算(修 fork 后压缩误删祖先上下文的结构隐患) | — |

## 2. 目标设计

1. **storage**:`session_events`/投影表 branch 一等化;`fork_from_event` 语义收编(fork = 新 branch 记录 + 祖先引用,不复制事件);lineage 查询单点实现;v11 迁移 + 「V2 真实库文件直接打开升级」golden。
...
| 波 | 内容 | 写入集 | 并行度 |
| --- | --- | --- | --- |
| 0 | ADR-040 起草(含 v10→v11 迁移方案与回滚不可行性说明)→ 用户确认 | docs/adr/ | 串行 |
| A | storage 原生化 + v11 迁移 + 升级 golden(含 fork 树、多分支交错、压缩折叠种子) | storage | 串行(契约面) |
...
- **重放 golden 全套**:v10 库升级 v11 后逐事件重放一致;fork 分支 resume 上下文连续;压缩后祖先链读取正确。
...
- [ ] ADR-040 Accepted;schema v11 迁移 + 升级 golden 绿
