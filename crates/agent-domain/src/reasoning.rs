...
/// Canonical reasoning effort（P15-8，P17-5 起由 agent-domain 定义）。
///
/// `AgentProfileV2.effort` 以本枚举为一等字段，经 provider-api
...
mod tests {
    ...
    fn reasoning_effort_is_canonical_and_serde_stable() {
        // Canonical 枚举序列化名稳定：Profile v2 / provider-api 共用同一词汇。
        assert_eq!(
