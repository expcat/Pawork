//! Session 分支树视图、从任意事件 Fork 以及按 branch 分页读取事件。
//!
//! 大型 session 不必全量加载：[`SessionStore::events_by_branch`] 仅按 branch 与
//! sequence 区间读取，[`SessionStore::session_tree`] 只返回分支拓扑而非事件。

use agent_domain::{EventId, SessionId};
...
impl SessionStore {
    ...
    pub async fn session_tree(
        ...
        if branches.is_empty() {
            // 没有 branch 说明 session 不存在（每个 session 至少有 main 分支）。
...
mod tests {
    ...
    fn temp_path() -> PathBuf {
        ...
        std::env::temp_dir().join(format!(
            "pawork-session-tree-{}-{unique}.sqlite3",
            std::process::id()
        ))
    }
    ...
    async fn events_by_branch_paginates_within_a_branch() {
        let path = temp_path();
        let (store, _) = SessionStore::open(&path).await.expect("store");
        let session = SessionId::from("session-page");
        store
            .create_session(&session, "page", Timestamp::from_unix_millis(1))
            .await
            .expect("session");
        for sequence in 1..=3 {
            store
