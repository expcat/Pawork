...
use agent_domain::{BackgroundTaskId, TaskEvent, TaskKind, TaskStatus};
use task_manager::{is_terminal_status, TaskManager, TaskManagerError, TaskManagerState};

fn manager() -> TaskManager {
    common::manager_with_recording_backend().0
}
...
fn snapshot_and_replay_rebuild_view() {
    ...
    let restored: task_manager::TaskManagerSnapshot = serde_json::from_str(&json).unwrap();
