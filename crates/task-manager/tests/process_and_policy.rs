...
use agent_domain::{BackgroundTaskId, TaskKind, TaskStatus};
use process_runtime::{CommandSpec, ProcessEvent, ProcessRuntime};
use sandbox_runtime::{
    default_env_allowlist, NativeRestricted, NetworkMode, SandboxPolicy, SandboxProcessSpec,
};
...
fn assert_same_view(
    left: &task_manager::TaskManagerSnapshot,
    right: &task_manager::TaskManagerSnapshot,
) {
...
async fn backend_receives_exact_policy_no_escalation() {
    ...
    assert!(matches!(
        ...
        Err(TaskManagerError::Sandbox(
            sandbox_runtime::SandboxError::Denied(_)
        ))
    ));
