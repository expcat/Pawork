...
impl TaskManagerState {
    ...
    /// 命令辅助：替换任务取消令牌。
    ///
    /// process 类任务在 spawn 前必须把记录 token 换成实际传给
    /// `SandboxBackend::spawn` 的那个 token，否则 cancel 无法触达进程树。
    pub(crate) fn set_cancel_token(
        &mut self,
        task_id: &BackgroundTaskId,
...
fn output_event_size(event: &ProcessEvent) -> u64 {
    match event {
        ProcessEvent::Stdout(bytes) | ProcessEvent::Stderr(bytes) => bytes.len() as u64,
        ProcessEvent::Exit { .. } => 0,
    }
}
