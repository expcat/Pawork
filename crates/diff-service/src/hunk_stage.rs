//! Hunk / Line 级暂存（P7-7）。
//!
//! 基于结构化 [`crate::model::DiffHunk`] / [`crate::model::DiffLine`] 构造
//! 最小 unified patch，经 [`git_service::StageService::apply_patch_to_index`]
//! 用 `git apply --cached [--reverse]` 应用到 index，实现按块 / 按行暂存与
//! 取消暂存，不触碰工作区。
//!
//! 语义约定：
//! - stage_*：`file` 应来自 **worktree vs index** 的 diff
//!   （[`crate::service::DiffOptions::default`]）。
...
use git_service::{GitError, GitRunner, StageService};

use crate::model::{DiffFile, DiffHunk, DiffLine, FileStatus, HunkId, LineKind};
...
mod tests {
    use super::*;
    use crate::service::{DiffOptions, DiffService};
