//! # tenant-service
//!
//! 租户 / 主体身份（`IdentityContext` 与解析器）与 `TenantPolicy` 执行
//! （P18-2 / P18-9 最小契约，供 Phase 12 编排与 app-service 消费）。
//!
//! 本 crate 只做四件事：
//! - 提供租户 / 主体的默认身份（`local/default`、`local/user`）与
//!   [`identity::IdentityContext`]（P18-2 canonical identity）；
//! - 通过 [`identity::IdentityResolver`] 把请求主体解析为身份上下文；未配置
//!   用户固定映射到默认身份，**缺失身份 fail-closed**（返回错误，不静默放行）；
...
pub use identity::{IdentityContext, IdentityError, IdentityResolver, LocalIdentityResolver};
...
pub use agent_domain::{AccountId, AgentId, ModelId, PrincipalId, ProviderId, SessionId, TenantId};
