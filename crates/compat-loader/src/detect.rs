...
use resource_loader::ConfigTier;

use crate::io::is_file_within;
use crate::limits::CompatLimits;
use crate::model::{CompatIssue, DetectedSourceSummary};
use crate::source::{ExternalSource, SourceFileKind};
...
fn scan_glob(
    ...
    let (entries, dir_truncated) =
        match crate::io::sorted_children(&scan_dir, limits.max_dir_entries) {
            Ok(pair) => pair,
            Err(_) => return,
        };
    if dir_truncated {
...
fn scan_agents_hierarchy(
    ...
    while let Some((dir, depth)) = stack.pop() {
        ...
        let (entries, dir_truncated) =
            match crate::io::sorted_children(&current, limits.max_dir_entries) {
                Ok(pair) => pair,
                Err(_) => continue,
            };
        if dir_truncated {
