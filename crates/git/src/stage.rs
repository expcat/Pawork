...
mod tests {
    ...
    #[tokio::test]
    async fn stage_option_like_filename_via_double_dash() {
        let (_dir, repo) = make_repo();
        std::fs::write(repo.join("--force"), "payload\n").expect("write --force");
        let runner = GitRunner::new();
        let svc = StageService::new(&runner, &repo);
        svc.stage(
            &StageRequest::new(vec!["--force".into()]),
            CancellationToken::new(),
        )
        .await
        .expect("path --force must be added via --, not parsed as an option");
        let p = porcelain(&repo);
        assert!(
            p.contains("--force"),
            "staged --force filename missing: {p:?}"
        );
        assert!(
            p.contains('A'),
            "expected added index status for --force: {p:?}"
        );
    }
}
