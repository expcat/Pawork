...
//! 以 `app-service` / Core 数据实现。

use std::collections::BTreeSet;
use std::path::Path;
use std::sync::Arc;

use agent_domain::{CommandId, QueryId, WorkspaceId};
...
impl AcpClientAdapter {
    ...
    pub fn decode_permission_response(
        ...
        fn reject_unknown(fields: &serde_json::Map<String, Value>) -> Result<(), AdapterError> {
            let unknown: Vec<&String> = fields
                .keys()
                .filter(|key| key.as_str() != crate::wire::META_FIELD)
                .collect();
            if unknown.is_empty() {
                Ok(())
            } else {
    ...
    ) -> Result<crate::wire::RequestPermissionParams, AdapterError> {
