...
pub(crate) async fn terminal_settings(
    ...
    Ok(AppResponse::Data(json!({
        ...
        "columns": terminal.and_then(|config| config.columns).unwrap_or(defaults.cols),
        "rows": terminal.and_then(|config| config.rows).unwrap_or(defaults.rows),
    })))
}
...
fn validate_terminal_shell(shell: &str) -> Result<(), String> {
    ...
    let path_like =
        shell.contains('/') || (cfg!(windows) && shell.contains('\\'));
    if path_like {
