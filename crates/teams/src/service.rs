...
use orchestration::{OrchestrationEvent, TaskId, TaskState, WorkerState};
...
use crate::state::{replay, TeamAggregate};
use crate::store::{shared_store, validate_sequence_contiguity, MemoryTeamStore, TeamEventStore};
...
impl TeamService {
    ...
    ///
    /// 复用 `orchestration::replay_workers`——P12 既有的 worker 状态源
    ...
    pub fn observe_worker_events(
        ...
        let mut inner = self.inner.lock();
        let rt = self.mutable_team(&mut inner, team_id)?;
        let states = orchestration::replay_workers(events);
    ...
    fn commit_batch(
        ...
        for envelope in &envelopes {
            crate::state::apply(&mut rt.aggregate, envelope.payload.clone());
