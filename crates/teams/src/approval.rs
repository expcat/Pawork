...
//! review 状态机的折叠直接复用 [`plan_service::state::PlanState`] 与
//! [`agent_domain::PlanEvent`]，不重复实现 P16-2。执行 gate
...
pub fn build_submitted_state(
    ...
    plan_service::apply(&mut state, &PlanEvent::ReviewRequested { plan_id, version });
    state
}
    ...
    fn submitted() -> PlanState {
        build_submitted_state(
            ...
            vec![PlanStepSnapshot {
                ...
                text: "step".into(),
                status: agent_domain::PlanStepStatus::Pending,
            }],
        )
    }
