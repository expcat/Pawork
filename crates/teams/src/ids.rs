//! Team 协作层的稳定标识与成员角色。
//!
//! 复用 agent_domain::AgentId 作为成员标识（一个 team 成员就是 P12 中被
...
pub type TeamId = agent_domain::SessionId;

/// Mailbox 消息 ID（团队内单调投递标识）。
pub type MailboxMessageId = agent_domain::MessageId;

/// 一次受控 peer messaging fan-out 的追踪 ID（用于 presence / policy 审计）。
pub type FanOutId = agent_domain::CommandId;

/// 成员在 team 中的角色。
///
/// `Supervisor` 即 P12 根 parent（默认可审批 plan、变更成员）；`Worker` 是
/// 普通成员（受 [`crate::peer::PeerPolicy`] 约束的 peer messaging 参与者）。
