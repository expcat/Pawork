...
//! [`TranslatedRequest`] 后自行分发。所有错误都是显式的 [`ProtocolError`]，
//! unknown / unsupported 有独立类别。

use crate::wire::{
    CompatHistoryQuery, CompatImportRequest, HeadlessRequest, HeadlessResponse, ProtocolError,
    ProtocolErrorKind, TranslatedRequest, MAX_FRAME_BYTES,
};
use core_api::{AppEventEnvelope, AppResponseEnvelope};
use serde::Serialize;
use serde_json::Value;

/// 解析并翻译一行请求 JSON（不含换行）。
///
/// 失败返回带类别的 [`ProtocolError`]；调用方应把它编码为
/// [`HeadlessResponse::Error`] 帧返回给客户端（见 [`encode_protocol_response`]）。
pub fn translate_request_line(line: &str) -> Result<TranslatedRequest, ProtocolError> {
...
/// [`translate_request`]。
pub fn parse_request_line(line: &str) -> Result<HeadlessRequest, ProtocolError> {
    ...
    let frame: Value = serde_json::from_str(line)
        .map_err(|error| ProtocolError::malformed(format!("request frame: {error}")))?;
    ...
    if !KNOWN_TYPES.contains(&frame_type) {
        return Err(ProtocolError::unknown_request(format!(
...
/// 把请求帧翻译为可分发切片。
pub fn translate_request(request: &HeadlessRequest) -> Result<TranslatedRequest, ProtocolError> {
    match request {
        HeadlessRequest::Hello { .. } => Err(ProtocolError::new(
...
/// 最终裁决由 Host 执行）。
fn validate_envelope_version(version: core_api::ApiVersion) -> Result<(), ProtocolError> {
    if version.is_compatible_with(core_api::API_VERSION) {
        Ok(())
    } else {
...
pub fn encode_response_line(response: &AppResponseEnvelope) -> Result<String, ProtocolError> {
...
pub fn encode_event_line(event: &AppEventEnvelope) -> Result<String, ProtocolError> {
...
/// 编码任意协议响应帧为一行 JSON。
pub fn encode_protocol_response(response: &HeadlessResponse) -> Result<String, ProtocolError> {
    encode(response)
}

/// 编码请求帧为一行 JSON（客户端侧 / 测试用）。
pub fn encode_request(request: &HeadlessRequest) -> Result<String, ProtocolError> {
    encode(request)
}
...
fn encode<T: Serialize>(value: &T) -> Result<String, ProtocolError> {
    let line = serde_json::to_string(value)
        .map_err(|error| ProtocolError::malformed(format!("encode frame: {error}")))?;
    if line.len() > MAX_FRAME_BYTES {
        return Err(ProtocolError::too_large(
