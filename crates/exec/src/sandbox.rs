//! Sandbox Runtime：在 Process Runtime 之上叠加执行隔离。
//!
...
pub struct SandboxProcess {
    /// 与 ProcessRuntime::spawn_stream 一致的事件流。
    pub events: mpsc::Receiver<ProcessEvent>,
    /// 进程树生命周期守卫：`ProcessHandle::drop` 会取消 kill token 并终止整棵进程树，
    /// 因此必须与 `events` 同生命周期（`kill` 方法已删，字段保留不可移除）。
    #[allow(dead_code)]
    _handle: ProcessHandle,
}

impl SandboxProcess {
    pub(crate) fn new(events: mpsc::Receiver<ProcessEvent>, handle: ProcessHandle) -> Self {
        Self {
            events,
            _handle: handle,
        }
...
impl SandboxBackend for NativeRestricted {
    ...
    async fn spawn(
        ...
        // 3. 委托 ProcessRuntime spawn_stream，保证 IO/timeout/cancel/进程树语义一致。
...
impl SandboxSelector {
    ...
    pub fn pick(&self) -> (Box<dyn SandboxBackend>, BackendSelection) {
        ...
        #[cfg(target_os = "macos")]
        {
            let backend = crate::os::macos::SandboxExecBackend::with_runtime(self.runtime);
            let avail = backend.available();
            attempted.push(ProbeOutcome {
                backend: backend.id(),
                available: avail,
                reason: crate::os::macos::probe_reason(),
            });
            if avail {
                let id = backend.id();
                return (
        ...
        #[cfg(target_os = "linux")]
        {
            let backend = crate::os::linux::BwrapBackend::with_runtime(self.runtime);
            let avail = backend.available();
            attempted.push(ProbeOutcome {
                backend: backend.id(),
                available: avail,
                reason: crate::os::linux::bwrap_probe_reason(),
            });
            if avail {
                let id = backend.id();
                return (
            ...
            let landlock = crate::os::linux::probe_landlock_support();
            attempted.push(ProbeOutcome {
                backend: "landlock",
            ...
            if landlock.supported {
                let backend = crate::os::linux::LandlockBackend::with_runtime(self.runtime);
                return (
                    Box::new(backend),
                    BackendSelection {
                        id: "landlock",
        ...
        #[cfg(windows)]
        {
            // AppContainer 不可承载时仍使用 ProcessRuntime 的 Job Object，
            // 真实施加进程数/内存/CPU限额和 KILL_ON_JOB_CLOSE；文件/网络保持软限制。
            attempted.push(crate::os::windows::probe_appcontainer_job());
            let backend = crate::os::windows::WindowsJobBackend::with_runtime(self.runtime);
            attempted.push(ProbeOutcome {
                backend: backend.id(),
...
fn ensure_within(
    ...
    let normalized = canonicalize_platform(target)
        .map_err(|error| SandboxError::PathEscape(format!("{}: {error}", target.display())))?;
    for d in &policy.filesystem.deny {
        let denied = canonicalize_platform(d).unwrap_or_else(|_| d.clone());
        if path_within_root(&normalized, &denied) {
    ...
    let inside = roots.iter().any(|root| {
        canonicalize_platform(root)
            .map(|root| path_within_root(&normalized, &root))
            .unwrap_or(false)
    });
    if !inside {
    ...
    let policy_allows = policy_roots
        .map(|root| canonicalize_platform(root))
        .any(|root| {
            root.map(|root| path_within_root(&normalized, &root))
                .unwrap_or(false)
        });
    if !policy_allows {
        return Err(SandboxError::Denied(format!(
