...
async fn run(
    ...
    let mut process = match spawn_result {
        ...
        Err(error) => {
            ...
            return Err(RunCommandError::Sandbox(error.to_string()));
        }
    };
    ...
    let event_result: Result<(), RunCommandError> = async {
...
mod tests {
    ...
    async fn emits_output_before_process_exits() {
        ...
        #[cfg(not(windows))]
        // `echo` 在子进程退出时刷新管道；纯 `printf` 在 pipe 上全缓冲，
        // 会把 first 留到整段脚本结束，误报「进程已退出」。
        let input = json!({"argv": ["sh", "-c", "echo first; /bin/sleep 1; echo second"]});
    ...
    fn descriptor_does_not_offer_model_controlled_network_bypass() {
        ...
        assert_eq!(descriptor.capability, ToolCapability::Process);
        assert!(!descriptor.requires_approval);
        assert!(!descriptor.read_only);
        assert!(!descriptor.supports_concurrency);
