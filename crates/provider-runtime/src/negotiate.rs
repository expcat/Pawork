...
//!
//! 两层交集语义：
//! - 证据层（[`model_registry::merge_capabilities`]）：已出现来源逐字段取交集，
...
//! （P6 基线）并记录 `LegacyTransport` fallback。
//!
...
/// 把请求的 canonical `ReasoningConfig` clamp 为旧 P6 `ThinkingConfig`。
///
...
mod tests {
    ...
    fn evidence_from(caps: ModelCapabilities) -> CapabilityEvidence {
        CapabilityEvidence {
            model: agent_domain::ModelId::new("test-model"),
            provider: None,
    ...
    fn full_caps() -> ModelCapabilities {
        ModelCapabilities {
            ...
            .collect(),
            citations: true,
            reasoning: provider_api::ReasoningStateCapability {
                state: provider_api::ReasoningStateDescriptor {
    ...
    fn unsupported_capability_is_fail_closed_and_requested_covered() {
        ...
        caps.thinking = false;
        caps.reasoning = provider_api::ReasoningStateCapability::default();
    ...
    fn evidence_layer_intersection_is_respected() {
        ...
        let override_caps = ModelCapabilities {
            thinking: false,
            reasoning: provider_api::ReasoningStateCapability::default(),
        ...
        let evidence = CapabilityEvidence {
            model: agent_domain::ModelId::new("m"),
            provider: None,
