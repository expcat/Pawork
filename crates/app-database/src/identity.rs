...
//!
//! 创建版本化、tenant-bound 的 `identity_tenants` 注册表，并把 legacy 本地
//! 单用户身份（`tenant_id = local/default`、`principal_id = local/user`）幂等
//! 种入（INSERT OR IGNORE）。迁移本身经 [`crate::migration`] 运行器在单事务内
//! 执行：任一迁移失败整批回滚，已存在的库先备份（可整库恢复），账本表保证
//! 重复执行为 no-op（幂等）。
//!
//! 本模块只依赖迁移运行器与 rusqlite，不依赖 tenant-service / provider-control，
//! 避免把领域行为类型拉入存储层（依赖方向：app-database → rusqlite）。
...
pub async fn schema_version(database: &DatabaseActor) -> Result<u32, MigrationError> {
    migration::schema_version(database, IDENTITY_MIGRATIONS_TABLE).await
}
...
mod tests {
    ...
    async fn failed_migration_rolls_back_whole_batch() {
        ...
        let error = migration::migrate(&actor, IDENTITY_MIGRATIONS_TABLE, &[bad], 1, &path, false)
            .await
            .expect_err("应迁移失败");
        assert!(matches!(
            error,
