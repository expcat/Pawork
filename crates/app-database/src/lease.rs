...
//! 非明文。
//!
//! 依赖方向：`app-database → rusqlite`（不依赖 `provider-control`，避免把控制面
//! 行为类型拉入存储层；本模块只持久化扁平行，宿主组合层在两侧间转换）。
...
/// `provider_control::lease::LEASE_SCHEMA_VERSION` 是两个独立版本轴，数值无需相同。
...
pub async fn schema_version(database: &DatabaseActor) -> Result<u32, MigrationError> {
    migration::schema_version(database, LEASE_MIGRATIONS_TABLE).await
}
...
mod tests {
    ...
    async fn failed_migration_rolls_back_batch() {
        ...
        let error = migration::migrate(&actor, LEASE_MIGRATIONS_TABLE, &bad, 2, &path, false)
            .await
