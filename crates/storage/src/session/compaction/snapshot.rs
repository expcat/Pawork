...
//! [`UnsupportedSnapshotVersion`](crate::session::compaction::CompactionError::UnsupportedSnapshotVersion)。
//!
//! serde 形状是 V1 冻结契约：字段名、`SnapshotVersion` / `EventSequence` 的
//! transparent 表示均不得改变。

use pawork_domain::{EventId, EventSequence};
use serde::{Deserialize, Serialize};

use crate::session::compaction::CompactionError;
...
mod tests {
    ...
    fn snapshot_serde_shape_matches_v1_frozen_contract() {
        ...
        assert_eq!(
            ...
            serde_json::json!({
                "version": 1,
                "summary": "前期讨论已折叠：用户要求实现压缩引擎。",
                "retained_event_ids": ["event-7", "event-8"],
                "replaced_range": [1, 6],
                "token_usage_before": 12_000,
                "token_usage_after": 3_000,
