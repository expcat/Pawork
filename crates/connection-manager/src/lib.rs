...
//!   `session.lagged` 并丢弃新事件，绝不阻塞发布者或其他 GUI。
...
use core_api::{ActorIdentity, AppEventEnvelope, EventStream, GlobalSequence};
use gui_protocol::GuiCapability;
use thiserror::Error;
use tokio::sync::mpsc;
use transport_api::ConnectionLocality;
...
impl ConnectionManager {
    ...
    /// [`ManagerError::Lagged`]（事件被丢弃，不阻塞发布者）。
    pub fn enqueue(
        &self,
        client_id: &GuiClientId,
...
mod tests {
    use super::*;
    use agent_domain::{CoreInstanceId, EventId, RunId};
    use core_api::{AppEvent, EventSource, RunState, API_VERSION};
