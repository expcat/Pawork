...
//! `target/debug/pawork`；两者都不可用时跳过（定向测试不要求先构建二进制）。

use std::path::PathBuf;
use std::time::Duration;

use agent_domain::{SessionId, WorkspaceId};
use agent_sdk::{BackpressurePolicy, PaworkClient, PaworkOptions, SdkErrorKind};
use core_api::{AppCommand, AppQuery, AppResponse, EventStream};
use headless_json::{CompatSource, ProtocolErrorKind, SdkCapability};
use serde_json::Value;

fn pawork_binary() -> Option<PathBuf> {
    if let Ok(binary) = std::env::var("PAWORK_BIN") {
...
async fn spawns_real_pawork_and_round_trips() {
    let Some(binary) = pawork_binary() else {
        eprintln!("SKIP: pawork binary not found; run `cargo build -p pawork` first");
        return;
    };
