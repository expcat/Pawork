...
use core_api::{ApiVersion, SUPPORTED_API_VERSIONS};

/// SDK 自身语义化版本（crate 版本；按 semver 演进）。
pub const SDK_VERSION: &str = env!("CARGO_PKG_VERSION");

/// SDK 期望的 `core-api` 协议版本（与 Host 握手时协商）。
///
...
pub const SDK_API_VERSION: ApiVersion = core_api::API_VERSION;
...
pub fn sdk_version_string() -> String {
    format!("agent-sdk {SDK_VERSION} (protocol {SDK_API_VERSION:?})")
}
...
mod tests {
    ...
    fn sdk_api_version_is_current() {
        assert_eq!(SDK_API_VERSION, core_api::API_VERSION);
        assert!(SDK_API_VERSION.is_compatible_with(core_api::API_VERSION));
    }
