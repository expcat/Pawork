...
//! 它是纯 client，通过稳定 NDJSON framing（[`headless_json`]）驱动 Host，
//! 把 [`AppCommand`](core_api::AppCommand) / [`AppQuery`](core_api::AppQuery)
//! 映射为请求、把 [`AppEvent`](core_api::AppEvent) 流式暴露给调用方。
//!
//! ```text
//! Rust Application → agent-sdk (PaworkClient)
...
//! use agent_sdk::{PaworkClient, PaworkOptions};
...
//!   `core-api` 协议版本，握手时与 Host 协商，major 不兼容即显式失败。
...
/// 便捷入口：`agent_sdk::spawn_pawork(options)`。
...
/// 便捷重导出：常用协议类型（与 `core-api` 同构，免去额外依赖）。
pub mod reexport {
    pub use core_api::{
