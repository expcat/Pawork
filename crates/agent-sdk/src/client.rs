...
impl PaworkClient {
    ...
    async fn exchange(
        ...
        let line = headless_json::translate::encode_request(&request)
        ...
        let frame = self
            .exchange(
                ...
                HeadlessRequest::CompatImport {
                    ...
                    source,
                    content,
                    options: Some(headless_json::CompatImportOptions { dry_run }),
                },
                self.options.timeout,
            )
            .await?;
    ...
    pub fn translate_request(request: &HeadlessRequest) -> Result<TranslatedRequest, SdkError> {
        headless_json::translate::translate_request(request)
            .map_err(|error| SdkError::from_error_frame(error.kind, error.message))
    }
}
