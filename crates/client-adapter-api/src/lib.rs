...
pub trait ClientAdapter: Send + Sync {
    ...
    async fn decode(&self, frame: ClientFrame) -> Result<CanonicalClientRequest, AdapterError> {
        frame.validate()?;
        self.decode_payload(frame).await
    }

    async fn encode(&self, frame: CanonicalCoreFrame) -> Result<ClientFrame, AdapterError> {
...
mod tests {
    ...
    async fn mock_adapter_encodes_all_canonical_frames() {
        use agent_domain::{CoreInstanceId, QueryId};
        use core_api::{
    ...
    fn client_frame_validates_schema_and_preserves_extensions() {
        ...
        let frame: ClientFrame = serde_json::from_value(raw.clone()).expect("decode");
