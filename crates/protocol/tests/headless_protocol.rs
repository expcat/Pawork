...
trait UnwrapOrPanic<T> {
    fn err_or_panic(self, name: &str) -> pawork_protocol::headless::HeadlessError;
}

impl<T> UnwrapOrPanic<T> for Result<T, pawork_protocol::headless::HeadlessError> {
    fn err_or_panic(self, name: &str) -> pawork_protocol::headless::HeadlessError {
        self.err()
...
mod stdio_tests {
    use super::*;
    use pawork_protocol::headless::stdio::{run_loop, LoopConfig};
    use pawork_protocol::headless::translate::encode_request;
    use pawork_protocol::headless::wire::HelloRequest;
    ...
    impl pawork_protocol::headless::stdio::Handler for EchoHandler {
        ...
        async fn handle(&mut self, request: TranslatedRequest) -> Vec<HeadlessResponse> {
            match request {
                TranslatedRequest::Command(envelope) => vec![HeadlessResponse::Response {
                    envelope: pawork_protocol::AppResponseEnvelope {
                        ...
                        response: pawork_protocol::AppResponse::Accepted {
                ...
                TranslatedRequest::Query(envelope) => vec![HeadlessResponse::Response {
                    envelope: pawork_protocol::AppResponseEnvelope {
                        ...
                        responded_at: Timestamp::from_unix_millis(1),
                        response: pawork_protocol::AppResponse::Data(json!({"ok": true})),
                    },
                }],
    ...
    fn sample_command_line() -> String {
        let request = HeadlessRequest::Command {
            envelope: pawork_protocol::AppCommandEnvelope {
                ...
                identity: pawork_protocol::ActorIdentity::Automation {
                    name: "test".into(),
                },
                expected_revision: None,
                idempotency_key: None,
                issued_at: Timestamp::from_unix_millis(0),
                command: pawork_protocol::AppCommand::RunCancel {
    ...
    fn bad_version_command_line() -> String {
        let request = HeadlessRequest::Command {
            envelope: pawork_protocol::AppCommandEnvelope {
                api_version: pawork_protocol::ApiVersion { major: 9, minor: 0 },
    ...
    async fn run_loop_translates_requests_to_responses() {
        ...
        match frame {
            HeadlessResponse::Response { envelope } => {
                assert!(matches!(
                    envelope.response,
                    pawork_protocol::AppResponse::Accepted { .. }
                ));
            }
    ...
    async fn run_loop_interleaves_events_with_responses() {
        ...
        handler.pending_events.push_back(HeadlessResponse::Event {
            envelope: pawork_protocol::AppEventEnvelope {
                api_version: pawork_protocol::API_VERSION,
                instance_id: pawork_domain::CoreInstanceId::from("test-instance"),
                event_id: pawork_domain::EventId::from("evt-1"),
                global_sequence: pawork_protocol::GlobalSequence(1),
    ...
    async fn stdio_writer_backpressure_rejects_unflushed_burst() {
        use pawork_protocol::headless::stdio::StdioWriter;
