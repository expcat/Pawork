...
fn all_client_frame_variants_round_trip() {
    for frame in sample_client_frames() {
        ...
        let decoded = pawork_protocol::decode_client_frame(&bytes).expect("decode client frame");
...
fn all_server_frame_variants_round_trip() {
    for frame in sample_server_frames() {
        ...
        let decoded = pawork_protocol::decode_server_frame(&bytes).expect("decode server frame");
...
fn oversized_payload_is_rejected_on_encode() {
    ...
    assert!(matches!(
        ...
        Err(pawork_protocol::ProtocolCodecError::FrameTooLarge { .. })
    ));
}
...
fn oversized_artifact_chunk_is_rejected_on_encode_and_decode() {
    ...
    assert!(matches!(
        ...
        Err(pawork_protocol::ProtocolCodecError::ArtifactChunkTooLarge { .. })
    ));

    let bytes = serde_json::to_vec(&frame).expect("serialize oversized chunk");
    assert!(matches!(
        pawork_protocol::decode_server_frame(&bytes),
        Err(pawork_protocol::ProtocolCodecError::ArtifactChunkTooLarge { .. })
    ));
}
...
fn snapshot_validation_runs_on_encode_and_decode() {
    ...
    assert!(matches!(
        encode_server_frame(&frame),
        Err(pawork_protocol::ProtocolCodecError::AmbiguousSnapshotSection)
    ));
...
fn oversized_encoded_frame_is_rejected_on_decode() {
    ...
    assert!(matches!(
        pawork_protocol::decode_client_frame(&bytes),
        Err(pawork_protocol::ProtocolCodecError::FrameTooLarge { .. })
    ));
}
