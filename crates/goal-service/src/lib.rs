...
//!
//! # 设计要点
//! - canonical 领域类型与事件载荷复用 `agent_domain::workflow`（`GoalEvent` /
//!   `GoalStatus` / `CriterionKind` / `SuccessCriterionSnapshot`）与
//!   `agent_domain::ids`；事件经 `agent_events::AgentEvent::Goal` 持久化
...
//!   满足位是命令面维护的运行内存事实；进度以 `ProgressUpdated` 命中率快照
//!   持久化可追溯，状态机 / 进度 / 转向历史 / 剩余预算均可从事件流恢复。
