...
//! v2（P17-5）：`agent_domain::AgentProfileV2` 全维度（prompt / model /
//! canonical effort / tools(denied) / skills / mcp / permissions / hooks /
//! memory / max-turns / background / isolation）。加载时自动迁移 v1→v2，
//! deny-first 校验工具规则、校验引用格式与跨类解析（skills / hooks 在本
//! bundle 内解析，失败即报错；mcp / permissions 由消费方解析）、拒绝明文
//! secret。memory 默认 off；生产记忆不可用时显式标注 `Unavailable`，绝不
//! 虚假可用。
...
/// 高 tier 覆盖低 tier。跨类引用（skills / hooks）由
...
/// 跨类引用解析（fail-closed）：skills / hooks 引用必须在本 bundle 已加载的
/// 集合中解析成功；解析失败的 profile 被移除并给出 error 诊断（报错而非
/// 静默降级，防止引用缺失时行为漂移）。mcp / permissions 由消费方子系统
/// 解析，本层只做格式校验（解析时已执行）。
pub(crate) fn resolve_profile_references(
    ...
    resolution.effective.retain(|name, loaded| {
        ...
        for reference in &loaded.profile.hooks {
            if reference.pins_version() {
                issues.push(
                    ResourceIssue::error(
                        "agent_profile_hook_ref_version_unsupported",
                        format!(
                            "profile '{name}' pins a version on hook '{}', but hooks have no version",
                            reference.id
                        ),
                    )
                    .for_resource(
            ...
            if !hooks.iter().any(|hook| hook.id == reference.id) {
                issues.push(
                    ResourceIssue::error(
                        "agent_profile_hook_ref_unresolved",
                        format!(
                            "profile '{name}' references hook '{}' which is not loaded",
                            reference.id
                        ),
                    )
                    .for_resource(
...
mod tests {
    ...
    fn v2_profile_loads_all_dimensions_and_migrates_v1_compat_view() {
        ...
        assert_eq!(
            ...
            agent_domain::ToolPolicyDecision::Denied
        );
        assert_eq!(
            loaded.profile.tools.policy("read_file"),
            agent_domain::ToolPolicyDecision::Allowed
        );
        assert_eq!(
        ...
        assert_eq!(
            ...
            agent_domain::ProfileMemoryAvailability::Disabled
        );
    ...
    #[test]
    fn unresolved_skill_and_hook_refs_are_errors_but_mcp_permissions_pass_format() {
        let workspace = tempfile::tempdir().expect("workspace");
        write_profile(
            workspace.path(),
        ...
        assert!(resolution
            ...
            .issues
            .iter()
            .any(|issue| issue.code == "agent_profile_hook_ref_unresolved"));
        assert!(
            resolution.profiles_v2.is_empty(),
            "profile with unresolved refs must be dropped"
        );
    }
    ...
    fn memory_is_default_off_and_never_falsely_available() {
        ...
        assert_eq!(
            ...
            agent_domain::ProfileMemoryAvailability::Enabled
        );
