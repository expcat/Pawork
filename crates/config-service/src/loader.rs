...
pub(crate) fn parse_file(path: &Path) -> Result<(ConfigValue, PaworkConfig), ConfigError> {
    ...
    let config: PaworkConfig = serde_json::from_value(json_value.clone()).map_err(|source| {
...
fn resolve_sources(sources: Vec<ConfigSource>) -> Result<ResolvedConfig, ConfigError> {
    if let Some(err) = find_first_error(&sources) {
        return Err(err);
    }
...
/// 若有来源在构造时记录了延迟错误（例如文件解析失败），取出第一个。
fn find_first_error(_sources: &[ConfigSource]) -> Option<ConfigError> {
    // 错误在 Loader::with_file 中已直接推迟；这里保留钩子用于未来扩展。
    None
}
