...
use super::error::ResourceFileError;
use super::io::{join_under_root, read_utf8_bounded_within, sorted_children_within};
use super::request::{ResourceLimits, ResourceSelection};
use super::source::{
...
pub(crate) fn load_skills(
    ...
    for (index, root) in workspace_roots.iter().enumerate() {
        let skills_dir = match join_under_root(root, &format!("{workspace_resource_dir}/skills")) {
            Ok(dir) => dir,
            Err(_) => continue,
        };
...
fn load_skill_dir(
    ...
    let raw: RawManifest = super::io::parse_toml_resource(
        &manifest_content,
    ...
    let (frontmatter, skill_markdown) = split_skill_markdown(&skill_markdown);
    if manifest.description.trim().is_empty() {
        if let Some(description) = frontmatter.description {
            manifest.description = description;
        }
    }
    let _ = frontmatter.name;
...
/// 可选读取 SKILL.md YAML frontmatter 的 `name` / `description`；未知字段忽略。
fn split_skill_markdown(content: &str) -> (SkillFrontmatter, String) {
    let rest = content
        .strip_prefix("---\n")
...
fn validate_declared_path(raw: &str) -> Result<(), ()> {
    if super::io::is_safe_relative_reference(raw) {
        Ok(())
    } else {
        Err(())
    }
...
fn valid_identifier(value: &str) -> bool {
    super::io::is_valid_identifier(value, true)
}
...
"#,
    ...
    #[test]
    fn skill_md_frontmatter_fills_empty_description_and_ignores_unknown_fields() {
        let global = tempdir().unwrap();
        let dir = global_skills_dir(global.path()).join("front");
        fs::create_dir_all(&dir).unwrap();
        fs::write(
            dir.join("manifest.toml"),
            r#"id = "front"
...
"#,
        ...
        assert_eq!(active_ids(&res), vec!["front".to_string()]);
        assert_eq!(res.skills[0].manifest.description, "from frontmatter");
        assert_eq!(res.skills[0].skill_markdown, "# body\n");
    }
}
