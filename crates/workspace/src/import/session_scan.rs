...
impl LocalSessionRoots {
    ...
    pub fn detect() -> Result<Self, CompatError> {
        let base = directories::BaseDirs::new().ok_or_else(|| {
            CompatError::Invalid("cannot resolve user home directory".into())
        })?;
...
fn walk_session_dir(
    ...
    for entry in entries {
        let metadata = std::fs::symlink_metadata(&entry)
            .map_err(|error| CompatError::io(&entry, error))?;
...
mod tests {
    ...
    fn scans_claude_projects_recursively_and_stably() {
        ...
        let files =
            scan_local_sessions(LocalSessionSource::Claude, &roots).expect("scan claude");
        ...
        expected.sort();
        assert_eq!(paths, expected.iter().map(|path| path.as_path()).collect::<Vec<_>>());
        assert!(files
            .iter()
    ...
    fn exceeding_scan_depth_returns_limit_error() {
        ...
        let roots = LocalSessionRoots::from_home(home.path());
        let error = scan_local_sessions(LocalSessionSource::Claude, &roots)
