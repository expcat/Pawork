...
维护工作区目录（canonicalize 后的 roots）、把相对路径交给 policy 内核做越界检查、扫描/监视文件索引、按 Builtin&lt;Global&lt;Profile&lt;Workspace&lt;Session&lt;Run 合并 TOML 配置、加载 AGENTS.md / Skills / profiles，以及只读导入本机 Claude/Codex/Grok/Cursor/Pi 配置。R1 波 B 并入原 resources / config / compat。
...
src/
  ...
  import/{detect,parse,apply,source,mcp,...}.rs
tests/
...
- **import**：`CompatLoader::{scan, dry_run, export_plan}`；`ExternalSource::{Claude,Codex,Grok,Cursor,Pi}`。扫描/预览不执行、不联网。
