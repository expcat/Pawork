//! Zhipu GLM contract and OpenAI-compatible differential tests.
...
use provider_zhipu::{ZhipuConfig, ZhipuProvider};
...
fn request(model: &str) -> CanonicalModelRequest {
    CanonicalModelRequest {
        request_id: agent_domain::RequestId::from("zhipu-r1"),
        ...
        trace_id: Some("trace-zhipu".into()),
    }
}

fn provider(server: &MockServer) -> ZhipuProvider {
    let mut config = ZhipuConfig::new(server.uri());
    config.http = provider_runtime::http::HttpClientConfig::builder()
        .disable_system_proxy()
        .build();
    ZhipuProvider::new(
        config,
        Some(ResolvedCredential::new(CredentialKind::ApiKey, "id.secret")),
    )
    .expect("build Zhipu adapter")
}
...
async fn unified_contract_api_key_text_reasoning_usage_and_stop() {
        ...
        .respond_with(
            ResponseTemplate::new(200)
                ...
                .set_body_string(sse_body(&[
                    ...
                    r#"{"choices":[],"usage":{"prompt_tokens":8,"completion_tokens":5}}"#,
    ...
    let adapter = provider(&server);
    let sink = RecordingProviderSink::default();
    let summary = adapter
        .stream(request("glm-4.6"), &sink, CancellationToken::new())
        .await
        .expect("Zhipu stream succeeds");

    assert_eq!(adapter.id(), ProviderId::new("zhipu"));
...
#[tokio::test]
async fn zhipu_errors_are_normalized_independently() {
    let cases = [
        (
            429,
            "too many requests",
        ...
        (
            400,
            r#"{"code":"1113","msg":"insufficient balance"}"#,
            ProviderErrorKind::QuotaExceeded,
            None,
        ),
        (
            400,
            r#"{"code":"1301","msg":"content filtered"}"#,
            ProviderErrorKind::ContentFiltered,
            None,
        ),
    ];
    for (status, body, expected, retry_after) in cases {
        let server = MockServer::start().await;
...
async fn differential_matches_openai_compatible_baseline() {
    ...
    let zhipu_sink = RecordingProviderSink::default();
    let baseline_sink = RecordingProviderSink::default();
    let zhipu_summary = provider(&server)
        .stream(request("glm-4.6"), &zhipu_sink, CancellationToken::new())
        .await
        .expect("Zhipu stream succeeds");
    let baseline_summary = baseline_provider(&server)
        .stream(request("glm-4.6"), &baseline_sink, CancellationToken::new())
        .await
        .expect("baseline stream succeeds");

    assert_eq!(zhipu_sink.events(), baseline_sink.events());
    assert_eq!(zhipu_summary, baseline_summary);
