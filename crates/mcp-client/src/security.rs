...
impl Drop for ResolvedSecret {
    fn drop(&mut self) {
        // Remove logical access to the value before the allocation is released.
        self.secret.clear();
    }
}

/// Map an [`AuthError`] into an [`McpError::Secret`].
///
/// `AuthError` never carries plaintext (see auth-service red-line), so its
/// `Display` is safe to embed here.
