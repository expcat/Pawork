...
use agent_domain::{SessionId, TerminalSessionId};
use parking_lot::Mutex;
use portable_pty::{native_pty_system, CommandBuilder, MasterPty, PtySize};
use process_runtime::{ProcessLimits, ProcessTreeGuard};
use thiserror::Error;
...
impl Default for PtyCreateSpec {
    fn default() -> Self {
        Self {
            owner_session: SessionId::new("default"),
            shell: None,
            args: Vec::new(),
            cwd: None,
            env: Vec::new(),
...
pub enum PtyError {
    ...
    Ownership(TerminalSessionId, SessionId),
...
struct ServiceInner {
    sessions: Mutex<HashMap<TerminalSessionId, Arc<SessionInner>>>,
    next_id: AtomicU64,
...
impl PtyService {
    ...
    pub async fn create(&self, spec: PtyCreateSpec) -> Result<TerminalSessionId, PtyError> {
        ...
        let terminal_id = TerminalSessionId::new(format!("pty-{id_num}"));
    ...
    /// 列出归属某 owner 的终端会话。
    pub fn list_for_owner(&self, owner: &SessionId) -> Vec<TerminalSessionId> {
        self.inner
            .sessions
            .lock()
            .iter()
    ...
    /// 清理某 owner 的全部会话（幂等）。
    pub async fn cleanup_owner(&self, owner: &SessionId) -> Result<usize, PtyError> {
        let ids: Vec<TerminalSessionId> = self.list_for_owner(owner);
        let count = ids.len();
        for id in ids {
...
mod tests {
    ...
    fn owner(name: &str) -> SessionId {
        SessionId::new(name)
    }

    fn echo_spec(owner_session: SessionId) -> PtyCreateSpec {
        #[cfg(windows)]
        {
            PtyCreateSpec {
                owner_session,
    ...
    fn sleep_spec(owner_session: SessionId) -> PtyCreateSpec {
        #[cfg(windows)]
        {
            PtyCreateSpec {
                owner_session,
    ...
    fn process_tree_spec(owner_session: SessionId) -> PtyCreateSpec {
        #[cfg(windows)]
        {
            PtyCreateSpec {
                owner_session,
